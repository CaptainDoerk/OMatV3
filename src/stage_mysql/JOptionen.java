...
//	JButton					butStarten;
	...
	PanFinale				panFinale;
 	...
 	public JOptionen(int intDebugLvl, DbAccess myAccess)
    {
		...
		labGruppen = new JLabel(String.valueOf(intGrupp), SwingConstants.CENTER); 
		...
		lDcGruppen = new JLabel("Gruppen", SwingConstants.CENTER);
        ...
        labMannsch = new JLabel(String.valueOf(intTeams), SwingConstants.CENTER); 
		...
		lDcMannsch = new JLabel("Teams pro Gruppe", SwingConstants.CENTER);
		...
		labHalbZtn = new JLabel(String.valueOf(intPerio), SwingConstants.CENTER); 
		...
		lDcHlbZeit = new JLabel("Spielabschnitt", SwingConstants.CENTER);
		...
		jtxSpDauer.setHorizontalAlignment(SwingConstants.CENTER);
		...
		labSpDauer = new JLabel("Dauer pro HZ (mm:ss)", SwingConstants.CENTER);
		...
		jcbSchuet.addMouseListener(this);
		...
		jcbKCMode.addMouseListener(this);
		...
		jcbFTPUpl.addMouseListener(this);
...
//		butStarten.setBounds	( 20, 600,  40,  40);
		...
		labInfWndw 	= new JTextArea();
		labInfWndw.setBounds	( 20, 580, 320,  60);
		labInfWndw.setMargin(new Insets(5, 5, 5, 5));
		labInfWndw.setBackground(Color.white);
		labInfWndw.setFont(fontSml);
		labInfWndw.setLineWrap(true);
		labInfWndw.setWrapStyleWord(true);
		
		//JPanels
		jtpRechts.setBounds   (360,  20, 640, 700);
		panVorrunde	= new PanVorrunde(intDebugLvl, myAccess);
		panFinale	= new PanFinale(intDebugLvl);
	...
	public void init()
	{
		...
		grpRunde.add(jrbZwirun);		add(jrbZwirun);		jrbZwirun.addActionListener(this);
		...
		if(bolFTPUpl){ jcbFTPUpl.setSelected(true); }else{ jcbFTPUpl.setSelected(false); }
...
//		add(butStarten);	butStarten.addActionListener(this);
	...
	static String setDauer(int myDauer)
	{
		int min = myDauer / 60;
		int sec = myDauer % 60;
		if (sec <= 9)
...
*/
	...
	public void sendData()
	{
		...
		bolFTPUpl		= jcbFTPUpl.isSelected();
		/*
		 * Anlage Spielplan
		...
		if(jrbVorrun.isSelected())
		{
			...
			try
			{
				if(myResultSet.next())
				{
					 ...
					 */
					sout.println(1, "JOptionen.sendData(): Update.. ");
					//Update in Tabelle "turniere"
				...
				{
					sout.println(1, "JOptionen.sendData(): Neuanlage..");
...
//				myAccess.delete(strTurnierName + "_tms", "true");	//Falls die Tabelle schon existierte, sie leeren.
				//Tabellen füllen
				String 	strGruppe;
				int i = 0;
				int id = 0;
...
//			myApp.init(strTurnierName);
	...
	public void mouseClicked(MouseEvent arg0) 
	{
		...
		if(arg0.getSource() instanceof JLabel) 
		{
			...
			sout.println(8, "JOptionen.mouseClicked(): aktuelles Team " + strActul);
			String myString = myJChooser.choose("Teams");
			sout.println(8, "ausgewählt: " + myString);
			if (myString.equals("")) myString = strActul;
			l.setIcon(new Icons().getIcon(myString, "_Wappen", 40));
			sout.println(8, "JOptionen.mouseClicked: " + myString);
			l.setText(myString);
			checkDoubles();
		}
	}
	...
	{
		if(arg0.getSource() == jcbSchuet)	labInfWndw.setText("Öffnet ein Fenster zur Torschützenauswahl");
		if(arg0.getSource() == jcbKCMode)	labInfWndw.setText("Spiel 'bis 10' Tore, bei 9:9 Toren geht das Spiel 'bis 11'");
		if(arg0.getSource() == jcbFTPUpl)	labInfWndw.setText("Instant-FTP-Upload des Turnierverlaufs (Internetverbindung erforderlich)");
	}
