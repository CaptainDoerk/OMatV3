...
public class SpielPlan 
{
	...
	public SpielPlan(int intDebugLvl)
	{
	...
	public void createVorrunde(String strTurnierName, int intAnzGruppen, int intAnzTeams, int intModus)
	{
		...
		if (intModus == 0)											//Immer ein kompletter Spieltag pro Gruppe --> KC-Style
		{
			...
			{
				sout.println(intDebugLvl, (r+1) + "ter Spieltag:");
				sout.println(intDebugLvl, "-----------------");
				if(r%2==0)//ungerader Spieltag
				{
					...
					for(int g=0; g<intGruppen; g++)
					{
						...
						{
							teamA = spielplan[(r * intSpRunde * 2) + h];
							teamB = spielplan[(r * intSpRunde * 2) + h + 1];
							if(strGruppe.equals("b"))
							{
								teamA += intAnzTeams;
								teamB += intAnzTeams;
							}	
							if(strGruppe.equals("c"))
							{
								teamA += intAnzTeams * 2;
								teamB += intAnzTeams * 2; 
							}	
							if(strGruppe.equals("d"))
							{
								teamA += intAnzTeams * 3;
								teamB += intAnzTeams * 3;
							}	
							if(strGruppe.equals("e"))
							{
								teamA += intAnzTeams * 4;
								teamB += intAnzTeams * 4;
							}	
							if(strGruppe.equals("f"))
							{
								teamA += intAnzTeams * 5;
								teamB += intAnzTeams * 5;
							}	
							if(strGruppe.equals("g"))
							{
								teamA += intAnzTeams * 6;
								teamB += intAnzTeams * 6;
							}	
							if(strGruppe.equals("h"))
							{
								teamA += intAnzTeams * 7;
								teamB += intAnzTeams * 7;
							}	
							sout.println(intDebugLvl, teamA + ":" + teamB + " in Gruppe " + strGruppe);
							myAccess.insert(strTurnierName + "_spl", "Team1, Team2, Kommentar, Gruppe", "'" + teamA + "', '" + teamB + "', '0', '" + strGruppe + "'");
						}
					}
				}
				else //gerader Spieltag
				{
					...
					for(int g=0; g<intGruppen; g++)
					{
						...
						for(int h=0; h<(intSpRunde*2); h+=2)
						{
							...
							sout.println(intDebugLvl, teamB + ":" + teamA + " in Gruppe " + strGruppe);
							myAccess.insert(strTurnierName + "_spl", "Team1, Team2, Kommentar, Gruppe", "'" + teamB + "', '" + teamA + "', '0', '" + strGruppe + "'");
						}
					}
				}
				sout.println(intDebugLvl, "-----------------");
...
//						System.out.print(r + " " + intAnzTeams + " " + h + "  ");
	...
	public void createEndrunde(int intAnzTeams, int intModus)
	{
		...
		{	
			//todo
			myResultSet = myAccess.getSet(tournamentName + "_tbl", "*", "true ORDER BY Punkte DESC, (ToreP - ToreM) AS DIFF DESC LIMIT 1, 4");
		}
