...
//	PanPenalty			panPenalty;			//TODO Ärger mit Sirenen
	...
	boolean				bolFTPUpl = false;						//Instant-FTP-Upload
	boolean				bolMP3Btn = true;
	...
	public void init(String strTurnierName)
	{
		...
		myResultSet		= myAccess.getSet("__turniere", "*", "TurnierName = '" + strTurnierName + "'");
		try
		{
			myResultSet.next();
		...
		try
		{
			...
			{
		    	jtpOmat.addTab("Gruppe " + myResultSet.getString("Gruppe").toUpperCase(), new PanGruppe(intDebugLvl, strTurnierName, myResultSet.getString("Gruppe")));
			}
		}
		catch(SQLException ex)
		{
		...
		panSpielPl		= new PanSpielPlan(intDebugLvl, strTurnierName);
		scPanSpielPl 	= new JScrollPane(panSpielPl);
    	jtpOmat.addTab("Spielplan", scPanSpielPl);

    	//gewünschte Module hinzu
		if(bolTorAnz) myJTorAnzeige	= new JTorAnzeige(intDebugLvl, strTurnierName);
		if(bolSchuet)
		{
			panSchuetzen	= new PanSchuetzen(intDebugLvl, strTurnierName);
	    	jtpOmat.addTab("Schützen", 	panSchuetzen);
		}
		if(bolFTPUpl) myFTPUpload = new FTPUpload();
		if(bolMP3Btn)
		{
...
//		jtpXtra.setSelectedComponent(panPenalty);								//TODO Ärger mit Sirenen
		...
		try
		{
		    ...
		    myWriter = new HTMLWriter(intDebugLvl, strTurnierName);
		    myWriter.writeAll(labClock.getText());
        	if(bolFTPUpl)myFTPUpload.uploadToFTP(strTurnierName);
		...
		{
			sout.println(1, "OMat.init(Zeile 368): intSpielID ist NULL \n " + ex );
			intSpielID = 0;
			showZeroGamesBox();
		}
	    showSpiel();
...
/*				myAccess.fireSQL("ALTER TABLE `" + strBase + "`.`" + strSpBerName + "` " +
								   "RENAME TO `" + strBase + "`.`" + strSpBerName + "_OK`");
*/
				...
				{
					/**
					 * TODO: intSpielAnz ist hier immer = 0
					 * --> wenn Spielplan beendet, keine Abfrage, ob JOptionen gestartet werden soll
					 */
					PanManager.getInstance().setSpielPlan(panSpielPl);					//-->ist hier falsch, bei Hole Spiele korrekt
//					sout.println(8, "TestOMat.actionPerformed('Übernehmen'): intSpielAnz = " + intSpielAnz);
					panSpielPl.selectNext();
					showSpiel();
					butReset.setText("Spiel laden");
				}
				...
				labNameH.setText("");
				labNameG.setText("");
				labWappH.setIcon(null);
				labWappG.setIcon(null);
			}
		}
	}
