...
public class PanSchuetzen extends JPanel
{
	...
	public PanSchuetzen(int intDebugLvl, String strTurnierName)
	{
		setLayout(null);
		...
		for(int i=0; i<3; i++) //i=3 für die besten drei Schützen
		{
			...
			labSchuetzen[i][0]	= new JLabel((i+1)+".".toString(), JLabel.CENTER);
			...
			labSchuetzen[i][1]	= new JLabel("", JLabel.CENTER);
		...
		for(int i=0; i<12; i++)
		{
			for(int j=0; j<4; j++)
			{
				labSchuetzen[i+3][j] 	= new JLabel("", JLabel.CENTER);
	...
	public void refresh()
	{
		...
		{
			System.err.println("PanSchuetzen.refresh(): " + ex);
		}
		catch(NullPointerException npex)
		{
			System.err.println("PanSchuetzen.refresh(): " + npex);
			length = 0;
		}
...
//				System.out.println(myResultSet.getString("Name"));
		...
		for(int i=3; i<length; i++)
		{
			...
			{
				System.err.println("PanSchuetzen.refresh(): 2 " + ex);
			}
		}
	}
}
