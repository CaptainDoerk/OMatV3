-- MySQL Administrator dump 1.4
--
-- ------------------------------------------------------
-- Server version	5.1.42-community


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8 */;

/*!40014 SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0 */;
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0 */;
/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;


--
-- Create schema kc
--

CREATE DATABASE IF NOT EXISTS kc;
USE kc;

--
-- Temporary table structure for view `kc2011_v_spl`
--
DROP TABLE IF EXISTS `kc2011_v_spl`;
DROP VIEW IF EXISTS `kc2011_v_spl`;
CREATE TABLE `kc2011_v_spl` (
  `TeamA` varchar(30),
  `TeamB` varchar(30),
  `SpielID` int(10) unsigned,
  `Team1` varchar(3),
  `Team2` varchar(3),
  `Tore1` int(10) unsigned,
  `Tore2` int(10) unsigned,
  `X` int(10) unsigned,
  `Gruppe` varchar(3)
);

--
-- Temporary table structure for view `kc2011_v_tbl`
--
DROP TABLE IF EXISTS `kc2011_v_tbl`;
DROP VIEW IF EXISTS `kc2011_v_tbl`;
CREATE TABLE `kc2011_v_tbl` (
  `TeamID` varchar(3),
  `TeamName` varchar(30),
  `Gruppe` varchar(3),
  `Spiele` int(3) unsigned,
  `S` int(3) unsigned,
  `U` int(3) unsigned,
  `N` int(3) unsigned,
  `ToreP` int(4) unsigned,
  `ToreM` int(4) unsigned,
  `Punkte` int(3) unsigned,
  `Status` int(3) unsigned
);

--
-- Temporary table structure for view `o12_v_spl`
--
DROP TABLE IF EXISTS `o12_v_spl`;
DROP VIEW IF EXISTS `o12_v_spl`;
CREATE TABLE `o12_v_spl` (
  `TeamA` varchar(30),
  `TeamB` varchar(30),
  `SpielID` int(10) unsigned,
  `Team1` varchar(3),
  `Team2` varchar(3),
  `Tore1` int(10) unsigned,
  `Tore2` int(10) unsigned,
  `X` int(10) unsigned,
  `Gruppe` varchar(3)
);

--
-- Temporary table structure for view `o12_v_tbl`
--
DROP TABLE IF EXISTS `o12_v_tbl`;
DROP VIEW IF EXISTS `o12_v_tbl`;
CREATE TABLE `o12_v_tbl` (
  `TeamID` varchar(3),
  `TeamName` varchar(30),
  `Gruppe` varchar(3),
  `Spiele` int(3) unsigned,
  `S` int(3) unsigned,
  `U` int(3) unsigned,
  `N` int(3) unsigned,
  `ToreP` int(4) unsigned,
  `ToreM` int(4) unsigned,
  `Punkte` int(3) unsigned,
  `Status` int(3) unsigned
);

--
-- Temporary table structure for view `olympiade2011_v_spl`
--
DROP TABLE IF EXISTS `olympiade2011_v_spl`;
DROP VIEW IF EXISTS `olympiade2011_v_spl`;
CREATE TABLE `olympiade2011_v_spl` (
  `TeamA` varchar(30),
  `TeamB` varchar(30),
  `SpielID` int(10) unsigned,
  `Team1` varchar(3),
  `Team2` varchar(3),
  `Tore1` int(10) unsigned,
  `Tore2` int(10) unsigned,
  `X` int(10) unsigned,
  `Gruppe` varchar(3)
);

--
-- Temporary table structure for view `olympiade2011_v_tbl`
--
DROP TABLE IF EXISTS `olympiade2011_v_tbl`;
DROP VIEW IF EXISTS `olympiade2011_v_tbl`;
CREATE TABLE `olympiade2011_v_tbl` (
  `TeamID` varchar(3),
  `TeamName` varchar(30),
  `Team1` varchar(45),
  `Gruppe` varchar(3),
  `Spiele` int(3) unsigned,
  `S` int(3) unsigned,
  `U` int(3) unsigned,
  `N` int(3) unsigned,
  `ToreP` int(4) unsigned,
  `ToreM` int(4) unsigned,
  `Punkte` int(3) unsigned,
  `Status` int(3) unsigned
);

--
-- Temporary table structure for view `olympiade2012_v_spl`
--
DROP TABLE IF EXISTS `olympiade2012_v_spl`;
DROP VIEW IF EXISTS `olympiade2012_v_spl`;
CREATE TABLE `olympiade2012_v_spl` (
  `TeamA` varchar(30),
  `TeamB` varchar(30),
  `SpielID` int(10) unsigned,
  `Team1` varchar(3),
  `Team2` varchar(3),
  `Tore1` int(10) unsigned,
  `Tore2` int(10) unsigned,
  `X` int(10) unsigned,
  `Gruppe` varchar(3)
);

--
-- Temporary table structure for view `olympiade2012_v_tbl`
--
DROP TABLE IF EXISTS `olympiade2012_v_tbl`;
DROP VIEW IF EXISTS `olympiade2012_v_tbl`;
CREATE TABLE `olympiade2012_v_tbl` (
  `TeamID` varchar(3),
  `TeamName` varchar(30),
  `Gruppe` varchar(3),
  `Spiele` int(3) unsigned,
  `S` int(3) unsigned,
  `U` int(3) unsigned,
  `N` int(3) unsigned,
  `ToreP` int(4) unsigned,
  `ToreM` int(4) unsigned,
  `Punkte` int(3) unsigned,
  `Status` int(3) unsigned
);

--
-- Temporary table structure for view `rsc11_v_spl`
--
DROP TABLE IF EXISTS `rsc11_v_spl`;
DROP VIEW IF EXISTS `rsc11_v_spl`;
CREATE TABLE `rsc11_v_spl` (
  `TeamA` varchar(30),
  `TeamB` varchar(30),
  `SpielID` int(10) unsigned,
  `Team1` varchar(3),
  `Team2` varchar(3),
  `Tore1` int(10) unsigned,
  `Tore2` int(10) unsigned,
  `X` int(10) unsigned,
  `Gruppe` varchar(3)
);

--
-- Temporary table structure for view `rsc11_v_tbl`
--
DROP TABLE IF EXISTS `rsc11_v_tbl`;
DROP VIEW IF EXISTS `rsc11_v_tbl`;
CREATE TABLE `rsc11_v_tbl` (
  `TeamID` varchar(3),
  `TeamName` varchar(30),
  `Team1` varchar(45),
  `Gruppe` varchar(3),
  `Spiele` int(3) unsigned,
  `S` int(3) unsigned,
  `U` int(3) unsigned,
  `N` int(3) unsigned,
  `ToreP` int(4) unsigned,
  `ToreM` int(4) unsigned,
  `Punkte` int(3) unsigned,
  `Status` int(3) unsigned
);

--
-- Temporary table structure for view `rsc12_v_spl`
--
DROP TABLE IF EXISTS `rsc12_v_spl`;
DROP VIEW IF EXISTS `rsc12_v_spl`;
CREATE TABLE `rsc12_v_spl` (
  `TeamA` varchar(30),
  `TeamB` varchar(30),
  `SpielID` int(10) unsigned,
  `Team1` varchar(3),
  `Team2` varchar(3),
  `Tore1` int(10) unsigned,
  `Tore2` int(10) unsigned,
  `X` int(10) unsigned,
  `Gruppe` varchar(3)
);

--
-- Temporary table structure for view `rsc12_v_tbl`
--
DROP TABLE IF EXISTS `rsc12_v_tbl`;
DROP VIEW IF EXISTS `rsc12_v_tbl`;
CREATE TABLE `rsc12_v_tbl` (
  `TeamID` varchar(3),
  `TeamName` varchar(30),
  `Gruppe` varchar(3),
  `Spiele` int(3) unsigned,
  `S` int(3) unsigned,
  `U` int(3) unsigned,
  `N` int(3) unsigned,
  `ToreP` int(4) unsigned,
  `ToreM` int(4) unsigned,
  `Punkte` int(3) unsigned,
  `Status` int(3) unsigned
);

--
-- Temporary table structure for view `rsc_2012_v_spl`
--
DROP TABLE IF EXISTS `rsc_2012_v_spl`;
DROP VIEW IF EXISTS `rsc_2012_v_spl`;
CREATE TABLE `rsc_2012_v_spl` (
  `TeamA` varchar(30),
  `TeamB` varchar(30),
  `SpielID` int(10) unsigned,
  `Team1` varchar(3),
  `Team2` varchar(3),
  `Tore1` int(10) unsigned,
  `Tore2` int(10) unsigned,
  `X` int(10) unsigned,
  `Gruppe` varchar(3)
);

--
-- Temporary table structure for view `rsc_2012_v_tbl`
--
DROP TABLE IF EXISTS `rsc_2012_v_tbl`;
DROP VIEW IF EXISTS `rsc_2012_v_tbl`;
CREATE TABLE `rsc_2012_v_tbl` (
  `TeamID` varchar(3),
  `TeamName` varchar(30),
  `Gruppe` varchar(3),
  `Spiele` int(3) unsigned,
  `S` int(3) unsigned,
  `U` int(3) unsigned,
  `N` int(3) unsigned,
  `ToreP` int(4) unsigned,
  `ToreM` int(4) unsigned,
  `Punkte` int(3) unsigned,
  `Status` int(3) unsigned
);

--
-- Temporary table structure for view `test_v_spl`
--
DROP TABLE IF EXISTS `test_v_spl`;
DROP VIEW IF EXISTS `test_v_spl`;
CREATE TABLE `test_v_spl` (
  `TeamA` varchar(30),
  `TeamB` varchar(30),
  `SpielID` int(10) unsigned,
  `Team1` varchar(3),
  `Team2` varchar(3),
  `Tore1` int(10) unsigned,
  `Tore2` int(10) unsigned,
  `X` int(10) unsigned,
  `Gruppe` varchar(3)
);

--
-- Definition of table `_tpl_spielbericht`
--

DROP TABLE IF EXISTS `_tpl_spielbericht`;
CREATE TABLE `_tpl_spielbericht` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `_tpl_spielbericht`
--

/*!40000 ALTER TABLE `_tpl_spielbericht` DISABLE KEYS */;
/*!40000 ALTER TABLE `_tpl_spielbericht` ENABLE KEYS */;


--
-- Definition of table `_tpl_spielplan`
--

DROP TABLE IF EXISTS `_tpl_spielplan`;
CREATE TABLE `_tpl_spielplan` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Tore1` int(10) unsigned DEFAULT NULL,
  `Tore2` int(10) unsigned DEFAULT NULL,
  `X` int(10) unsigned NOT NULL DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL,
  `Team1Name` varchar(30) DEFAULT NULL,
  `Team2Name` varchar(30) DEFAULT NULL,
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 PACK_KEYS=1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `_tpl_spielplan`
--

/*!40000 ALTER TABLE `_tpl_spielplan` DISABLE KEYS */;
/*!40000 ALTER TABLE `_tpl_spielplan` ENABLE KEYS */;


--
-- Definition of table `_tpl_spielplan_1x3`
--

DROP TABLE IF EXISTS `_tpl_spielplan_1x3`;
CREATE TABLE `_tpl_spielplan_1x3` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Runde` int(10) unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8 PACK_KEYS=1;

--
-- Dumping data for table `_tpl_spielplan_1x3`
--

/*!40000 ALTER TABLE `_tpl_spielplan_1x3` DISABLE KEYS */;
INSERT INTO `_tpl_spielplan_1x3` (`SpielID`,`Team1`,`Team2`,`Runde`) VALUES 
 (1,'1','2',1),
 (2,'3','1',2),
 (3,'2','3',3);
/*!40000 ALTER TABLE `_tpl_spielplan_1x3` ENABLE KEYS */;


--
-- Definition of table `_tpl_spielplan_1x4`
--

DROP TABLE IF EXISTS `_tpl_spielplan_1x4`;
CREATE TABLE `_tpl_spielplan_1x4` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Runde` int(10) unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8 PACK_KEYS=1;

--
-- Dumping data for table `_tpl_spielplan_1x4`
--

/*!40000 ALTER TABLE `_tpl_spielplan_1x4` DISABLE KEYS */;
INSERT INTO `_tpl_spielplan_1x4` (`SpielID`,`Team1`,`Team2`,`Runde`) VALUES 
 (1,'1','2',1),
 (2,'3','4',1),
 (3,'1','3',2),
 (4,'2','4',2),
 (5,'4','1',3),
 (6,'2','3',3);
/*!40000 ALTER TABLE `_tpl_spielplan_1x4` ENABLE KEYS */;


--
-- Definition of table `_tpl_spielplan_1x5`
--

DROP TABLE IF EXISTS `_tpl_spielplan_1x5`;
CREATE TABLE `_tpl_spielplan_1x5` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Runde` int(10) unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8 PACK_KEYS=1;

--
-- Dumping data for table `_tpl_spielplan_1x5`
--

/*!40000 ALTER TABLE `_tpl_spielplan_1x5` DISABLE KEYS */;
INSERT INTO `_tpl_spielplan_1x5` (`SpielID`,`Team1`,`Team2`,`Runde`) VALUES 
 (1,'1','2',1),
 (2,'3','4',1),
 (3,'5','1',2),
 (4,'3','2',2),
 (5,'4','5',3),
 (6,'1','3',3),
 (7,'2','5',4),
 (8,'4','1',4),
 (9,'5','3',5),
 (10,'2','4',5);
/*!40000 ALTER TABLE `_tpl_spielplan_1x5` ENABLE KEYS */;


--
-- Definition of table `_tpl_spielplan_1x6`
--

DROP TABLE IF EXISTS `_tpl_spielplan_1x6`;
CREATE TABLE `_tpl_spielplan_1x6` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Runde` int(3) unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8 PACK_KEYS=1;

--
-- Dumping data for table `_tpl_spielplan_1x6`
--

/*!40000 ALTER TABLE `_tpl_spielplan_1x6` DISABLE KEYS */;
INSERT INTO `_tpl_spielplan_1x6` (`SpielID`,`Team1`,`Team2`,`Runde`) VALUES 
 (1,'1','2',1),
 (2,'2','4',1),
 (3,'5','6',1),
 (4,'1','3',2),
 (5,'2','5',2),
 (6,'6','4',2),
 (7,'3','2',3),
 (8,'5','4',3),
 (9,'1','6',3),
 (10,'3','5',4),
 (11,'2','6',4),
 (12,'4','1',4),
 (13,'6','3',5),
 (14,'1','5',5),
 (15,'4','2',5);
/*!40000 ALTER TABLE `_tpl_spielplan_1x6` ENABLE KEYS */;


--
-- Definition of table `_tpl_spielplan_1x7`
--

DROP TABLE IF EXISTS `_tpl_spielplan_1x7`;
CREATE TABLE `_tpl_spielplan_1x7` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Runde` int(3) unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=22 DEFAULT CHARSET=utf8 PACK_KEYS=1;

--
-- Dumping data for table `_tpl_spielplan_1x7`
--

/*!40000 ALTER TABLE `_tpl_spielplan_1x7` DISABLE KEYS */;
INSERT INTO `_tpl_spielplan_1x7` (`SpielID`,`Team1`,`Team2`,`Runde`) VALUES 
 (1,'1','2',1),
 (2,'3','4',1),
 (3,'5','6',1),
 (4,'7','1',2),
 (5,'2','3',2),
 (6,'4','5',2),
 (7,'6','7',3),
 (8,'1','3',3),
 (9,'5','2',3),
 (10,'7','4',4),
 (11,'6','1',4),
 (12,'3','5',4),
 (13,'2','7',5),
 (14,'4','6',5),
 (15,'1','5',5),
 (16,'7','3',6),
 (17,'6','2',6),
 (18,'4','1',6),
 (19,'5','7',7),
 (20,'3','6',7),
 (21,'2','4',7);
/*!40000 ALTER TABLE `_tpl_spielplan_1x7` ENABLE KEYS */;


--
-- Definition of table `_tpl_spielplan_1x8`
--

DROP TABLE IF EXISTS `_tpl_spielplan_1x8`;
CREATE TABLE `_tpl_spielplan_1x8` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Runde` int(10) unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=29 DEFAULT CHARSET=utf8 PACK_KEYS=1;

--
-- Dumping data for table `_tpl_spielplan_1x8`
--

/*!40000 ALTER TABLE `_tpl_spielplan_1x8` DISABLE KEYS */;
INSERT INTO `_tpl_spielplan_1x8` (`SpielID`,`Team1`,`Team2`,`Runde`) VALUES 
 (1,'1','2',1),
 (2,'3','4',1),
 (3,'5','6',1),
 (4,'7','8',1),
 (5,'1','3',2),
 (6,'2','5',2),
 (7,'4','7',2),
 (8,'6','8',2),
 (9,'5','1',3),
 (10,'3','7',3),
 (11,'8','2',3),
 (12,'4','6',3),
 (13,'1','7',4),
 (14,'5','8',4),
 (15,'6','3',4),
 (16,'2','4',4),
 (17,'8','1',5),
 (18,'7','6',5),
 (19,'4','5',5),
 (20,'3','2',5),
 (21,'1','6',6),
 (22,'8','4',6),
 (23,'2','7',6),
 (24,'5','3',6),
 (25,'4','1',7),
 (26,'6','2',7),
 (27,'3','8',7),
 (28,'7','5',7);
/*!40000 ALTER TABLE `_tpl_spielplan_1x8` ENABLE KEYS */;


--
-- Definition of table `_tpl_spielplan_1x9`
--

DROP TABLE IF EXISTS `_tpl_spielplan_1x9`;
CREATE TABLE `_tpl_spielplan_1x9` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(45) CHARACTER SET latin1 NOT NULL,
  `Runde` int(10) unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=37 DEFAULT CHARSET=utf8 PACK_KEYS=1;

--
-- Dumping data for table `_tpl_spielplan_1x9`
--

/*!40000 ALTER TABLE `_tpl_spielplan_1x9` DISABLE KEYS */;
INSERT INTO `_tpl_spielplan_1x9` (`SpielID`,`Team1`,`Team2`,`Runde`) VALUES 
 (1,'1','2',1),
 (2,'3','4',1),
 (3,'5','6',1),
 (4,'7','8',1),
 (5,'9','1',2),
 (6,'2','3',2),
 (7,'4','5',2),
 (8,'6','7',2),
 (9,'8','9',3),
 (10,'1','3',3),
 (11,'5','2',3),
 (12,'7','4',3),
 (13,'9','6',4),
 (14,'8','1',4),
 (15,'3','5',4),
 (16,'2','7',4),
 (17,'4','9',5),
 (18,'6','8',5),
 (19,'1','5',5),
 (20,'7','3',5),
 (21,'9','2',6),
 (22,'8','4',6),
 (23,'6','1',6),
 (24,'5','7',6),
 (25,'3','9',7),
 (26,'2','8',7),
 (27,'4','6',7),
 (28,'1','7',7),
 (29,'9','5',8),
 (30,'8','3',8),
 (31,'6','2',8),
 (32,'4','1',8),
 (33,'7','9',9),
 (34,'5','8',9),
 (35,'3','6',9),
 (36,'2','4',9);
/*!40000 ALTER TABLE `_tpl_spielplan_1x9` ENABLE KEYS */;


--
-- Definition of table `_tpl_tabelle`
--

DROP TABLE IF EXISTS `_tpl_tabelle`;
CREATE TABLE `_tpl_tabelle` (
  `TeamID` varchar(3) NOT NULL,
  `Spieler1` int(3) DEFAULT NULL,
  `Spieler2` int(3) DEFAULT NULL,
  `Spieler3` int(3) unsigned DEFAULT '0',
  `Spieler4` int(3) unsigned DEFAULT '0',
  `Spiele` int(3) unsigned DEFAULT '0',
  `S` int(3) unsigned DEFAULT '0',
  `U` int(3) unsigned DEFAULT '0',
  `N` int(3) unsigned DEFAULT '0',
  `ToreP` int(4) unsigned DEFAULT '0',
  `ToreM` int(4) unsigned DEFAULT '0',
  `Punkte` int(3) unsigned DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL DEFAULT 'a',
  `Status` int(3) unsigned DEFAULT '1',
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `_tpl_tabelle`
--

/*!40000 ALTER TABLE `_tpl_tabelle` DISABLE KEYS */;
/*!40000 ALTER TABLE `_tpl_tabelle` ENABLE KEYS */;


--
-- Definition of table `_tpl_teams`
--

DROP TABLE IF EXISTS `_tpl_teams`;
CREATE TABLE `_tpl_teams` (
  `TeamID` varchar(3) NOT NULL,
  `TeamName` varchar(30) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `_tpl_teams`
--

/*!40000 ALTER TABLE `_tpl_teams` DISABLE KEYS */;
/*!40000 ALTER TABLE `_tpl_teams` ENABLE KEYS */;


--
-- Definition of table `_tpl_tore`
--

DROP TABLE IF EXISTS `_tpl_tore`;
CREATE TABLE `_tpl_tore` (
  `Spieler` varchar(30) NOT NULL,
  `Team` varchar(30) NOT NULL,
  `Tore` int(10) unsigned NOT NULL,
  PRIMARY KEY (`Spieler`,`Team`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `_tpl_tore`
--

/*!40000 ALTER TABLE `_tpl_tore` DISABLE KEYS */;
/*!40000 ALTER TABLE `_tpl_tore` ENABLE KEYS */;


--
-- Definition of table `default_spl`
--

DROP TABLE IF EXISTS `default_spl`;
CREATE TABLE `default_spl` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Tore1` int(10) unsigned DEFAULT NULL,
  `Tore2` int(10) unsigned DEFAULT NULL,
  `X` int(10) unsigned NOT NULL DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL,
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8 PACK_KEYS=1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `default_spl`
--

/*!40000 ALTER TABLE `default_spl` DISABLE KEYS */;
INSERT INTO `default_spl` (`SpielID`,`Team1`,`Team2`,`Tore1`,`Tore2`,`X`,`Gruppe`) VALUES 
 (1,'a1','a2',1,1,0,'a'),
 (2,'a3','a4',2,2,0,'a'),
 (3,'a1','a3',3,3,0,'a'),
 (4,'a4','a2',4,4,0,'a'),
 (5,'a1','a4',5,5,0,'a'),
 (6,'a2','a3',6,6,0,'a');
/*!40000 ALTER TABLE `default_spl` ENABLE KEYS */;


--
-- Definition of table `default_tbl`
--

DROP TABLE IF EXISTS `default_tbl`;
CREATE TABLE `default_tbl` (
  `TeamID` int(3) unsigned NOT NULL,
  `Spieler1` int(3) DEFAULT NULL,
  `Spieler2` int(3) DEFAULT NULL,
  `Spieler3` int(3) unsigned DEFAULT '0',
  `Spieler4` int(3) unsigned DEFAULT '0',
  `Spiele` int(3) unsigned DEFAULT '0',
  `S` int(3) unsigned DEFAULT '0',
  `U` int(3) unsigned DEFAULT '0',
  `N` int(3) unsigned DEFAULT '0',
  `ToreP` int(4) unsigned DEFAULT '0',
  `ToreM` int(4) unsigned DEFAULT '0',
  `Punkte` int(3) unsigned DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL DEFAULT 'a',
  `Status` int(3) unsigned DEFAULT '1',
  `Team1` varchar(45) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `default_tbl`
--

/*!40000 ALTER TABLE `default_tbl` DISABLE KEYS */;
INSERT INTO `default_tbl` (`TeamID`,`Spieler1`,`Spieler2`,`Spieler3`,`Spieler4`,`Spiele`,`S`,`U`,`N`,`ToreP`,`ToreM`,`Punkte`,`Gruppe`,`Status`,`Team1`) VALUES 
 (1,NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1,'a1'),
 (2,NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1,'a2'),
 (3,NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1,'a3'),
 (4,NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1,'a4');
/*!40000 ALTER TABLE `default_tbl` ENABLE KEYS */;


--
-- Definition of table `default_tms`
--

DROP TABLE IF EXISTS `default_tms`;
CREATE TABLE `default_tms` (
  `TeamID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `TeamName` varchar(30) NOT NULL,
  `TurnierPos` varchar(3) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `default_tms`
--

/*!40000 ALTER TABLE `default_tms` DISABLE KEYS */;
INSERT INTO `default_tms` (`TeamID`,`TeamName`,`TurnierPos`) VALUES 
 (1,'Deutschland','a1'),
 (2,'England','a2'),
 (3,'Frankreich','a3'),
 (4,'Griechenland','a4'),
 (5,'Holland','a5'),
 (6,'Italien','b1'),
 (7,'Kroatien','b2'),
 (8,'Polen','b3'),
 (9,'Portugal','b4'),
 (10,'Schottland','b5');
/*!40000 ALTER TABLE `default_tms` ENABLE KEYS */;


--
-- Definition of table `default_tor`
--

DROP TABLE IF EXISTS `default_tor`;
CREATE TABLE `default_tor` (
  `Spieler` varchar(30) NOT NULL,
  `Team` varchar(30) NOT NULL,
  `Tore` int(10) unsigned NOT NULL,
  PRIMARY KEY (`Spieler`,`Team`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `default_tor`
--

/*!40000 ALTER TABLE `default_tor` DISABLE KEYS */;
/*!40000 ALTER TABLE `default_tor` ENABLE KEYS */;


--
-- Definition of table `kc2011_01_090657_condom_kickers_vs_mollutz`
--

DROP TABLE IF EXISTS `kc2011_01_090657_condom_kickers_vs_mollutz`;
CREATE TABLE `kc2011_01_090657_condom_kickers_vs_mollutz` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=18 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_01_090657_condom_kickers_vs_mollutz`
--

/*!40000 ALTER TABLE `kc2011_01_090657_condom_kickers_vs_mollutz` DISABLE KEYS */;
INSERT INTO `kc2011_01_090657_condom_kickers_vs_mollutz` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:20',1,0,'Condom Kickers','Shorty',NULL),
 (2,'02:15',1,1,'Mollutz','Molli',NULL),
 (3,'02:30',2,1,'Condom Kickers','Shorty',NULL),
 (4,'03:12',3,1,'Condom Kickers','Shorty',NULL),
 (5,'03:49',3,2,'Mollutz','Molli',NULL),
 (6,'04:05',4,2,'Condom Kickers','Quicky',NULL),
 (7,'06:09',5,2,'Condom Kickers','Shorty',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'06:27',5,3,'Mollutz','bewave',NULL),
 (10,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (11,'06:52',5,4,'Mollutz','bewave',NULL),
 (12,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (13,'07:42',6,4,'Condom Kickers','Shorty',NULL),
 (14,'08:34',7,4,'Condom Kickers','Quicky',NULL),
 (15,'08:57',7,5,'Mollutz','Molli',NULL),
 (16,'09:19',7,6,'Mollutz','Molli',NULL),
 (17,'09:41',8,6,'Condom Kickers','Shorty',NULL);
/*!40000 ALTER TABLE `kc2011_01_090657_condom_kickers_vs_mollutz` ENABLE KEYS */;


--
-- Definition of table `kc2011_01_223807_condom_kickers_vs_mollutz`
--

DROP TABLE IF EXISTS `kc2011_01_223807_condom_kickers_vs_mollutz`;
CREATE TABLE `kc2011_01_223807_condom_kickers_vs_mollutz` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_01_223807_condom_kickers_vs_mollutz`
--

/*!40000 ALTER TABLE `kc2011_01_223807_condom_kickers_vs_mollutz` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_01_223807_condom_kickers_vs_mollutz` ENABLE KEYS */;


--
-- Definition of table `kc2011_02_091822_wolfbräu_vs_porsche`
--

DROP TABLE IF EXISTS `kc2011_02_091822_wolfbräu_vs_porsche`;
CREATE TABLE `kc2011_02_091822_wolfbräu_vs_porsche` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_02_091822_wolfbräu_vs_porsche`
--

/*!40000 ALTER TABLE `kc2011_02_091822_wolfbräu_vs_porsche` DISABLE KEYS */;
INSERT INTO `kc2011_02_091822_wolfbräu_vs_porsche` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:30',0,1,'Porsche','Mad Maz',NULL),
 (2,'01:13',1,1,'Wolfbräu','Tha Man',NULL),
 (3,'02:24',1,2,'Porsche','Luca T.',NULL),
 (4,'02:49',2,2,'Wolfbräu','Tha Man',NULL),
 (5,'03:09',2,3,'Porsche','Luca T.',NULL),
 (6,'06:49',4,3,'Wolfbräu','Tha Man',NULL),
 (7,'08:04',4,4,'Porsche','Mad Maz',NULL),
 (8,'09:08',5,4,'Wolfbräu','Tha Man',NULL),
 (9,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (10,'09:17',5,5,'Porsche','Luca T.',NULL);
/*!40000 ALTER TABLE `kc2011_02_091822_wolfbräu_vs_porsche` ENABLE KEYS */;


--
-- Definition of table `kc2011_03_092925_ensinger_vs_sparda_orange`
--

DROP TABLE IF EXISTS `kc2011_03_092925_ensinger_vs_sparda_orange`;
CREATE TABLE `kc2011_03_092925_ensinger_vs_sparda_orange` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_03_092925_ensinger_vs_sparda_orange`
--

/*!40000 ALTER TABLE `kc2011_03_092925_ensinger_vs_sparda_orange` DISABLE KEYS */;
INSERT INTO `kc2011_03_092925_ensinger_vs_sparda_orange` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'02:26',0,1,'Sparda Orange','Schlampe',NULL),
 (2,'03:22',0,2,'Sparda Orange','Fips',NULL),
 (3,'05:43',1,2,'Ensinger','Jens L.',NULL),
 (4,'06:00',2,2,'Ensinger','Fraggy',NULL),
 (5,'06:14',3,2,'Ensinger','Jens L.',NULL),
 (6,'06:26',4,2,'Ensinger','Jens L.',NULL),
 (7,'07:10',4,3,'Sparda Orange','Fips',NULL),
 (8,'07:34',5,3,'Ensinger','Fraggy',NULL),
 (9,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (10,'08:53',5,4,'Sparda Orange','Fips',NULL),
 (11,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (12,'09:46',5,5,'Sparda Orange','Fips',NULL),
 (13,'10:01',5,6,'Sparda Orange','Schlampe',NULL);
/*!40000 ALTER TABLE `kc2011_03_092925_ensinger_vs_sparda_orange` ENABLE KEYS */;


--
-- Definition of table `kc2011_04_094046_waldmeister_vs_bergmacht`
--

DROP TABLE IF EXISTS `kc2011_04_094046_waldmeister_vs_bergmacht`;
CREATE TABLE `kc2011_04_094046_waldmeister_vs_bergmacht` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_04_094046_waldmeister_vs_bergmacht`
--

/*!40000 ALTER TABLE `kc2011_04_094046_waldmeister_vs_bergmacht` DISABLE KEYS */;
INSERT INTO `kc2011_04_094046_waldmeister_vs_bergmacht` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:37',0,1,'Bergmacht','Dave vom Berg',NULL),
 (2,'00:51',0,2,'Bergmacht','Flo vom Berg',NULL),
 (3,'00:54',1,2,'Waldmeister','Mario vom Wald',NULL),
 (4,'00:59',2,2,'Waldmeister','Mario vom Wald',NULL),
 (5,'01:46',3,2,'Waldmeister','Ben vom Wald',NULL),
 (6,'02:55',4,3,'Waldmeister','Mario vom Wald',NULL),
 (7,'03:21',5,3,'Waldmeister','Ben vom Wald',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'04:16',5,4,'Bergmacht','Dave vom Berg',NULL),
 (10,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (11,'04:55',5,5,'Bergmacht','Dave vom Berg',NULL),
 (12,'05:20',6,5,'Waldmeister','Ben vom Wald',NULL),
 (13,'06:12',7,5,'Waldmeister','Mario vom Wald',NULL),
 (14,'06:51',8,5,'Waldmeister','Mario vom Wald',NULL),
 (15,'06:54',8,6,'Bergmacht','Flo vom Berg',NULL),
 (16,'08:28',9,6,'Waldmeister','Ben vom Wald',NULL),
 (17,'08:50',9,7,'Bergmacht','Dave vom Berg',NULL),
 (18,'09:33',9,8,'Bergmacht','Flo vom Berg',NULL);
/*!40000 ALTER TABLE `kc2011_04_094046_waldmeister_vs_bergmacht` ENABLE KEYS */;


--
-- Definition of table `kc2011_04_095330_waldmeister_vs_bergmacht`
--

DROP TABLE IF EXISTS `kc2011_04_095330_waldmeister_vs_bergmacht`;
CREATE TABLE `kc2011_04_095330_waldmeister_vs_bergmacht` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_04_095330_waldmeister_vs_bergmacht`
--

/*!40000 ALTER TABLE `kc2011_04_095330_waldmeister_vs_bergmacht` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_04_095330_waldmeister_vs_bergmacht` ENABLE KEYS */;


--
-- Definition of table `kc2011_05_095315_sauffpark_vs_olympiakos`
--

DROP TABLE IF EXISTS `kc2011_05_095315_sauffpark_vs_olympiakos`;
CREATE TABLE `kc2011_05_095315_sauffpark_vs_olympiakos` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_05_095315_sauffpark_vs_olympiakos`
--

/*!40000 ALTER TABLE `kc2011_05_095315_sauffpark_vs_olympiakos` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_05_095315_sauffpark_vs_olympiakos` ENABLE KEYS */;


--
-- Definition of table `kc2011_05_095805_sauffpark_vs_olympiakos`
--

DROP TABLE IF EXISTS `kc2011_05_095805_sauffpark_vs_olympiakos`;
CREATE TABLE `kc2011_05_095805_sauffpark_vs_olympiakos` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_05_095805_sauffpark_vs_olympiakos`
--

/*!40000 ALTER TABLE `kc2011_05_095805_sauffpark_vs_olympiakos` DISABLE KEYS */;
INSERT INTO `kc2011_05_095805_sauffpark_vs_olympiakos` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'01:23',1,1,'Olympiakos','Theo',NULL),
 (2,'01:32',1,2,'Olympiakos','Andreas K.',NULL),
 (3,'02:30',1,3,'Olympiakos','Theo',NULL),
 (4,'02:59',1,4,'Olympiakos','Andreas K.',NULL),
 (5,'03:33',2,4,'Sauffpark','Mahony',NULL),
 (6,'04:58',2,5,'Olympiakos','Andreas K.',NULL),
 (7,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (8,'05:41',2,6,'Olympiakos','Theo',NULL),
 (9,'05:55',2,7,'Olympiakos','Andreas K.',NULL),
 (10,'06:43',2,8,'Olympiakos','Theo',NULL),
 (11,'07:03',3,8,'Sauffpark','Mahony',NULL),
 (12,'07:19',4,8,'Sauffpark','Mahony',NULL),
 (13,'07:37',4,9,'Olympiakos','Andreas K.',NULL),
 (14,'07:53',4,10,'Olympiakos','Andreas K.',NULL);
/*!40000 ALTER TABLE `kc2011_05_095805_sauffpark_vs_olympiakos` ENABLE KEYS */;


--
-- Definition of table `kc2011_06_100622_deportivo_vs_sion_kölsch`
--

DROP TABLE IF EXISTS `kc2011_06_100622_deportivo_vs_sion_kölsch`;
CREATE TABLE `kc2011_06_100622_deportivo_vs_sion_kölsch` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_06_100622_deportivo_vs_sion_kölsch`
--

/*!40000 ALTER TABLE `kc2011_06_100622_deportivo_vs_sion_kölsch` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_06_100622_deportivo_vs_sion_kölsch` ENABLE KEYS */;


--
-- Definition of table `kc2011_06_100640_deportivo_vs_sion_kölsch`
--

DROP TABLE IF EXISTS `kc2011_06_100640_deportivo_vs_sion_kölsch`;
CREATE TABLE `kc2011_06_100640_deportivo_vs_sion_kölsch` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_06_100640_deportivo_vs_sion_kölsch`
--

/*!40000 ALTER TABLE `kc2011_06_100640_deportivo_vs_sion_kölsch` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_06_100640_deportivo_vs_sion_kölsch` ENABLE KEYS */;


--
-- Definition of table `kc2011_06_100648_deportivo_vs_sion_kölsch`
--

DROP TABLE IF EXISTS `kc2011_06_100648_deportivo_vs_sion_kölsch`;
CREATE TABLE `kc2011_06_100648_deportivo_vs_sion_kölsch` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_06_100648_deportivo_vs_sion_kölsch`
--

/*!40000 ALTER TABLE `kc2011_06_100648_deportivo_vs_sion_kölsch` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_06_100648_deportivo_vs_sion_kölsch` ENABLE KEYS */;


--
-- Definition of table `kc2011_06_101202_deportivo_vs_sion_kölsch`
--

DROP TABLE IF EXISTS `kc2011_06_101202_deportivo_vs_sion_kölsch`;
CREATE TABLE `kc2011_06_101202_deportivo_vs_sion_kölsch` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_06_101202_deportivo_vs_sion_kölsch`
--

/*!40000 ALTER TABLE `kc2011_06_101202_deportivo_vs_sion_kölsch` DISABLE KEYS */;
INSERT INTO `kc2011_06_101202_deportivo_vs_sion_kölsch` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'01:07',1,0,'Deportivo','Inge',NULL),
 (2,'01:22',1,1,'Sion Kölsch','Captain Doerk',NULL),
 (3,'04:06',2,2,'Sion Kölsch','Emre',NULL),
 (4,'04:37',3,2,'Deportivo','Inge',NULL),
 (5,'04:41',3,3,'Sion Kölsch','Captain Doerk',NULL),
 (6,'05:42',3,4,'Sion Kölsch','Emre',NULL),
 (7,'06:04',3,5,'Sion Kölsch','Emre',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'07:16',3,6,'Sion Kölsch','Captain Doerk',NULL),
 (10,'07:21',4,6,'Deportivo','Inge',NULL),
 (11,'08:51',4,7,'Sion Kölsch','Captain Doerk',NULL),
 (12,'09:07',4,8,'Sion Kölsch','Emre',NULL),
 (13,'09:27',4,9,'Sion Kölsch','Emre',NULL);
/*!40000 ALTER TABLE `kc2011_06_101202_deportivo_vs_sion_kölsch` ENABLE KEYS */;


--
-- Definition of table `kc2011_08_103401_hulaboys_vs_gaffel_kölsch`
--

DROP TABLE IF EXISTS `kc2011_08_103401_hulaboys_vs_gaffel_kölsch`;
CREATE TABLE `kc2011_08_103401_hulaboys_vs_gaffel_kölsch` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_08_103401_hulaboys_vs_gaffel_kölsch`
--

/*!40000 ALTER TABLE `kc2011_08_103401_hulaboys_vs_gaffel_kölsch` DISABLE KEYS */;
INSERT INTO `kc2011_08_103401_hulaboys_vs_gaffel_kölsch` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:52',1,0,'Hulaboys','Wengi',NULL),
 (2,'01:39',2,0,'Hulaboys','Schmiddi',NULL),
 (3,'01:59',3,0,'Hulaboys','Schmiddi',NULL),
 (4,'02:21',4,0,'Hulaboys','Wengi',NULL),
 (5,'03:09',4,1,'Gaffel Kölsch','Rio',NULL),
 (6,'03:36',5,1,'Hulaboys','Schmiddi',NULL),
 (7,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (8,'05:06',6,1,'Hulaboys','Schmiddi',NULL),
 (9,'05:31',7,1,'Hulaboys','Wengi',NULL),
 (10,'06:46',7,3,'Gaffel Kölsch','Rio',NULL),
 (11,'06:58',8,3,'Hulaboys','Wengi',NULL),
 (12,'07:13',9,3,'Hulaboys','Schmiddi',NULL),
 (13,'07:44',10,3,'Hulaboys','Wengi',NULL);
/*!40000 ALTER TABLE `kc2011_08_103401_hulaboys_vs_gaffel_kölsch` ENABLE KEYS */;


--
-- Definition of table `kc2011_09_104325_die_wilden_kerle_vs_galatasaray`
--

DROP TABLE IF EXISTS `kc2011_09_104325_die_wilden_kerle_vs_galatasaray`;
CREATE TABLE `kc2011_09_104325_die_wilden_kerle_vs_galatasaray` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_09_104325_die_wilden_kerle_vs_galatasaray`
--

/*!40000 ALTER TABLE `kc2011_09_104325_die_wilden_kerle_vs_galatasaray` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_09_104325_die_wilden_kerle_vs_galatasaray` ENABLE KEYS */;


--
-- Definition of table `kc2011_09_104446_die_wilden_kerle_vs_galatasaray`
--

DROP TABLE IF EXISTS `kc2011_09_104446_die_wilden_kerle_vs_galatasaray`;
CREATE TABLE `kc2011_09_104446_die_wilden_kerle_vs_galatasaray` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_09_104446_die_wilden_kerle_vs_galatasaray`
--

/*!40000 ALTER TABLE `kc2011_09_104446_die_wilden_kerle_vs_galatasaray` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_09_104446_die_wilden_kerle_vs_galatasaray` ENABLE KEYS */;


--
-- Definition of table `kc2011_09_104519_die_wilden_kerle_vs_galatasaray`
--

DROP TABLE IF EXISTS `kc2011_09_104519_die_wilden_kerle_vs_galatasaray`;
CREATE TABLE `kc2011_09_104519_die_wilden_kerle_vs_galatasaray` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_09_104519_die_wilden_kerle_vs_galatasaray`
--

/*!40000 ALTER TABLE `kc2011_09_104519_die_wilden_kerle_vs_galatasaray` DISABLE KEYS */;
INSERT INTO `kc2011_09_104519_die_wilden_kerle_vs_galatasaray` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'01:14',0,1,'Galatasaray','Mirko',NULL),
 (2,'02:40',2,2,'die wilden Kerle','Dolschewida',NULL),
 (3,'02:42',3,2,'die wilden Kerle','Dolschewida',NULL),
 (4,'02:44',4,2,'die wilden Kerle','Dolschewida',NULL),
 (5,'04:08',4,3,'Galatasaray','Mirko',NULL),
 (6,'05:02',4,4,'Galatasaray','Marvin',NULL),
 (7,'05:37',4,5,'Galatasaray','Marvin',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'05:39',5,5,'die wilden Kerle','Dolschewida',NULL),
 (10,'05:56',6,5,'die wilden Kerle','Dolschewida',NULL),
 (11,'08:30',7,5,'die wilden Kerle','Jet-Ski',NULL);
/*!40000 ALTER TABLE `kc2011_09_104519_die_wilden_kerle_vs_galatasaray` ENABLE KEYS */;


--
-- Definition of table `kc2011_10_105755_condom_kickers_vs_wolfbräu`
--

DROP TABLE IF EXISTS `kc2011_10_105755_condom_kickers_vs_wolfbräu`;
CREATE TABLE `kc2011_10_105755_condom_kickers_vs_wolfbräu` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_10_105755_condom_kickers_vs_wolfbräu`
--

/*!40000 ALTER TABLE `kc2011_10_105755_condom_kickers_vs_wolfbräu` DISABLE KEYS */;
INSERT INTO `kc2011_10_105755_condom_kickers_vs_wolfbräu` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'02:11',0,1,'Wolfbräu','Ben vom Berg',NULL),
 (2,'03:23',0,2,'Wolfbräu','Tha Man',NULL),
 (3,'03:51',0,3,'Wolfbräu','Tha Man',NULL),
 (4,'05:42',0,4,'Wolfbräu','Tha Man',NULL),
 (5,'06:41',1,4,'Condom Kickers','Quicky',NULL),
 (6,'07:22',2,4,'Condom Kickers','Shorty',NULL),
 (7,'07:32',2,5,'Wolfbräu','Tha Man',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'08:03',3,5,'Condom Kickers','Quicky',NULL),
 (10,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (11,'08:34',4,5,'Condom Kickers','Quicky',NULL),
 (12,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (13,'09:24',5,5,'Condom Kickers','Quicky',NULL);
/*!40000 ALTER TABLE `kc2011_10_105755_condom_kickers_vs_wolfbräu` ENABLE KEYS */;


--
-- Definition of table `kc2011_11_110834_ensinger_vs_mollutz`
--

DROP TABLE IF EXISTS `kc2011_11_110834_ensinger_vs_mollutz`;
CREATE TABLE `kc2011_11_110834_ensinger_vs_mollutz` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_11_110834_ensinger_vs_mollutz`
--

/*!40000 ALTER TABLE `kc2011_11_110834_ensinger_vs_mollutz` DISABLE KEYS */;
INSERT INTO `kc2011_11_110834_ensinger_vs_mollutz` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:37',0,1,'Mollutz','Molli',NULL),
 (2,'01:37',0,2,'Mollutz','Molli',NULL),
 (3,'02:01',0,3,'Mollutz','Molli',NULL),
 (4,'02:35',0,4,'Mollutz','Molli',NULL),
 (5,'03:38',0,5,'Mollutz','bewave',NULL),
 (6,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (7,'04:08',0,6,'Mollutz','Molli',NULL),
 (8,'05:17',0,7,'Mollutz','Molli',NULL),
 (9,'06:22',1,7,'Ensinger','Fraggy',NULL),
 (10,'06:48',1,8,'Mollutz','Molli',NULL),
 (11,'07:04',2,8,'Ensinger','Fraggy',NULL),
 (12,'07:19',2,9,'Mollutz','Molli',NULL),
 (13,'08:11',2,10,'Mollutz','Molli',NULL);
/*!40000 ALTER TABLE `kc2011_11_110834_ensinger_vs_mollutz` ENABLE KEYS */;


--
-- Definition of table `kc2011_12_111835_sparda_orange_vs_porsche`
--

DROP TABLE IF EXISTS `kc2011_12_111835_sparda_orange_vs_porsche`;
CREATE TABLE `kc2011_12_111835_sparda_orange_vs_porsche` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_12_111835_sparda_orange_vs_porsche`
--

/*!40000 ALTER TABLE `kc2011_12_111835_sparda_orange_vs_porsche` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_12_111835_sparda_orange_vs_porsche` ENABLE KEYS */;


--
-- Definition of table `kc2011_12_111857_sparda_orange_vs_porsche`
--

DROP TABLE IF EXISTS `kc2011_12_111857_sparda_orange_vs_porsche`;
CREATE TABLE `kc2011_12_111857_sparda_orange_vs_porsche` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_12_111857_sparda_orange_vs_porsche`
--

/*!40000 ALTER TABLE `kc2011_12_111857_sparda_orange_vs_porsche` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_12_111857_sparda_orange_vs_porsche` ENABLE KEYS */;


--
-- Definition of table `kc2011_12_111900_sparda_orange_vs_porsche`
--

DROP TABLE IF EXISTS `kc2011_12_111900_sparda_orange_vs_porsche`;
CREATE TABLE `kc2011_12_111900_sparda_orange_vs_porsche` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_12_111900_sparda_orange_vs_porsche`
--

/*!40000 ALTER TABLE `kc2011_12_111900_sparda_orange_vs_porsche` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_12_111900_sparda_orange_vs_porsche` ENABLE KEYS */;


--
-- Definition of table `kc2011_12_111902_sparda_orange_vs_porsche`
--

DROP TABLE IF EXISTS `kc2011_12_111902_sparda_orange_vs_porsche`;
CREATE TABLE `kc2011_12_111902_sparda_orange_vs_porsche` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_12_111902_sparda_orange_vs_porsche`
--

/*!40000 ALTER TABLE `kc2011_12_111902_sparda_orange_vs_porsche` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_12_111902_sparda_orange_vs_porsche` ENABLE KEYS */;


--
-- Definition of table `kc2011_12_111903_sparda_orange_vs_porsche`
--

DROP TABLE IF EXISTS `kc2011_12_111903_sparda_orange_vs_porsche`;
CREATE TABLE `kc2011_12_111903_sparda_orange_vs_porsche` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_12_111903_sparda_orange_vs_porsche`
--

/*!40000 ALTER TABLE `kc2011_12_111903_sparda_orange_vs_porsche` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_12_111903_sparda_orange_vs_porsche` ENABLE KEYS */;


--
-- Definition of table `kc2011_12_111919_sparda_orange_vs_porsche`
--

DROP TABLE IF EXISTS `kc2011_12_111919_sparda_orange_vs_porsche`;
CREATE TABLE `kc2011_12_111919_sparda_orange_vs_porsche` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_12_111919_sparda_orange_vs_porsche`
--

/*!40000 ALTER TABLE `kc2011_12_111919_sparda_orange_vs_porsche` DISABLE KEYS */;
INSERT INTO `kc2011_12_111919_sparda_orange_vs_porsche` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:44',1,0,'Sparda Orange','Fips',NULL),
 (2,'01:33',2,0,'Sparda Orange','Schlampe',NULL),
 (3,'02:11',3,0,'Sparda Orange','Fips',NULL),
 (4,'04:20',3,1,'Porsche','Mad Maz',NULL),
 (5,'05:03',4,1,'Sparda Orange','Fips',NULL),
 (6,'06:53',4,2,'Porsche','Luca T.',NULL),
 (7,'07:19',4,3,'Porsche','Luca T.',NULL),
 (8,'08:09',4,4,'Porsche','Mad Maz',NULL),
 (9,'08:43',6,4,'Sparda Orange','Fips',NULL),
 (10,'09:56',7,4,'Sparda Orange','Schlampe',NULL);
/*!40000 ALTER TABLE `kc2011_12_111919_sparda_orange_vs_porsche` ENABLE KEYS */;


--
-- Definition of table `kc2011_13_112956_waldmeister_vs_sauffpark`
--

DROP TABLE IF EXISTS `kc2011_13_112956_waldmeister_vs_sauffpark`;
CREATE TABLE `kc2011_13_112956_waldmeister_vs_sauffpark` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=13 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_13_112956_waldmeister_vs_sauffpark`
--

/*!40000 ALTER TABLE `kc2011_13_112956_waldmeister_vs_sauffpark` DISABLE KEYS */;
INSERT INTO `kc2011_13_112956_waldmeister_vs_sauffpark` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'02:15',0,1,'Sauffpark','Mahony',NULL),
 (2,'02:19',1,1,'Waldmeister','Mario vom Wald',NULL),
 (3,'02:43',2,1,'Waldmeister','Mario vom Wald',NULL),
 (4,'03:07',3,1,'Waldmeister','Mario vom Wald',NULL),
 (5,'03:16',4,1,'Waldmeister','Ben vom Wald',NULL),
 (6,'04:10',5,1,'Waldmeister','Mario vom Wald',NULL),
 (7,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (8,'05:30',6,1,'Waldmeister','Ben vom Wald',NULL),
 (9,'06:29',7,2,'Waldmeister','Mario vom Wald',NULL),
 (10,'07:00',8,2,'Waldmeister','Mario vom Wald',NULL),
 (11,'07:07',8,3,'Sauffpark','Shooter',NULL),
 (12,'08:29',10,3,'Waldmeister','Ben vom Wald',NULL);
/*!40000 ALTER TABLE `kc2011_13_112956_waldmeister_vs_sauffpark` ENABLE KEYS */;


--
-- Definition of table `kc2011_14_114918_deportivo_vs_bergmacht`
--

DROP TABLE IF EXISTS `kc2011_14_114918_deportivo_vs_bergmacht`;
CREATE TABLE `kc2011_14_114918_deportivo_vs_bergmacht` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_14_114918_deportivo_vs_bergmacht`
--

/*!40000 ALTER TABLE `kc2011_14_114918_deportivo_vs_bergmacht` DISABLE KEYS */;
INSERT INTO `kc2011_14_114918_deportivo_vs_bergmacht` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:26',0,1,'Bergmacht','Dave vom Berg',NULL),
 (2,'00:46',0,2,'Bergmacht','Dave vom Berg',NULL),
 (3,'01:05',1,2,'Deportivo','Inge',NULL),
 (4,'01:41',2,2,'Deportivo','Inge',NULL),
 (5,'02:54',3,2,'Deportivo','Heif',NULL),
 (6,'03:23',4,2,'Deportivo','Heif',NULL),
 (7,'03:53',4,3,'Bergmacht','Dave vom Berg',NULL),
 (8,'04:24',5,3,'Deportivo','Heif',NULL),
 (9,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (10,'04:55',6,3,'Deportivo','Heif',NULL),
 (11,'05:31',6,4,'Bergmacht','Dave vom Berg',NULL),
 (12,'06:50',6,5,'Bergmacht','Dave vom Berg',NULL),
 (13,'08:02',6,6,'Bergmacht','Flo vom Berg',NULL),
 (14,'09:30',6,7,'Bergmacht','Flo vom Berg',NULL),
 (15,'09:40',6,8,'Bergmacht','Flo vom Berg',NULL);
/*!40000 ALTER TABLE `kc2011_14_114918_deportivo_vs_bergmacht` ENABLE KEYS */;


--
-- Definition of table `kc2011_15_120233_sion_kölsch_vs_olympiakos`
--

DROP TABLE IF EXISTS `kc2011_15_120233_sion_kölsch_vs_olympiakos`;
CREATE TABLE `kc2011_15_120233_sion_kölsch_vs_olympiakos` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_15_120233_sion_kölsch_vs_olympiakos`
--

/*!40000 ALTER TABLE `kc2011_15_120233_sion_kölsch_vs_olympiakos` DISABLE KEYS */;
INSERT INTO `kc2011_15_120233_sion_kölsch_vs_olympiakos` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'07:13',1,0,'Sion Kölsch','Captain Doerk',NULL),
 (2,'07:22',1,1,'Olympiakos','Andreas K.',NULL),
 (3,'08:37',1,2,'Olympiakos','Andreas K.',NULL),
 (4,'09:02',2,2,'Sion Kölsch','Emre',NULL);
/*!40000 ALTER TABLE `kc2011_15_120233_sion_kölsch_vs_olympiakos` ENABLE KEYS */;


--
-- Definition of table `kc2011_15_121620_sion_kölsch_vs_olympiakos`
--

DROP TABLE IF EXISTS `kc2011_15_121620_sion_kölsch_vs_olympiakos`;
CREATE TABLE `kc2011_15_121620_sion_kölsch_vs_olympiakos` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_15_121620_sion_kölsch_vs_olympiakos`
--

/*!40000 ALTER TABLE `kc2011_15_121620_sion_kölsch_vs_olympiakos` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_15_121620_sion_kölsch_vs_olympiakos` ENABLE KEYS */;


--
-- Definition of table `kc2011_17_122808_die_wilden_kerle_vs_tesa_power`
--

DROP TABLE IF EXISTS `kc2011_17_122808_die_wilden_kerle_vs_tesa_power`;
CREATE TABLE `kc2011_17_122808_die_wilden_kerle_vs_tesa_power` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_17_122808_die_wilden_kerle_vs_tesa_power`
--

/*!40000 ALTER TABLE `kc2011_17_122808_die_wilden_kerle_vs_tesa_power` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_17_122808_die_wilden_kerle_vs_tesa_power` ENABLE KEYS */;


--
-- Definition of table `kc2011_17_122950_die_wilden_kerle_vs_tesa_power`
--

DROP TABLE IF EXISTS `kc2011_17_122950_die_wilden_kerle_vs_tesa_power`;
CREATE TABLE `kc2011_17_122950_die_wilden_kerle_vs_tesa_power` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_17_122950_die_wilden_kerle_vs_tesa_power`
--

/*!40000 ALTER TABLE `kc2011_17_122950_die_wilden_kerle_vs_tesa_power` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_17_122950_die_wilden_kerle_vs_tesa_power` ENABLE KEYS */;


--
-- Definition of table `kc2011_17_123006_die_wilden_kerle_vs_tesa_power`
--

DROP TABLE IF EXISTS `kc2011_17_123006_die_wilden_kerle_vs_tesa_power`;
CREATE TABLE `kc2011_17_123006_die_wilden_kerle_vs_tesa_power` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_17_123006_die_wilden_kerle_vs_tesa_power`
--

/*!40000 ALTER TABLE `kc2011_17_123006_die_wilden_kerle_vs_tesa_power` DISABLE KEYS */;
INSERT INTO `kc2011_17_123006_die_wilden_kerle_vs_tesa_power` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'02:12',1,0,'die wilden Kerle','Dolschewida',NULL),
 (2,'05:21',5,1,'die wilden Kerle','Dolschewida',NULL),
 (3,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (4,'05:44',6,1,'die wilden Kerle','Jet-Ski',NULL),
 (5,'06:04',7,1,'die wilden Kerle','Jet-Ski',NULL),
 (6,'06:24',8,1,'die wilden Kerle','Dolschewida',NULL),
 (7,'08:02',8,2,'Tesa Power','Timo P.',NULL),
 (8,'08:10',9,2,'die wilden Kerle','Jet-Ski',NULL),
 (9,'08:45',10,2,'die wilden Kerle','Jet-Ski',NULL);
/*!40000 ALTER TABLE `kc2011_17_123006_die_wilden_kerle_vs_tesa_power` ENABLE KEYS */;


--
-- Definition of table `kc2011_18_123940_galatasaray_vs_gaffel_kölsch`
--

DROP TABLE IF EXISTS `kc2011_18_123940_galatasaray_vs_gaffel_kölsch`;
CREATE TABLE `kc2011_18_123940_galatasaray_vs_gaffel_kölsch` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_18_123940_galatasaray_vs_gaffel_kölsch`
--

/*!40000 ALTER TABLE `kc2011_18_123940_galatasaray_vs_gaffel_kölsch` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_18_123940_galatasaray_vs_gaffel_kölsch` ENABLE KEYS */;


--
-- Definition of table `kc2011_18_124047_galatasaray_vs_gaffel_kölsch`
--

DROP TABLE IF EXISTS `kc2011_18_124047_galatasaray_vs_gaffel_kölsch`;
CREATE TABLE `kc2011_18_124047_galatasaray_vs_gaffel_kölsch` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_18_124047_galatasaray_vs_gaffel_kölsch`
--

/*!40000 ALTER TABLE `kc2011_18_124047_galatasaray_vs_gaffel_kölsch` DISABLE KEYS */;
INSERT INTO `kc2011_18_124047_galatasaray_vs_gaffel_kölsch` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'01:17',1,0,'Galatasaray','Mirko',NULL),
 (2,'01:36',1,1,'Gaffel Kölsch','Rio',NULL),
 (3,'02:35',1,2,'Gaffel Kölsch','Der Jupp',NULL),
 (4,'03:24',3,2,'Galatasaray','Mirko',NULL),
 (5,'04:09',4,2,'Galatasaray','Mirko',NULL),
 (6,'04:17',4,3,'Gaffel Kölsch','Der Jupp',NULL),
 (7,'04:20',4,4,'Gaffel Kölsch','Der Jupp',NULL),
 (8,'05:21',6,4,'Galatasaray','Mirko',NULL),
 (9,'06:51',7,4,'Galatasaray','Mirko',NULL),
 (10,'07:51',8,4,'Galatasaray','Mirko',NULL),
 (11,'08:03',8,5,'Gaffel Kölsch','Rio',NULL),
 (12,'08:41',9,5,'Galatasaray','Mirko',NULL),
 (13,'09:04',9,6,'Gaffel Kölsch','Rio',NULL),
 (14,'09:45',10,6,'Galatasaray','Mirko',NULL);
/*!40000 ALTER TABLE `kc2011_18_124047_galatasaray_vs_gaffel_kölsch` ENABLE KEYS */;


--
-- Definition of table `kc2011_19_125125_condom_kickers_vs_ensinger`
--

DROP TABLE IF EXISTS `kc2011_19_125125_condom_kickers_vs_ensinger`;
CREATE TABLE `kc2011_19_125125_condom_kickers_vs_ensinger` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_19_125125_condom_kickers_vs_ensinger`
--

/*!40000 ALTER TABLE `kc2011_19_125125_condom_kickers_vs_ensinger` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_19_125125_condom_kickers_vs_ensinger` ENABLE KEYS */;


--
-- Definition of table `kc2011_19_125225_condom_kickers_vs_ensinger`
--

DROP TABLE IF EXISTS `kc2011_19_125225_condom_kickers_vs_ensinger`;
CREATE TABLE `kc2011_19_125225_condom_kickers_vs_ensinger` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_19_125225_condom_kickers_vs_ensinger`
--

/*!40000 ALTER TABLE `kc2011_19_125225_condom_kickers_vs_ensinger` DISABLE KEYS */;
INSERT INTO `kc2011_19_125225_condom_kickers_vs_ensinger` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:20',1,0,'Condom Kickers','Shorty',NULL),
 (2,'02:14',2,0,'Condom Kickers','Quicky',NULL),
 (3,'03:35',3,0,'Condom Kickers','Quicky',NULL),
 (4,'04:08',4,0,'Condom Kickers','Shorty',NULL),
 (5,'04:48',4,1,'Ensinger','Fraggy',NULL),
 (6,'06:52',5,1,'Condom Kickers','Shorty',NULL),
 (7,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (8,'07:38',6,1,'Condom Kickers','Quicky',NULL),
 (9,'08:35',6,2,'Ensinger','Jens L.',NULL),
 (10,'08:40',7,2,'Condom Kickers','Quicky',NULL),
 (11,'08:47',8,2,'Condom Kickers','Quicky',NULL),
 (12,'09:34',9,2,'Condom Kickers','Quicky',NULL),
 (13,'09:57',10,2,'Condom Kickers','Quicky',NULL);
/*!40000 ALTER TABLE `kc2011_19_125225_condom_kickers_vs_ensinger` ENABLE KEYS */;


--
-- Definition of table `kc2011_20_130408_sparda_orange_vs_wolfbräu`
--

DROP TABLE IF EXISTS `kc2011_20_130408_sparda_orange_vs_wolfbräu`;
CREATE TABLE `kc2011_20_130408_sparda_orange_vs_wolfbräu` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_20_130408_sparda_orange_vs_wolfbräu`
--

/*!40000 ALTER TABLE `kc2011_20_130408_sparda_orange_vs_wolfbräu` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_20_130408_sparda_orange_vs_wolfbräu` ENABLE KEYS */;


--
-- Definition of table `kc2011_20_130506_sparda_orange_vs_wolfbräu`
--

DROP TABLE IF EXISTS `kc2011_20_130506_sparda_orange_vs_wolfbräu`;
CREATE TABLE `kc2011_20_130506_sparda_orange_vs_wolfbräu` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_20_130506_sparda_orange_vs_wolfbräu`
--

/*!40000 ALTER TABLE `kc2011_20_130506_sparda_orange_vs_wolfbräu` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_20_130506_sparda_orange_vs_wolfbräu` ENABLE KEYS */;


--
-- Definition of table `kc2011_20_130510_sparda_orange_vs_wolfbräu`
--

DROP TABLE IF EXISTS `kc2011_20_130510_sparda_orange_vs_wolfbräu`;
CREATE TABLE `kc2011_20_130510_sparda_orange_vs_wolfbräu` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_20_130510_sparda_orange_vs_wolfbräu`
--

/*!40000 ALTER TABLE `kc2011_20_130510_sparda_orange_vs_wolfbräu` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_20_130510_sparda_orange_vs_wolfbräu` ENABLE KEYS */;


--
-- Definition of table `kc2011_20_130523_sparda_orange_vs_wolfbräu`
--

DROP TABLE IF EXISTS `kc2011_20_130523_sparda_orange_vs_wolfbräu`;
CREATE TABLE `kc2011_20_130523_sparda_orange_vs_wolfbräu` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_20_130523_sparda_orange_vs_wolfbräu`
--

/*!40000 ALTER TABLE `kc2011_20_130523_sparda_orange_vs_wolfbräu` DISABLE KEYS */;
INSERT INTO `kc2011_20_130523_sparda_orange_vs_wolfbräu` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'01:31',0,1,'Wolfbräu','Ben vom Berg',NULL),
 (2,'02:06',1,1,'Sparda Orange','Fips',NULL),
 (3,'02:24',2,1,'Sparda Orange','Fips',NULL),
 (4,'02:38',2,2,'Wolfbräu','Ben vom Berg',NULL),
 (5,'04:45',2,3,'Wolfbräu','Tha Man',NULL),
 (6,'06:13',3,3,'Sparda Orange','Schlampe',NULL),
 (7,'08:23',4,3,'Sparda Orange','Schlampe',NULL);
/*!40000 ALTER TABLE `kc2011_20_130523_sparda_orange_vs_wolfbräu` ENABLE KEYS */;


--
-- Definition of table `kc2011_21_131609_porsche_vs_mollutz`
--

DROP TABLE IF EXISTS `kc2011_21_131609_porsche_vs_mollutz`;
CREATE TABLE `kc2011_21_131609_porsche_vs_mollutz` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_21_131609_porsche_vs_mollutz`
--

/*!40000 ALTER TABLE `kc2011_21_131609_porsche_vs_mollutz` DISABLE KEYS */;
INSERT INTO `kc2011_21_131609_porsche_vs_mollutz` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:57',0,1,'Mollutz','Molli',NULL),
 (2,'01:58',0,2,'Mollutz','Molli',NULL),
 (3,'02:17',0,3,'Mollutz','Molli',NULL),
 (4,'02:34',0,4,'Mollutz','Molli',NULL),
 (5,'02:52',1,4,'Porsche','Luca T.',NULL),
 (6,'03:34',2,4,'Porsche','Luca T.',NULL),
 (7,'04:06',3,4,'Porsche','Mad Maz',NULL),
 (8,'04:21',3,5,'Mollutz','Molli',NULL),
 (9,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (10,'06:31',3,6,'Mollutz','Molli',NULL),
 (11,'08:05',3,7,'Mollutz','Molli',NULL),
 (12,'09:07',3,8,'Mollutz','Molli',NULL),
 (13,'09:57',4,8,'Porsche','Mad Maz',NULL);
/*!40000 ALTER TABLE `kc2011_21_131609_porsche_vs_mollutz` ENABLE KEYS */;


--
-- Definition of table `kc2011_22_133019_waldmeister_vs_deportivo`
--

DROP TABLE IF EXISTS `kc2011_22_133019_waldmeister_vs_deportivo`;
CREATE TABLE `kc2011_22_133019_waldmeister_vs_deportivo` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_22_133019_waldmeister_vs_deportivo`
--

/*!40000 ALTER TABLE `kc2011_22_133019_waldmeister_vs_deportivo` DISABLE KEYS */;
INSERT INTO `kc2011_22_133019_waldmeister_vs_deportivo` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:36',1,0,'Waldmeister','Ben vom Wald',NULL),
 (2,'01:13',2,0,'Waldmeister','Mario vom Wald',NULL),
 (3,'01:21',3,0,'Waldmeister','Mario vom Wald',NULL),
 (4,'01:30',3,1,'Deportivo','Inge',NULL),
 (5,'02:06',4,1,'Waldmeister','Ben vom Wald',NULL),
 (6,'02:22',4,2,'Deportivo','Heif',NULL),
 (7,'02:39',5,2,'Waldmeister','Ben vom Wald',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'03:21',6,2,'Waldmeister','Mario vom Wald',NULL),
 (10,'03:37',7,2,'Waldmeister','Mario vom Wald',NULL),
 (11,'04:47',8,2,'Waldmeister','Mario vom Wald',NULL),
 (12,'05:34',9,2,'Waldmeister','Ben vom Wald',NULL),
 (13,'06:29',9,3,'Deportivo','Heif',NULL),
 (14,'06:49',10,3,'Waldmeister','Ben vom Wald',NULL);
/*!40000 ALTER TABLE `kc2011_22_133019_waldmeister_vs_deportivo` ENABLE KEYS */;


--
-- Definition of table `kc2011_23_133856_sion_kölsch_vs_sauffpark`
--

DROP TABLE IF EXISTS `kc2011_23_133856_sion_kölsch_vs_sauffpark`;
CREATE TABLE `kc2011_23_133856_sion_kölsch_vs_sauffpark` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_23_133856_sion_kölsch_vs_sauffpark`
--

/*!40000 ALTER TABLE `kc2011_23_133856_sion_kölsch_vs_sauffpark` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_23_133856_sion_kölsch_vs_sauffpark` ENABLE KEYS */;


--
-- Definition of table `kc2011_23_134015_sion_kölsch_vs_sauffpark`
--

DROP TABLE IF EXISTS `kc2011_23_134015_sion_kölsch_vs_sauffpark`;
CREATE TABLE `kc2011_23_134015_sion_kölsch_vs_sauffpark` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_23_134015_sion_kölsch_vs_sauffpark`
--

/*!40000 ALTER TABLE `kc2011_23_134015_sion_kölsch_vs_sauffpark` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_23_134015_sion_kölsch_vs_sauffpark` ENABLE KEYS */;


--
-- Definition of table `kc2011_23_134028_sion_kölsch_vs_sauffpark`
--

DROP TABLE IF EXISTS `kc2011_23_134028_sion_kölsch_vs_sauffpark`;
CREATE TABLE `kc2011_23_134028_sion_kölsch_vs_sauffpark` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_23_134028_sion_kölsch_vs_sauffpark`
--

/*!40000 ALTER TABLE `kc2011_23_134028_sion_kölsch_vs_sauffpark` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_23_134028_sion_kölsch_vs_sauffpark` ENABLE KEYS */;


--
-- Definition of table `kc2011_23_134041_sion_kölsch_vs_sauffpark`
--

DROP TABLE IF EXISTS `kc2011_23_134041_sion_kölsch_vs_sauffpark`;
CREATE TABLE `kc2011_23_134041_sion_kölsch_vs_sauffpark` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_23_134041_sion_kölsch_vs_sauffpark`
--

/*!40000 ALTER TABLE `kc2011_23_134041_sion_kölsch_vs_sauffpark` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_23_134041_sion_kölsch_vs_sauffpark` ENABLE KEYS */;


--
-- Definition of table `kc2011_23_134051_sion_kölsch_vs_sauffpark`
--

DROP TABLE IF EXISTS `kc2011_23_134051_sion_kölsch_vs_sauffpark`;
CREATE TABLE `kc2011_23_134051_sion_kölsch_vs_sauffpark` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_23_134051_sion_kölsch_vs_sauffpark`
--

/*!40000 ALTER TABLE `kc2011_23_134051_sion_kölsch_vs_sauffpark` DISABLE KEYS */;
INSERT INTO `kc2011_23_134051_sion_kölsch_vs_sauffpark` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:35',0,1,'Sauffpark','Mahony',NULL),
 (2,'02:20',1,1,'Sion Kölsch','Emre',NULL),
 (3,'03:52',1,2,'Sauffpark','Mahony',NULL),
 (4,'04:26',2,2,'Sion Kölsch','Emre',NULL),
 (5,'05:43',2,3,'Sauffpark','Shooter',NULL),
 (6,'06:02',2,4,'Sauffpark','Mahony',NULL),
 (7,'06:39',3,4,'Sion Kölsch','Captain Doerk',NULL),
 (8,'07:07',3,5,'Sauffpark','Mahony',NULL),
 (9,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (10,'07:17',4,5,'Sion Kölsch','Captain Doerk',NULL),
 (11,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (12,'07:41',5,5,'Sion Kölsch','Emre',NULL),
 (13,'08:50',6,6,'Sion Kölsch','Captain Doerk',NULL),
 (14,'09:13',7,6,'Sion Kölsch','Emre',NULL),
 (15,'09:50',8,6,'Sion Kölsch','Emre',NULL);
/*!40000 ALTER TABLE `kc2011_23_134051_sion_kölsch_vs_sauffpark` ENABLE KEYS */;


--
-- Definition of table `kc2011_24_135225_olympiakos_vs_bergmacht`
--

DROP TABLE IF EXISTS `kc2011_24_135225_olympiakos_vs_bergmacht`;
CREATE TABLE `kc2011_24_135225_olympiakos_vs_bergmacht` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_24_135225_olympiakos_vs_bergmacht`
--

/*!40000 ALTER TABLE `kc2011_24_135225_olympiakos_vs_bergmacht` DISABLE KEYS */;
INSERT INTO `kc2011_24_135225_olympiakos_vs_bergmacht` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:24',0,1,'Bergmacht','Flo vom Berg',NULL),
 (2,'01:09',1,1,'Olympiakos','Theo',NULL),
 (3,'03:49',1,2,'Bergmacht','Flo vom Berg',NULL),
 (4,'04:14',2,2,'Olympiakos','Andreas K.',NULL),
 (5,'04:41',2,3,'Bergmacht','Flo vom Berg',NULL),
 (6,'06:06',3,3,'Olympiakos','Theo',NULL),
 (7,'06:51',4,3,'Olympiakos','Theo',NULL),
 (8,'07:28',4,4,'Bergmacht','Flo vom Berg',NULL),
 (9,'08:07',4,4,'Bergmacht','Flo vom Berg',NULL),
 (10,'08:20',5,4,'Olympiakos','Theo',NULL),
 (11,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (12,'08:34',6,4,'Olympiakos','Theo',NULL),
 (13,'09:26',6,5,'Bergmacht','Flo vom Berg',NULL),
 (14,'09:37',6,6,'Bergmacht','Flo vom Berg',NULL);
/*!40000 ALTER TABLE `kc2011_24_135225_olympiakos_vs_bergmacht` ENABLE KEYS */;


--
-- Definition of table `kc2011_26_141546_galatasaray_vs_hulaboys`
--

DROP TABLE IF EXISTS `kc2011_26_141546_galatasaray_vs_hulaboys`;
CREATE TABLE `kc2011_26_141546_galatasaray_vs_hulaboys` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_26_141546_galatasaray_vs_hulaboys`
--

/*!40000 ALTER TABLE `kc2011_26_141546_galatasaray_vs_hulaboys` DISABLE KEYS */;
INSERT INTO `kc2011_26_141546_galatasaray_vs_hulaboys` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'02:12',0,1,'Hulaboys','Schmiddi',NULL),
 (2,'02:18',1,1,'Galatasaray','Mirko',NULL),
 (3,'02:50',2,1,'Galatasaray','Mirko',NULL),
 (4,'03:07',2,2,'Hulaboys','Schmiddi',NULL),
 (5,'04:31',3,2,'Hulaboys','Wengi',NULL),
 (6,'05:28',3,3,'Hulaboys','Wengi',NULL),
 (7,'06:15',4,3,'Galatasaray','Marvin',NULL),
 (8,'06:54',4,4,'Hulaboys','Schmiddi',NULL),
 (9,'08:46',4,5,'Hulaboys','Wengi',NULL),
 (10,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (11,'09:07',4,6,'Hulaboys','Wengi',NULL);
/*!40000 ALTER TABLE `kc2011_26_141546_galatasaray_vs_hulaboys` ENABLE KEYS */;


--
-- Definition of table `kc2011_27_142732_gaffel_kölsch_vs_tesa_power`
--

DROP TABLE IF EXISTS `kc2011_27_142732_gaffel_kölsch_vs_tesa_power`;
CREATE TABLE `kc2011_27_142732_gaffel_kölsch_vs_tesa_power` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_27_142732_gaffel_kölsch_vs_tesa_power`
--

/*!40000 ALTER TABLE `kc2011_27_142732_gaffel_kölsch_vs_tesa_power` DISABLE KEYS */;
INSERT INTO `kc2011_27_142732_gaffel_kölsch_vs_tesa_power` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'01:47',0,1,'Tesa Power','Dirk L.',NULL),
 (2,'02:53',0,2,'Tesa Power','Timo P.',NULL),
 (3,'03:49',0,3,'Tesa Power','Timo P.',NULL),
 (4,'04:29',0,3,'Tesa Power','Timo P.',NULL),
 (5,'06:07',0,4,'Tesa Power','Timo P.',NULL),
 (6,'06:38',1,4,'Gaffel Kölsch','Der Jupp',NULL),
 (7,'07:56',1,5,'Tesa Power','Timo P.',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'08:46',1,6,'Tesa Power','Dirk L.',NULL);
/*!40000 ALTER TABLE `kc2011_27_142732_gaffel_kölsch_vs_tesa_power` ENABLE KEYS */;


--
-- Definition of table `kc2011_28_144037_condom_kickers_vs_sparda_orange`
--

DROP TABLE IF EXISTS `kc2011_28_144037_condom_kickers_vs_sparda_orange`;
CREATE TABLE `kc2011_28_144037_condom_kickers_vs_sparda_orange` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=13 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_28_144037_condom_kickers_vs_sparda_orange`
--

/*!40000 ALTER TABLE `kc2011_28_144037_condom_kickers_vs_sparda_orange` DISABLE KEYS */;
INSERT INTO `kc2011_28_144037_condom_kickers_vs_sparda_orange` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:36',1,0,'Condom Kickers','Shorty',NULL),
 (2,'01:42',1,1,'Sparda Orange','Fips',NULL),
 (3,'02:01',2,1,'Condom Kickers','Shorty',NULL),
 (4,'04:13',3,1,'Condom Kickers','Quicky',NULL),
 (5,'04:17',3,2,'Sparda Orange','Fips',NULL),
 (6,'06:59',3,3,'Sparda Orange','Schlampe',NULL),
 (7,'07:06',4,3,'Condom Kickers','Quicky',NULL),
 (8,'07:39',5,3,'Condom Kickers','Shorty',NULL),
 (9,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (10,'08:08',5,4,'Sparda Orange','Fips',NULL),
 (11,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (12,'08:53',5,5,'Sparda Orange','Schlampe',NULL);
/*!40000 ALTER TABLE `kc2011_28_144037_condom_kickers_vs_sparda_orange` ENABLE KEYS */;


--
-- Definition of table `kc2011_29_145547_porsche_vs_ensinger`
--

DROP TABLE IF EXISTS `kc2011_29_145547_porsche_vs_ensinger`;
CREATE TABLE `kc2011_29_145547_porsche_vs_ensinger` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_29_145547_porsche_vs_ensinger`
--

/*!40000 ALTER TABLE `kc2011_29_145547_porsche_vs_ensinger` DISABLE KEYS */;
INSERT INTO `kc2011_29_145547_porsche_vs_ensinger` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:12',0,1,'Ensinger','Fraggy',NULL),
 (2,'01:48',1,1,'Porsche','Mad Maz',NULL),
 (3,'02:41',2,1,'Porsche','Mad Maz',NULL),
 (4,'03:07',3,1,'Porsche','Mad Maz',NULL),
 (5,'03:50',4,1,'Porsche','Luca T.',NULL),
 (6,'04:05',4,2,'Ensinger','Jens L.',NULL),
 (7,'04:31',5,1,'Porsche','Luca T.',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'05:44',6,1,'Porsche','Luca T.',NULL),
 (10,'06:35',7,1,'Porsche','Luca T.',NULL),
 (11,'06:38',7,2,'Ensinger','Jens L.',NULL),
 (12,'08:24',7,3,'Ensinger','Jens L.',NULL),
 (13,'09:26',8,3,'Porsche','Luca T.',NULL),
 (14,'09:29',8,4,'Ensinger','Fraggy',NULL),
 (15,'09:59',9,4,'Porsche','Luca T.',NULL);
/*!40000 ALTER TABLE `kc2011_29_145547_porsche_vs_ensinger` ENABLE KEYS */;


--
-- Definition of table `kc2011_30_151244_mollutz_vs_wolfbräu`
--

DROP TABLE IF EXISTS `kc2011_30_151244_mollutz_vs_wolfbräu`;
CREATE TABLE `kc2011_30_151244_mollutz_vs_wolfbräu` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_30_151244_mollutz_vs_wolfbräu`
--

/*!40000 ALTER TABLE `kc2011_30_151244_mollutz_vs_wolfbräu` DISABLE KEYS */;
INSERT INTO `kc2011_30_151244_mollutz_vs_wolfbräu` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:40',0,1,'Wolfbräu','Ben vom Berg',NULL),
 (2,'00:53',1,1,'Mollutz','bewave',NULL),
 (3,'01:07',2,1,'Mollutz','Molli',NULL),
 (4,'01:13',2,2,'Wolfbräu','Tha Man',NULL),
 (5,'01:29',3,2,'Mollutz','Molli',NULL),
 (6,'01:39',3,3,'Wolfbräu','Ben vom Berg',NULL),
 (7,'03:14',4,3,'Mollutz','bewave',NULL),
 (8,'03:44',4,4,'Wolfbräu','Ben vom Berg',NULL),
 (9,'04:15',5,4,'Mollutz','Molli',NULL),
 (10,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (11,'04:39',5,5,'Wolfbräu','Tha Man',NULL),
 (12,'05:04',6,5,'Mollutz','bewave',NULL),
 (13,'09:17',6,6,'Wolfbräu','Tha Man',NULL);
/*!40000 ALTER TABLE `kc2011_30_151244_mollutz_vs_wolfbräu` ENABLE KEYS */;


--
-- Definition of table `kc2011_31_152718_waldmeister_vs_sion_kölsch`
--

DROP TABLE IF EXISTS `kc2011_31_152718_waldmeister_vs_sion_kölsch`;
CREATE TABLE `kc2011_31_152718_waldmeister_vs_sion_kölsch` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_31_152718_waldmeister_vs_sion_kölsch`
--

/*!40000 ALTER TABLE `kc2011_31_152718_waldmeister_vs_sion_kölsch` DISABLE KEYS */;
INSERT INTO `kc2011_31_152718_waldmeister_vs_sion_kölsch` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:52',1,0,'Waldmeister','Ben vom Wald',NULL),
 (2,'01:39',1,1,'Sion Kölsch','Emre',NULL),
 (3,'01:53',2,1,'Waldmeister','Ben vom Wald',NULL),
 (4,'03:30',2,2,'Sion Kölsch','Emre',NULL),
 (5,'03:49',3,2,'Waldmeister','Ben vom Wald',NULL),
 (6,'04:32',4,2,'Waldmeister','Mario vom Wald',NULL),
 (7,'04:50',5,2,'Waldmeister','Ben vom Wald',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'06:16',6,2,'Waldmeister','Mario vom Wald',NULL),
 (10,'07:48',6,3,'Sion Kölsch','Captain Doerk',NULL),
 (11,'08:07',6,4,'Sion Kölsch','Captain Doerk',NULL),
 (12,'09:26',6,6,'Sion Kölsch','Captain Doerk',NULL),
 (13,'09:40',6,7,'Sion Kölsch','Emre',NULL),
 (14,'09:57',7,7,'Waldmeister','Mario vom Wald',NULL);
/*!40000 ALTER TABLE `kc2011_31_152718_waldmeister_vs_sion_kölsch` ENABLE KEYS */;


--
-- Definition of table `kc2011_32_153918_olympiakos_vs_deportivo`
--

DROP TABLE IF EXISTS `kc2011_32_153918_olympiakos_vs_deportivo`;
CREATE TABLE `kc2011_32_153918_olympiakos_vs_deportivo` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_32_153918_olympiakos_vs_deportivo`
--

/*!40000 ALTER TABLE `kc2011_32_153918_olympiakos_vs_deportivo` DISABLE KEYS */;
INSERT INTO `kc2011_32_153918_olympiakos_vs_deportivo` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'01:00',0,1,'Deportivo','Inge',NULL),
 (2,'01:34',1,1,'Deportivo','Inge',NULL),
 (3,'02:41',3,0,'Olympiakos','Andreas K.',NULL),
 (4,'03:05',3,1,'Deportivo','Inge',NULL),
 (5,'03:28',4,1,'Olympiakos','Theo',NULL),
 (6,'03:41',4,2,'Deportivo','Heif',NULL),
 (7,'04:29',5,2,'Olympiakos','Andreas K.',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'06:19',5,3,'Deportivo','Heif',NULL),
 (10,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (11,'07:00',7,2,'Olympiakos','Theo',NULL),
 (12,'07:57',7,3,'Deportivo','Heif',NULL),
 (13,'08:54',8,3,'Olympiakos','Theo',NULL);
/*!40000 ALTER TABLE `kc2011_32_153918_olympiakos_vs_deportivo` ENABLE KEYS */;


--
-- Definition of table `kc2011_33_154959_bergmacht_vs_sauffpark`
--

DROP TABLE IF EXISTS `kc2011_33_154959_bergmacht_vs_sauffpark`;
CREATE TABLE `kc2011_33_154959_bergmacht_vs_sauffpark` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_33_154959_bergmacht_vs_sauffpark`
--

/*!40000 ALTER TABLE `kc2011_33_154959_bergmacht_vs_sauffpark` DISABLE KEYS */;
INSERT INTO `kc2011_33_154959_bergmacht_vs_sauffpark` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'01:21',1,0,'Bergmacht','Flo vom Berg',NULL),
 (2,'01:34',1,1,'Sauffpark','Shooter',NULL),
 (3,'02:13',2,1,'Bergmacht','Dave vom Berg',NULL),
 (4,'02:19',2,2,'Sauffpark','Shooter',NULL),
 (5,'02:48',3,2,'Bergmacht','Dave vom Berg',NULL),
 (6,'03:11',4,2,'Bergmacht','Flo vom Berg',NULL),
 (7,'04:21',5,2,'Bergmacht','Flo vom Berg',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'05:47',6,2,'Bergmacht','Dave vom Berg',NULL),
 (10,'06:00',7,2,'Bergmacht','Flo vom Berg',NULL),
 (11,'06:13',7,3,'Sauffpark','Shooter',NULL),
 (12,'06:23',8,3,'Bergmacht','Dave vom Berg',NULL),
 (13,'07:17',9,3,'Bergmacht','Flo vom Berg',NULL),
 (14,'08:12',10,3,'Bergmacht','Dave vom Berg',NULL);
/*!40000 ALTER TABLE `kc2011_33_154959_bergmacht_vs_sauffpark` ENABLE KEYS */;


--
-- Definition of table `kc2011_35_160245_gaffel_kölsch_vs_die_wilden_kerle`
--

DROP TABLE IF EXISTS `kc2011_35_160245_gaffel_kölsch_vs_die_wilden_kerle`;
CREATE TABLE `kc2011_35_160245_gaffel_kölsch_vs_die_wilden_kerle` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_35_160245_gaffel_kölsch_vs_die_wilden_kerle`
--

/*!40000 ALTER TABLE `kc2011_35_160245_gaffel_kölsch_vs_die_wilden_kerle` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_35_160245_gaffel_kölsch_vs_die_wilden_kerle` ENABLE KEYS */;


--
-- Definition of table `kc2011_35_161745_gaffel_kölsch_vs_die_wilden_kerle`
--

DROP TABLE IF EXISTS `kc2011_35_161745_gaffel_kölsch_vs_die_wilden_kerle`;
CREATE TABLE `kc2011_35_161745_gaffel_kölsch_vs_die_wilden_kerle` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_35_161745_gaffel_kölsch_vs_die_wilden_kerle`
--

/*!40000 ALTER TABLE `kc2011_35_161745_gaffel_kölsch_vs_die_wilden_kerle` DISABLE KEYS */;
INSERT INTO `kc2011_35_161745_gaffel_kölsch_vs_die_wilden_kerle` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:37',0,1,'die wilden Kerle','Dolschewida',NULL),
 (2,'01:17',0,2,'die wilden Kerle','Jet-Ski',NULL),
 (3,'03:14',0,3,'die wilden Kerle','Jet-Ski',NULL),
 (4,'03:50',0,4,'die wilden Kerle','Dolschewida',NULL),
 (5,'04:38',0,6,'die wilden Kerle','Jet-Ski',NULL),
 (6,'05:01',0,7,'die wilden Kerle','Jet-Ski',NULL);
/*!40000 ALTER TABLE `kc2011_35_161745_gaffel_kölsch_vs_die_wilden_kerle` ENABLE KEYS */;


--
-- Definition of table `kc2011_36_162854_tesa_power_vs_hulaboys`
--

DROP TABLE IF EXISTS `kc2011_36_162854_tesa_power_vs_hulaboys`;
CREATE TABLE `kc2011_36_162854_tesa_power_vs_hulaboys` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_36_162854_tesa_power_vs_hulaboys`
--

/*!40000 ALTER TABLE `kc2011_36_162854_tesa_power_vs_hulaboys` DISABLE KEYS */;
INSERT INTO `kc2011_36_162854_tesa_power_vs_hulaboys` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'01:09',0,1,'Hulaboys','Wengi',NULL),
 (2,'01:33',0,2,'Hulaboys','Schmiddi',NULL),
 (3,'02:09',1,2,'Tesa Power','Dirk L.',NULL),
 (4,'03:37',1,3,'Hulaboys','Wengi',NULL),
 (5,'04:40',1,4,'Hulaboys','Wengi',NULL),
 (6,'05:02',1,5,'Hulaboys','Wengi',NULL),
 (7,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (8,'07:26',1,6,'Hulaboys','Schmiddi',NULL);
/*!40000 ALTER TABLE `kc2011_36_162854_tesa_power_vs_hulaboys` ENABLE KEYS */;


--
-- Definition of table `kc2011_37_163933_condom_kickers_vs_porsche`
--

DROP TABLE IF EXISTS `kc2011_37_163933_condom_kickers_vs_porsche`;
CREATE TABLE `kc2011_37_163933_condom_kickers_vs_porsche` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=20 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_37_163933_condom_kickers_vs_porsche`
--

/*!40000 ALTER TABLE `kc2011_37_163933_condom_kickers_vs_porsche` DISABLE KEYS */;
INSERT INTO `kc2011_37_163933_condom_kickers_vs_porsche` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:37',1,0,'Condom Kickers','Shorty',NULL),
 (2,'01:02',2,0,'Condom Kickers','Shorty',NULL),
 (3,'02:51',3,0,'Condom Kickers','Shorty',NULL),
 (4,'02:54',3,1,'Porsche','Mad Maz',NULL),
 (5,'02:59',4,1,'Condom Kickers','Quicky',NULL),
 (6,'04:13',5,1,'Condom Kickers','Shorty',NULL),
 (7,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (8,'04:16',5,2,'Porsche','Mad Maz',NULL),
 (9,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (10,'06:15',5,3,'Porsche','Mad Maz',NULL),
 (11,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (12,'06:42',6,3,'Condom Kickers','Quicky',NULL),
 (13,'07:05',6,4,'Porsche','Mad Maz',NULL),
 (14,'07:35',6,5,'Porsche','Mad Maz',NULL),
 (15,'07:57',7,5,'Condom Kickers','Quicky',NULL),
 (16,'08:03',8,5,'Condom Kickers','Quicky',NULL),
 (17,'08:11',9,5,'Condom Kickers','Quicky',NULL),
 (18,'08:34',9,6,'Porsche','Mad Maz',NULL),
 (19,'09:43',10,6,'Condom Kickers','Quicky',NULL);
/*!40000 ALTER TABLE `kc2011_37_163933_condom_kickers_vs_porsche` ENABLE KEYS */;


--
-- Definition of table `kc2011_38_165233_mollutz_vs_sparda_orange`
--

DROP TABLE IF EXISTS `kc2011_38_165233_mollutz_vs_sparda_orange`;
CREATE TABLE `kc2011_38_165233_mollutz_vs_sparda_orange` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_38_165233_mollutz_vs_sparda_orange`
--

/*!40000 ALTER TABLE `kc2011_38_165233_mollutz_vs_sparda_orange` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_38_165233_mollutz_vs_sparda_orange` ENABLE KEYS */;


--
-- Definition of table `kc2011_38_165643_mollutz_vs_sparda_orange`
--

DROP TABLE IF EXISTS `kc2011_38_165643_mollutz_vs_sparda_orange`;
CREATE TABLE `kc2011_38_165643_mollutz_vs_sparda_orange` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_38_165643_mollutz_vs_sparda_orange`
--

/*!40000 ALTER TABLE `kc2011_38_165643_mollutz_vs_sparda_orange` DISABLE KEYS */;
INSERT INTO `kc2011_38_165643_mollutz_vs_sparda_orange` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'02:39',1,0,'Mollutz','bewave',NULL),
 (2,'02:47',1,1,'Sparda Orange','Fips',NULL),
 (3,'03:22',2,2,'Mollutz','bewave',NULL),
 (4,'03:51',3,2,'Mollutz','bewave',NULL),
 (5,'04:07',4,2,'Mollutz','Molli',NULL),
 (6,'04:42',5,2,'Mollutz','Molli',NULL),
 (7,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (8,'05:06',5,3,'Sparda Orange','Fips',NULL),
 (9,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (10,'06:47',6,4,'Mollutz','bewave',NULL),
 (11,'08:20',6,6,'Mollutz','Molli',NULL),
 (12,'08:40',6,7,'Sparda Orange','Schlampe',NULL),
 (13,'09:59',6,8,'Sparda Orange','Fips',NULL);
/*!40000 ALTER TABLE `kc2011_38_165643_mollutz_vs_sparda_orange` ENABLE KEYS */;


--
-- Definition of table `kc2011_39_165535_wolfbräu_vs_ensinger`
--

DROP TABLE IF EXISTS `kc2011_39_165535_wolfbräu_vs_ensinger`;
CREATE TABLE `kc2011_39_165535_wolfbräu_vs_ensinger` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_39_165535_wolfbräu_vs_ensinger`
--

/*!40000 ALTER TABLE `kc2011_39_165535_wolfbräu_vs_ensinger` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_39_165535_wolfbräu_vs_ensinger` ENABLE KEYS */;


--
-- Definition of table `kc2011_39_170738_wolfbräu_vs_ensinger`
--

DROP TABLE IF EXISTS `kc2011_39_170738_wolfbräu_vs_ensinger`;
CREATE TABLE `kc2011_39_170738_wolfbräu_vs_ensinger` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_39_170738_wolfbräu_vs_ensinger`
--

/*!40000 ALTER TABLE `kc2011_39_170738_wolfbräu_vs_ensinger` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_39_170738_wolfbräu_vs_ensinger` ENABLE KEYS */;


--
-- Definition of table `kc2011_39_171023_wolfbräu_vs_ensinger`
--

DROP TABLE IF EXISTS `kc2011_39_171023_wolfbräu_vs_ensinger`;
CREATE TABLE `kc2011_39_171023_wolfbräu_vs_ensinger` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_39_171023_wolfbräu_vs_ensinger`
--

/*!40000 ALTER TABLE `kc2011_39_171023_wolfbräu_vs_ensinger` DISABLE KEYS */;
INSERT INTO `kc2011_39_171023_wolfbräu_vs_ensinger` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:35',0,1,'Ensinger','Fraggy',NULL),
 (2,'01:07',1,1,'Wolfbräu','Ben vom Berg',NULL),
 (3,'01:29',2,1,'Wolfbräu','Ben vom Berg',NULL),
 (4,'01:52',3,1,'Wolfbräu','Ben vom Berg',NULL),
 (5,'02:11',4,1,'Wolfbräu','Ben vom Berg',NULL),
 (6,'03:01',4,1,'Wolfbräu','Ben vom Berg',NULL),
 (7,'03:19',5,1,'Wolfbräu','Tha Man',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'03:49',6,1,'Wolfbräu','Ben vom Berg',NULL),
 (10,'04:08',6,2,'Ensinger','Jens L.',NULL),
 (11,'04:36',6,3,'Ensinger','Fraggy',NULL),
 (12,'04:44',7,3,'Wolfbräu','Ben vom Berg',NULL),
 (13,'04:51',8,3,'Wolfbräu','Ben vom Berg',NULL),
 (14,'05:30',9,3,'Wolfbräu','Tha Man',NULL),
 (15,'06:06',10,3,'Wolfbräu','Ben vom Berg',NULL);
/*!40000 ALTER TABLE `kc2011_39_171023_wolfbräu_vs_ensinger` ENABLE KEYS */;


--
-- Definition of table `kc2011_40_172230_waldmeister_vs_olympiakos`
--

DROP TABLE IF EXISTS `kc2011_40_172230_waldmeister_vs_olympiakos`;
CREATE TABLE `kc2011_40_172230_waldmeister_vs_olympiakos` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_40_172230_waldmeister_vs_olympiakos`
--

/*!40000 ALTER TABLE `kc2011_40_172230_waldmeister_vs_olympiakos` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_40_172230_waldmeister_vs_olympiakos` ENABLE KEYS */;


--
-- Definition of table `kc2011_40_172454_waldmeister_vs_olympiakos`
--

DROP TABLE IF EXISTS `kc2011_40_172454_waldmeister_vs_olympiakos`;
CREATE TABLE `kc2011_40_172454_waldmeister_vs_olympiakos` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_40_172454_waldmeister_vs_olympiakos`
--

/*!40000 ALTER TABLE `kc2011_40_172454_waldmeister_vs_olympiakos` DISABLE KEYS */;
INSERT INTO `kc2011_40_172454_waldmeister_vs_olympiakos` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:04',0,1,'Olympiakos','Andreas K.',NULL),
 (2,'00:52',1,1,'Waldmeister','Ben vom Wald',NULL),
 (3,'00:57',1,2,'Olympiakos','Theo',NULL),
 (4,'06:26',3,6,'Olympiakos','Theo',NULL),
 (5,'08:19',3,7,'Olympiakos','Andreas K.',NULL),
 (6,'09:35',4,7,'Waldmeister','Ben vom Wald',NULL),
 (7,'09:35',4,7,'Waldmeister','Ben vom Wald',NULL),
 (8,'09:52',4,8,'Olympiakos','Theo',NULL);
/*!40000 ALTER TABLE `kc2011_40_172454_waldmeister_vs_olympiakos` ENABLE KEYS */;


--
-- Definition of table `kc2011_41_173648_bergmacht_vs_sion_kölsch`
--

DROP TABLE IF EXISTS `kc2011_41_173648_bergmacht_vs_sion_kölsch`;
CREATE TABLE `kc2011_41_173648_bergmacht_vs_sion_kölsch` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_41_173648_bergmacht_vs_sion_kölsch`
--

/*!40000 ALTER TABLE `kc2011_41_173648_bergmacht_vs_sion_kölsch` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_41_173648_bergmacht_vs_sion_kölsch` ENABLE KEYS */;


--
-- Definition of table `kc2011_41_173653_bergmacht_vs_sion_kölsch`
--

DROP TABLE IF EXISTS `kc2011_41_173653_bergmacht_vs_sion_kölsch`;
CREATE TABLE `kc2011_41_173653_bergmacht_vs_sion_kölsch` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_41_173653_bergmacht_vs_sion_kölsch`
--

/*!40000 ALTER TABLE `kc2011_41_173653_bergmacht_vs_sion_kölsch` DISABLE KEYS */;
INSERT INTO `kc2011_41_173653_bergmacht_vs_sion_kölsch` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:17',0,1,'Sion Kölsch','Emre',NULL),
 (2,'00:17',0,1,'Sion Kölsch','Emre',NULL),
 (3,'01:17',0,2,'Sion Kölsch','Emre',NULL),
 (4,'01:17',0,2,'Sion Kölsch','Emre',NULL),
 (5,'01:17',0,2,'Sion Kölsch','Emre',NULL),
 (6,'01:17',0,2,'Sion Kölsch','Emre',NULL),
 (7,'02:11',4,2,'Bergmacht','Flo vom Berg',NULL),
 (8,'03:01',4,3,'Sion Kölsch','Emre',NULL),
 (9,'04:23',5,3,'Bergmacht','Dave vom Berg',NULL),
 (10,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (11,'04:46',7,3,'Bergmacht','Dave vom Berg',NULL),
 (12,'07:54',8,3,'Bergmacht','Dave vom Berg',NULL),
 (13,'08:32',9,3,'Bergmacht','Dave vom Berg',NULL),
 (14,'09:27',10,3,'Bergmacht','Dave vom Berg',NULL);
/*!40000 ALTER TABLE `kc2011_41_173653_bergmacht_vs_sion_kölsch` ENABLE KEYS */;


--
-- Definition of table `kc2011_42_174750_sauffpark_vs_deportivo`
--

DROP TABLE IF EXISTS `kc2011_42_174750_sauffpark_vs_deportivo`;
CREATE TABLE `kc2011_42_174750_sauffpark_vs_deportivo` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_42_174750_sauffpark_vs_deportivo`
--

/*!40000 ALTER TABLE `kc2011_42_174750_sauffpark_vs_deportivo` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_42_174750_sauffpark_vs_deportivo` ENABLE KEYS */;


--
-- Definition of table `kc2011_42_175010_sauffpark_vs_deportivo`
--

DROP TABLE IF EXISTS `kc2011_42_175010_sauffpark_vs_deportivo`;
CREATE TABLE `kc2011_42_175010_sauffpark_vs_deportivo` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_42_175010_sauffpark_vs_deportivo`
--

/*!40000 ALTER TABLE `kc2011_42_175010_sauffpark_vs_deportivo` DISABLE KEYS */;
INSERT INTO `kc2011_42_175010_sauffpark_vs_deportivo` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'01:14',0,1,'Deportivo','Inge',NULL),
 (2,'02:22',0,2,'Deportivo','Heif',NULL),
 (3,'03:07',0,3,'Deportivo','Inge',NULL),
 (4,'03:53',1,3,'Sauffpark','Shooter',NULL),
 (5,'04:38',1,4,'Deportivo','Inge',NULL),
 (6,'06:27',2,4,'Sauffpark','Mahony',NULL),
 (7,'06:58',2,5,'Deportivo','Inge',NULL),
 (8,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (9,'07:09',3,5,'Sauffpark','Mahony',NULL),
 (10,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (11,'07:21',3,6,'Deportivo','Inge',NULL),
 (12,'07:36',4,6,'Sauffpark','Mahony',NULL),
 (13,'08:12',5,6,'Sauffpark','Shooter',NULL),
 (14,'08:40',6,6,'Sauffpark','Shooter',NULL);
/*!40000 ALTER TABLE `kc2011_42_175010_sauffpark_vs_deportivo` ENABLE KEYS */;


--
-- Definition of table `kc2011_44_181355_tesa_power_vs_galatasaray`
--

DROP TABLE IF EXISTS `kc2011_44_181355_tesa_power_vs_galatasaray`;
CREATE TABLE `kc2011_44_181355_tesa_power_vs_galatasaray` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_44_181355_tesa_power_vs_galatasaray`
--

/*!40000 ALTER TABLE `kc2011_44_181355_tesa_power_vs_galatasaray` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_44_181355_tesa_power_vs_galatasaray` ENABLE KEYS */;


--
-- Definition of table `kc2011_44_181431_tesa_power_vs_galatasaray`
--

DROP TABLE IF EXISTS `kc2011_44_181431_tesa_power_vs_galatasaray`;
CREATE TABLE `kc2011_44_181431_tesa_power_vs_galatasaray` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_44_181431_tesa_power_vs_galatasaray`
--

/*!40000 ALTER TABLE `kc2011_44_181431_tesa_power_vs_galatasaray` DISABLE KEYS */;
INSERT INTO `kc2011_44_181431_tesa_power_vs_galatasaray` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'01:11',0,1,'Galatasaray','Mirko',NULL),
 (2,'02:05',1,1,'Tesa Power','Dirk L.',NULL),
 (3,'02:41',2,2,'Tesa Power','Dirk L.',NULL),
 (4,'02:57',3,2,'Tesa Power','Dirk L.',NULL),
 (5,'03:02',4,2,'Tesa Power','Dirk L.',NULL),
 (6,'03:23',5,2,'Tesa Power','Dirk L.',NULL),
 (7,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (8,'03:36',5,3,'Galatasaray','Mirko',NULL),
 (9,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (10,'04:31',6,3,'Tesa Power','Dirk L.',NULL),
 (11,'04:31',6,3,'Tesa Power','Dirk L.',NULL),
 (12,'05:18',7,3,'Tesa Power','Dirk L.',NULL),
 (13,'05:44',7,4,'Galatasaray','Mirko',NULL),
 (14,'05:53',8,4,'Tesa Power','Dirk L.',NULL),
 (15,'07:12',9,4,'Tesa Power','Dirk L.',NULL),
 (16,'07:30',9,5,'Galatasaray','Mirko',NULL),
 (17,'08:13',9,6,'Galatasaray','Mirko',NULL),
 (18,'09:04',10,6,'Tesa Power','Dirk L.',NULL);
/*!40000 ALTER TABLE `kc2011_44_181431_tesa_power_vs_galatasaray` ENABLE KEYS */;


--
-- Definition of table `kc2011_45_182357_hulaboys_vs_die_wilden_kerle`
--

DROP TABLE IF EXISTS `kc2011_45_182357_hulaboys_vs_die_wilden_kerle`;
CREATE TABLE `kc2011_45_182357_hulaboys_vs_die_wilden_kerle` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_45_182357_hulaboys_vs_die_wilden_kerle`
--

/*!40000 ALTER TABLE `kc2011_45_182357_hulaboys_vs_die_wilden_kerle` DISABLE KEYS */;
/*!40000 ALTER TABLE `kc2011_45_182357_hulaboys_vs_die_wilden_kerle` ENABLE KEYS */;


--
-- Definition of table `kc2011_45_182417_hulaboys_vs_die_wilden_kerle`
--

DROP TABLE IF EXISTS `kc2011_45_182417_hulaboys_vs_die_wilden_kerle`;
CREATE TABLE `kc2011_45_182417_hulaboys_vs_die_wilden_kerle` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_45_182417_hulaboys_vs_die_wilden_kerle`
--

/*!40000 ALTER TABLE `kc2011_45_182417_hulaboys_vs_die_wilden_kerle` DISABLE KEYS */;
INSERT INTO `kc2011_45_182417_hulaboys_vs_die_wilden_kerle` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'00:47',0,1,'die wilden Kerle','Dolschewida',NULL),
 (2,'01:19',1,1,'Hulaboys','Schmiddi',NULL),
 (3,'04:40',2,1,'Hulaboys','Schmiddi',NULL),
 (4,'05:44',2,2,'die wilden Kerle','Jet-Ski',NULL),
 (5,'06:18',2,3,'die wilden Kerle','Jet-Ski',NULL),
 (6,'06:48',2,4,'die wilden Kerle','Dolschewida',NULL),
 (7,'06:57',3,4,'Hulaboys','Schmiddi',NULL),
 (8,'07:40',3,5,'die wilden Kerle','Jet-Ski',NULL),
 (9,NULL,NULL,NULL,'-----','-----','--Halbzeit--'),
 (10,'08:18',3,6,'die wilden Kerle','Jet-Ski',NULL),
 (11,'08:56',4,6,'Hulaboys','Schmiddi',NULL),
 (12,'09:34',5,6,'Hulaboys','Wengi',NULL),
 (13,'09:44',6,6,'Hulaboys','Schmiddi',NULL),
 (14,'09:58',7,6,'Hulaboys','Schmiddi',NULL);
/*!40000 ALTER TABLE `kc2011_45_182417_hulaboys_vs_die_wilden_kerle` ENABLE KEYS */;


--
-- Definition of table `kc2011_spl`
--

DROP TABLE IF EXISTS `kc2011_spl`;
CREATE TABLE `kc2011_spl` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Tore1` int(10) unsigned DEFAULT NULL,
  `Tore2` int(10) unsigned DEFAULT NULL,
  `X` int(10) unsigned NOT NULL DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL,
  `Team1Name` varchar(30) DEFAULT NULL,
  `Team2Name` varchar(30) DEFAULT NULL,
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=46 DEFAULT CHARSET=utf8 PACK_KEYS=1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `kc2011_spl`
--

/*!40000 ALTER TABLE `kc2011_spl` DISABLE KEYS */;
INSERT INTO `kc2011_spl` (`SpielID`,`Team1`,`Team2`,`Tore1`,`Tore2`,`X`,`Gruppe`,`Team1Name`,`Team2Name`) VALUES 
 (1,'a1','a2',8,6,8,'a',NULL,NULL),
 (2,'a3','a4',5,5,8,'a',NULL,NULL),
 (3,'a5','a6',5,6,8,'a',NULL,NULL),
 (4,'b1','b2',9,8,8,'b',NULL,NULL),
 (5,'b3','b4',4,10,8,'b',NULL,NULL),
 (6,'b5','b6',4,9,8,'b',NULL,NULL),
 (7,'c1','c2',8,6,8,'c',NULL,NULL),
 (8,'c3','c4',10,3,8,'c',NULL,NULL),
 (9,'c5','c6',7,5,8,'c',NULL,NULL),
 (10,'a1','a3',5,4,8,'a',NULL,NULL),
 (11,'a5','a2',2,10,8,'a',NULL,NULL),
 (12,'a6','a4',7,4,8,'a',NULL,NULL),
 (13,'b1','b3',10,3,8,'b',NULL,NULL),
 (14,'b5','b2',6,8,8,'b',NULL,NULL),
 (15,'b6','b4',2,2,8,'b',NULL,NULL),
 (16,'c1','c3',2,10,8,'c',NULL,NULL),
 (17,'c5','c2',10,2,8,'c',NULL,NULL),
 (18,'c6','c4',10,6,8,'c',NULL,NULL),
 (19,'a1','a5',10,2,8,'a',NULL,NULL),
 (20,'a6','a3',4,3,8,'a',NULL,NULL),
 (21,'a4','a2',4,8,8,'a',NULL,NULL),
 (22,'b1','b5',10,3,8,'b',NULL,NULL),
 (23,'b6','b3',8,6,8,'b',NULL,NULL),
 (24,'b4','b2',6,6,8,'b',NULL,NULL),
 (25,'c1','c5',5,2,8,'c',NULL,NULL),
 (26,'c6','c3',4,6,8,'c',NULL,NULL),
 (27,'c4','c2',1,6,8,'c',NULL,NULL),
 (28,'a1','a6',5,5,8,'a',NULL,NULL),
 (29,'a4','a5',9,4,8,'a',NULL,NULL),
 (30,'a2','a3',6,6,8,'a',NULL,NULL),
 (31,'b1','b6',7,7,8,'b',NULL,NULL),
 (32,'b4','b5',8,3,8,'b',NULL,NULL),
 (33,'b2','b3',10,3,8,'b',NULL,NULL),
 (34,'c1','c6',5,3,8,'c',NULL,NULL),
 (35,'c4','c5',3,10,8,'c',NULL,NULL),
 (36,'c2','c3',1,6,8,'c',NULL,NULL),
 (37,'a1','a4',10,6,8,'a',NULL,NULL),
 (38,'a2','a6',9,6,8,'a',NULL,NULL),
 (39,'a3','a5',10,3,8,'a',NULL,NULL),
 (40,'b1','b4',4,8,8,'b',NULL,NULL),
 (41,'b2','b6',10,3,8,'b',NULL,NULL),
 (42,'b3','b5',6,6,8,'b',NULL,NULL),
 (43,'c1','c4',10,7,8,'c',NULL,NULL),
 (44,'c2','c6',10,6,8,'c',NULL,NULL),
 (45,'c3','c5',7,6,8,'c',NULL,NULL);
/*!40000 ALTER TABLE `kc2011_spl` ENABLE KEYS */;


--
-- Definition of table `kc2011_tbl`
--

DROP TABLE IF EXISTS `kc2011_tbl`;
CREATE TABLE `kc2011_tbl` (
  `TeamID` varchar(3) NOT NULL,
  `Spieler1` int(3) DEFAULT NULL,
  `Spieler2` int(3) DEFAULT NULL,
  `Spieler3` int(3) unsigned DEFAULT '0',
  `Spieler4` int(3) unsigned DEFAULT '0',
  `Spiele` int(3) unsigned DEFAULT '0',
  `S` int(3) unsigned DEFAULT '0',
  `U` int(3) unsigned DEFAULT '0',
  `N` int(3) unsigned DEFAULT '0',
  `ToreP` int(4) unsigned DEFAULT '0',
  `ToreM` int(4) unsigned DEFAULT '0',
  `Punkte` int(3) unsigned DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL DEFAULT 'a',
  `Status` int(3) unsigned DEFAULT '1',
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `kc2011_tbl`
--

/*!40000 ALTER TABLE `kc2011_tbl` DISABLE KEYS */;
INSERT INTO `kc2011_tbl` (`TeamID`,`Spieler1`,`Spieler2`,`Spieler3`,`Spieler4`,`Spiele`,`S`,`U`,`N`,`ToreP`,`ToreM`,`Punkte`,`Gruppe`,`Status`) VALUES 
 ('a1',NULL,NULL,0,0,5,4,1,0,38,23,13,'a',1),
 ('a2',NULL,NULL,0,0,6,2,2,2,36,28,8,'a',1),
 ('a3',NULL,NULL,0,0,5,1,2,2,28,23,5,'a',1),
 ('a4',NULL,NULL,0,0,5,1,1,3,28,34,4,'a',1),
 ('a5',NULL,NULL,0,0,5,0,0,5,16,45,0,'a',1),
 ('a6',NULL,NULL,0,0,6,4,2,0,30,23,14,'a',1),
 ('b1',NULL,NULL,0,0,5,3,1,1,40,29,10,'b',1),
 ('b2',NULL,NULL,0,0,5,3,1,0,42,27,10,'b',1),
 ('b3',NULL,NULL,0,0,5,0,1,4,22,44,1,'b',1),
 ('b4',NULL,NULL,0,0,5,3,2,0,34,19,11,'b',1),
 ('b5',NULL,NULL,0,0,5,0,1,4,22,41,1,'b',1),
 ('b6',NULL,NULL,0,0,5,2,2,1,29,29,8,'b',1),
 ('c1',NULL,NULL,0,0,6,4,1,1,30,28,13,'c',1),
 ('c2',NULL,NULL,0,0,5,2,0,3,25,31,6,'c',1),
 ('c3',NULL,NULL,0,0,5,5,0,0,39,16,15,'c',1),
 ('c4',NULL,NULL,0,0,5,0,0,5,20,46,0,'c',1),
 ('c5',NULL,NULL,0,0,5,3,0,2,35,22,9,'c',1),
 ('c6',NULL,NULL,0,0,6,1,1,4,28,34,4,'c',1);
/*!40000 ALTER TABLE `kc2011_tbl` ENABLE KEYS */;


--
-- Definition of table `kc2011_tms`
--

DROP TABLE IF EXISTS `kc2011_tms`;
CREATE TABLE `kc2011_tms` (
  `TeamID` varchar(3) NOT NULL,
  `TeamName` varchar(30) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_tms`
--

/*!40000 ALTER TABLE `kc2011_tms` DISABLE KEYS */;
INSERT INTO `kc2011_tms` (`TeamID`,`TeamName`) VALUES 
 ('a1','Condom Kickers'),
 ('a2','Mollutz'),
 ('a3','Wolfbräu'),
 ('a4','Porsche'),
 ('a5','Ensinger'),
 ('a6','Sparda Orange'),
 ('b1','Waldmeister'),
 ('b2','Bergmacht'),
 ('b3','Sauffpark'),
 ('b4','Olympiakos'),
 ('b5','Deportivo'),
 ('b6','Sion Kölsch'),
 ('c1','Maggi & Nietzsche'),
 ('c2','Tesa Power'),
 ('c3','Hulaboys'),
 ('c4','Gaffel Kölsch'),
 ('c5','die wilden Kerle'),
 ('c6','Galatasaray');
/*!40000 ALTER TABLE `kc2011_tms` ENABLE KEYS */;


--
-- Definition of table `kc2011_tor`
--

DROP TABLE IF EXISTS `kc2011_tor`;
CREATE TABLE `kc2011_tor` (
  `Spieler` varchar(30) NOT NULL,
  `Team` varchar(30) NOT NULL,
  `Tore` int(10) unsigned NOT NULL,
  PRIMARY KEY (`Spieler`,`Team`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `kc2011_tor`
--

/*!40000 ALTER TABLE `kc2011_tor` DISABLE KEYS */;
INSERT INTO `kc2011_tor` (`Spieler`,`Team`,`Tore`) VALUES 
 ('Andreas K.','Olympiakos',13),
 ('Ben vom Berg','Wolfbräu',15),
 ('Ben vom Wald','Waldmeister',19),
 ('bewave','Mollutz',10),
 ('Captain Doerk','Sion Kölsch',11),
 ('Dave vom Berg','Bergmacht',19),
 ('Der Jupp','Gaffel Kölsch',4),
 ('Dirk L.','Tesa Power',17),
 ('Dolschewida','die wilden Kerle',15),
 ('Emre','Sion Kölsch',21),
 ('Fips','Sparda Orange',16),
 ('Flo vom Berg','Bergmacht',19),
 ('Fraggy','Ensinger',9),
 ('Heif','Deportivo',10),
 ('Inge','Deportivo',14),
 ('Jens L.','Ensinger',8),
 ('Jet-Ski','die wilden Kerle',13),
 ('Luca T.','Porsche',13),
 ('Mad Maz','Porsche',15),
 ('Maggi','Maggi & Nietzsche',26),
 ('Mahony','Sauffpark',11),
 ('Mario vom Wald','Waldmeister',19),
 ('Marvin','Galatasaray',3),
 ('Mirko','Galatasaray',18),
 ('Molli','Mollutz',27),
 ('Nietzsche','Maggi & Nietzsche',3),
 ('Quicky','Condom Kickers',21),
 ('Rio','Gaffel Kölsch',11),
 ('Schlampe','Sparda Orange',9),
 ('Schmiddi','Hulaboys',23),
 ('Shooter','Sauffpark',8),
 ('Shorty','Condom Kickers',17),
 ('Tha Man','Wolfbräu',14),
 ('Theo','Olympiakos',15),
 ('Timo P.','Tesa Power',8),
 ('Wengi','Hulaboys',17);
/*!40000 ALTER TABLE `kc2011_tor` ENABLE KEYS */;


--
-- Definition of table `o12_01_174544_aktive_vs_alte_herren`
--

DROP TABLE IF EXISTS `o12_01_174544_aktive_vs_alte_herren`;
CREATE TABLE `o12_01_174544_aktive_vs_alte_herren` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_01_174544_aktive_vs_alte_herren`
--

/*!40000 ALTER TABLE `o12_01_174544_aktive_vs_alte_herren` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_01_174544_aktive_vs_alte_herren` ENABLE KEYS */;


--
-- Definition of table `o12_01_174721_aktive_vs_alte_herren`
--

DROP TABLE IF EXISTS `o12_01_174721_aktive_vs_alte_herren`;
CREATE TABLE `o12_01_174721_aktive_vs_alte_herren` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_01_174721_aktive_vs_alte_herren`
--

/*!40000 ALTER TABLE `o12_01_174721_aktive_vs_alte_herren` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_01_174721_aktive_vs_alte_herren` ENABLE KEYS */;


--
-- Definition of table `o12_01_181325_aktive_vs_alte_herren`
--

DROP TABLE IF EXISTS `o12_01_181325_aktive_vs_alte_herren`;
CREATE TABLE `o12_01_181325_aktive_vs_alte_herren` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_01_181325_aktive_vs_alte_herren`
--

/*!40000 ALTER TABLE `o12_01_181325_aktive_vs_alte_herren` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_01_181325_aktive_vs_alte_herren` ENABLE KEYS */;


--
-- Definition of table `o12_01_181340_aktive_vs_alte_herren`
--

DROP TABLE IF EXISTS `o12_01_181340_aktive_vs_alte_herren`;
CREATE TABLE `o12_01_181340_aktive_vs_alte_herren` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_01_181340_aktive_vs_alte_herren`
--

/*!40000 ALTER TABLE `o12_01_181340_aktive_vs_alte_herren` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_01_181340_aktive_vs_alte_herren` ENABLE KEYS */;


--
-- Definition of table `o12_02_180215_eltern_vs_green_tigers`
--

DROP TABLE IF EXISTS `o12_02_180215_eltern_vs_green_tigers`;
CREATE TABLE `o12_02_180215_eltern_vs_green_tigers` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_02_180215_eltern_vs_green_tigers`
--

/*!40000 ALTER TABLE `o12_02_180215_eltern_vs_green_tigers` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_02_180215_eltern_vs_green_tigers` ENABLE KEYS */;


--
-- Definition of table `o12_02_181254_eltern_vs_green_tigers`
--

DROP TABLE IF EXISTS `o12_02_181254_eltern_vs_green_tigers`;
CREATE TABLE `o12_02_181254_eltern_vs_green_tigers` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_02_181254_eltern_vs_green_tigers`
--

/*!40000 ALTER TABLE `o12_02_181254_eltern_vs_green_tigers` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_02_181254_eltern_vs_green_tigers` ENABLE KEYS */;


--
-- Definition of table `o12_03_182347_players_vs_schrotties`
--

DROP TABLE IF EXISTS `o12_03_182347_players_vs_schrotties`;
CREATE TABLE `o12_03_182347_players_vs_schrotties` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_03_182347_players_vs_schrotties`
--

/*!40000 ALTER TABLE `o12_03_182347_players_vs_schrotties` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_03_182347_players_vs_schrotties` ENABLE KEYS */;


--
-- Definition of table `o12_04_183254_tennis_vs_trainer`
--

DROP TABLE IF EXISTS `o12_04_183254_tennis_vs_trainer`;
CREATE TABLE `o12_04_183254_tennis_vs_trainer` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_04_183254_tennis_vs_trainer`
--

/*!40000 ALTER TABLE `o12_04_183254_tennis_vs_trainer` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_04_183254_tennis_vs_trainer` ENABLE KEYS */;


--
-- Definition of table `o12_05_184130_aktive_vs_eltern`
--

DROP TABLE IF EXISTS `o12_05_184130_aktive_vs_eltern`;
CREATE TABLE `o12_05_184130_aktive_vs_eltern` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_05_184130_aktive_vs_eltern`
--

/*!40000 ALTER TABLE `o12_05_184130_aktive_vs_eltern` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_05_184130_aktive_vs_eltern` ENABLE KEYS */;


--
-- Definition of table `o12_06_185120_players_vs_alte_herren`
--

DROP TABLE IF EXISTS `o12_06_185120_players_vs_alte_herren`;
CREATE TABLE `o12_06_185120_players_vs_alte_herren` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_06_185120_players_vs_alte_herren`
--

/*!40000 ALTER TABLE `o12_06_185120_players_vs_alte_herren` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_06_185120_players_vs_alte_herren` ENABLE KEYS */;


--
-- Definition of table `o12_07_190017_tennis_vs_green_tigers`
--

DROP TABLE IF EXISTS `o12_07_190017_tennis_vs_green_tigers`;
CREATE TABLE `o12_07_190017_tennis_vs_green_tigers` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_07_190017_tennis_vs_green_tigers`
--

/*!40000 ALTER TABLE `o12_07_190017_tennis_vs_green_tigers` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_07_190017_tennis_vs_green_tigers` ENABLE KEYS */;


--
-- Definition of table `o12_08_190855_trainer_vs_schrotties`
--

DROP TABLE IF EXISTS `o12_08_190855_trainer_vs_schrotties`;
CREATE TABLE `o12_08_190855_trainer_vs_schrotties` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_08_190855_trainer_vs_schrotties`
--

/*!40000 ALTER TABLE `o12_08_190855_trainer_vs_schrotties` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_08_190855_trainer_vs_schrotties` ENABLE KEYS */;


--
-- Definition of table `o12_09_191956_aktive_vs_players`
--

DROP TABLE IF EXISTS `o12_09_191956_aktive_vs_players`;
CREATE TABLE `o12_09_191956_aktive_vs_players` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_09_191956_aktive_vs_players`
--

/*!40000 ALTER TABLE `o12_09_191956_aktive_vs_players` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_09_191956_aktive_vs_players` ENABLE KEYS */;


--
-- Definition of table `o12_10_192835_tennis_vs_eltern`
--

DROP TABLE IF EXISTS `o12_10_192835_tennis_vs_eltern`;
CREATE TABLE `o12_10_192835_tennis_vs_eltern` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_10_192835_tennis_vs_eltern`
--

/*!40000 ALTER TABLE `o12_10_192835_tennis_vs_eltern` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_10_192835_tennis_vs_eltern` ENABLE KEYS */;


--
-- Definition of table `o12_11_193716_trainer_vs_alte_herren`
--

DROP TABLE IF EXISTS `o12_11_193716_trainer_vs_alte_herren`;
CREATE TABLE `o12_11_193716_trainer_vs_alte_herren` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_11_193716_trainer_vs_alte_herren`
--

/*!40000 ALTER TABLE `o12_11_193716_trainer_vs_alte_herren` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_11_193716_trainer_vs_alte_herren` ENABLE KEYS */;


--
-- Definition of table `o12_12_194724_schrotties_vs_green_tigers`
--

DROP TABLE IF EXISTS `o12_12_194724_schrotties_vs_green_tigers`;
CREATE TABLE `o12_12_194724_schrotties_vs_green_tigers` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_12_194724_schrotties_vs_green_tigers`
--

/*!40000 ALTER TABLE `o12_12_194724_schrotties_vs_green_tigers` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_12_194724_schrotties_vs_green_tigers` ENABLE KEYS */;


--
-- Definition of table `o12_13_195545_aktive_vs_tennis`
--

DROP TABLE IF EXISTS `o12_13_195545_aktive_vs_tennis`;
CREATE TABLE `o12_13_195545_aktive_vs_tennis` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_13_195545_aktive_vs_tennis`
--

/*!40000 ALTER TABLE `o12_13_195545_aktive_vs_tennis` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_13_195545_aktive_vs_tennis` ENABLE KEYS */;


--
-- Definition of table `o12_14_200507_trainer_vs_players`
--

DROP TABLE IF EXISTS `o12_14_200507_trainer_vs_players`;
CREATE TABLE `o12_14_200507_trainer_vs_players` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_14_200507_trainer_vs_players`
--

/*!40000 ALTER TABLE `o12_14_200507_trainer_vs_players` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_14_200507_trainer_vs_players` ENABLE KEYS */;


--
-- Definition of table `o12_14_200524_trainer_vs_players`
--

DROP TABLE IF EXISTS `o12_14_200524_trainer_vs_players`;
CREATE TABLE `o12_14_200524_trainer_vs_players` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_14_200524_trainer_vs_players`
--

/*!40000 ALTER TABLE `o12_14_200524_trainer_vs_players` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_14_200524_trainer_vs_players` ENABLE KEYS */;


--
-- Definition of table `o12_15_201447_schrotties_vs_eltern`
--

DROP TABLE IF EXISTS `o12_15_201447_schrotties_vs_eltern`;
CREATE TABLE `o12_15_201447_schrotties_vs_eltern` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_15_201447_schrotties_vs_eltern`
--

/*!40000 ALTER TABLE `o12_15_201447_schrotties_vs_eltern` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_15_201447_schrotties_vs_eltern` ENABLE KEYS */;


--
-- Definition of table `o12_16_202405_green_tigers_vs_alte_herren`
--

DROP TABLE IF EXISTS `o12_16_202405_green_tigers_vs_alte_herren`;
CREATE TABLE `o12_16_202405_green_tigers_vs_alte_herren` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_16_202405_green_tigers_vs_alte_herren`
--

/*!40000 ALTER TABLE `o12_16_202405_green_tigers_vs_alte_herren` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_16_202405_green_tigers_vs_alte_herren` ENABLE KEYS */;


--
-- Definition of table `o12_17_203430_aktive_vs_trainer`
--

DROP TABLE IF EXISTS `o12_17_203430_aktive_vs_trainer`;
CREATE TABLE `o12_17_203430_aktive_vs_trainer` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_17_203430_aktive_vs_trainer`
--

/*!40000 ALTER TABLE `o12_17_203430_aktive_vs_trainer` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_17_203430_aktive_vs_trainer` ENABLE KEYS */;


--
-- Definition of table `o12_18_204355_schrotties_vs_tennis`
--

DROP TABLE IF EXISTS `o12_18_204355_schrotties_vs_tennis`;
CREATE TABLE `o12_18_204355_schrotties_vs_tennis` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_18_204355_schrotties_vs_tennis`
--

/*!40000 ALTER TABLE `o12_18_204355_schrotties_vs_tennis` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_18_204355_schrotties_vs_tennis` ENABLE KEYS */;


--
-- Definition of table `o12_19_205245_green_tigers_vs_players`
--

DROP TABLE IF EXISTS `o12_19_205245_green_tigers_vs_players`;
CREATE TABLE `o12_19_205245_green_tigers_vs_players` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_19_205245_green_tigers_vs_players`
--

/*!40000 ALTER TABLE `o12_19_205245_green_tigers_vs_players` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_19_205245_green_tigers_vs_players` ENABLE KEYS */;


--
-- Definition of table `o12_20_210202_alte_herren_vs_eltern`
--

DROP TABLE IF EXISTS `o12_20_210202_alte_herren_vs_eltern`;
CREATE TABLE `o12_20_210202_alte_herren_vs_eltern` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_20_210202_alte_herren_vs_eltern`
--

/*!40000 ALTER TABLE `o12_20_210202_alte_herren_vs_eltern` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_20_210202_alte_herren_vs_eltern` ENABLE KEYS */;


--
-- Definition of table `o12_21_211129_aktive_vs_schrotties`
--

DROP TABLE IF EXISTS `o12_21_211129_aktive_vs_schrotties`;
CREATE TABLE `o12_21_211129_aktive_vs_schrotties` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_21_211129_aktive_vs_schrotties`
--

/*!40000 ALTER TABLE `o12_21_211129_aktive_vs_schrotties` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_21_211129_aktive_vs_schrotties` ENABLE KEYS */;


--
-- Definition of table `o12_22_212350_green_tigers_vs_trainer`
--

DROP TABLE IF EXISTS `o12_22_212350_green_tigers_vs_trainer`;
CREATE TABLE `o12_22_212350_green_tigers_vs_trainer` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_22_212350_green_tigers_vs_trainer`
--

/*!40000 ALTER TABLE `o12_22_212350_green_tigers_vs_trainer` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_22_212350_green_tigers_vs_trainer` ENABLE KEYS */;


--
-- Definition of table `o12_22_212402_green_tigers_vs_trainer`
--

DROP TABLE IF EXISTS `o12_22_212402_green_tigers_vs_trainer`;
CREATE TABLE `o12_22_212402_green_tigers_vs_trainer` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_22_212402_green_tigers_vs_trainer`
--

/*!40000 ALTER TABLE `o12_22_212402_green_tigers_vs_trainer` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_22_212402_green_tigers_vs_trainer` ENABLE KEYS */;


--
-- Definition of table `o12_23_213256_alte_herren_vs_tennis`
--

DROP TABLE IF EXISTS `o12_23_213256_alte_herren_vs_tennis`;
CREATE TABLE `o12_23_213256_alte_herren_vs_tennis` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_23_213256_alte_herren_vs_tennis`
--

/*!40000 ALTER TABLE `o12_23_213256_alte_herren_vs_tennis` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_23_213256_alte_herren_vs_tennis` ENABLE KEYS */;


--
-- Definition of table `o12_24_214225_eltern_vs_players`
--

DROP TABLE IF EXISTS `o12_24_214225_eltern_vs_players`;
CREATE TABLE `o12_24_214225_eltern_vs_players` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_24_214225_eltern_vs_players`
--

/*!40000 ALTER TABLE `o12_24_214225_eltern_vs_players` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_24_214225_eltern_vs_players` ENABLE KEYS */;


--
-- Definition of table `o12_25_215120_aktive_vs_green_tigers`
--

DROP TABLE IF EXISTS `o12_25_215120_aktive_vs_green_tigers`;
CREATE TABLE `o12_25_215120_aktive_vs_green_tigers` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_25_215120_aktive_vs_green_tigers`
--

/*!40000 ALTER TABLE `o12_25_215120_aktive_vs_green_tigers` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_25_215120_aktive_vs_green_tigers` ENABLE KEYS */;


--
-- Definition of table `o12_25_215234_aktive_vs_green_tigers`
--

DROP TABLE IF EXISTS `o12_25_215234_aktive_vs_green_tigers`;
CREATE TABLE `o12_25_215234_aktive_vs_green_tigers` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_25_215234_aktive_vs_green_tigers`
--

/*!40000 ALTER TABLE `o12_25_215234_aktive_vs_green_tigers` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_25_215234_aktive_vs_green_tigers` ENABLE KEYS */;


--
-- Definition of table `o12_26_220208_alte_herren_vs_schrotties`
--

DROP TABLE IF EXISTS `o12_26_220208_alte_herren_vs_schrotties`;
CREATE TABLE `o12_26_220208_alte_herren_vs_schrotties` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_26_220208_alte_herren_vs_schrotties`
--

/*!40000 ALTER TABLE `o12_26_220208_alte_herren_vs_schrotties` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_26_220208_alte_herren_vs_schrotties` ENABLE KEYS */;


--
-- Definition of table `o12_27_221025_eltern_vs_trainer`
--

DROP TABLE IF EXISTS `o12_27_221025_eltern_vs_trainer`;
CREATE TABLE `o12_27_221025_eltern_vs_trainer` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_27_221025_eltern_vs_trainer`
--

/*!40000 ALTER TABLE `o12_27_221025_eltern_vs_trainer` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_27_221025_eltern_vs_trainer` ENABLE KEYS */;


--
-- Definition of table `o12_27_221041_eltern_vs_trainer`
--

DROP TABLE IF EXISTS `o12_27_221041_eltern_vs_trainer`;
CREATE TABLE `o12_27_221041_eltern_vs_trainer` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_27_221041_eltern_vs_trainer`
--

/*!40000 ALTER TABLE `o12_27_221041_eltern_vs_trainer` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_27_221041_eltern_vs_trainer` ENABLE KEYS */;


--
-- Definition of table `o12_28_222140_players_vs_tennis`
--

DROP TABLE IF EXISTS `o12_28_222140_players_vs_tennis`;
CREATE TABLE `o12_28_222140_players_vs_tennis` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_28_222140_players_vs_tennis`
--

/*!40000 ALTER TABLE `o12_28_222140_players_vs_tennis` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_28_222140_players_vs_tennis` ENABLE KEYS */;


--
-- Definition of table `o12_spl`
--

DROP TABLE IF EXISTS `o12_spl`;
CREATE TABLE `o12_spl` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Tore1` int(10) unsigned DEFAULT NULL,
  `Tore2` int(10) unsigned DEFAULT NULL,
  `X` int(10) unsigned NOT NULL DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL,
  `Team1Name` varchar(30) DEFAULT NULL,
  `Team2Name` varchar(30) DEFAULT NULL,
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=29 DEFAULT CHARSET=utf8 PACK_KEYS=1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `o12_spl`
--

/*!40000 ALTER TABLE `o12_spl` DISABLE KEYS */;
INSERT INTO `o12_spl` (`SpielID`,`Team1`,`Team2`,`Tore1`,`Tore2`,`X`,`Gruppe`,`Team1Name`,`Team2Name`) VALUES 
 (1,'a1','a2',0,0,8,'a',NULL,NULL),
 (2,'a3','a4',1,5,8,'a',NULL,NULL),
 (3,'a5','a6',2,0,8,'a',NULL,NULL),
 (4,'a7','a8',3,0,8,'a',NULL,NULL),
 (5,'a1','a3',4,0,8,'a',NULL,NULL),
 (6,'a5','a2',0,0,8,'a',NULL,NULL),
 (7,'a7','a4',0,2,8,'a',NULL,NULL),
 (8,'a8','a6',0,1,8,'a',NULL,NULL),
 (9,'a1','a5',1,3,8,'a',NULL,NULL),
 (10,'a7','a3',2,0,8,'a',NULL,NULL),
 (11,'a8','a2',0,0,8,'a',NULL,NULL),
 (12,'a6','a4',1,3,8,'a',NULL,NULL),
 (13,'a1','a7',0,2,8,'a',NULL,NULL),
 (14,'a8','a5',1,3,8,'a',NULL,NULL),
 (15,'a6','a3',4,1,8,'a',NULL,NULL),
 (16,'a4','a2',3,0,8,'a',NULL,NULL),
 (17,'a1','a8',3,0,8,'a',NULL,NULL),
 (18,'a6','a7',1,2,8,'a',NULL,NULL),
 (19,'a4','a5',4,0,8,'a',NULL,NULL),
 (20,'a2','a3',4,0,8,'a',NULL,NULL),
 (21,'a1','a6',1,2,8,'a',NULL,NULL),
 (22,'a4','a8',2,0,8,'a',NULL,NULL),
 (23,'a2','a7',1,1,8,'a',NULL,NULL),
 (24,'a3','a5',2,2,8,'a',NULL,NULL),
 (25,'a1','a4',1,0,8,'a',NULL,NULL),
 (26,'a2','a6',1,0,8,'a',NULL,NULL),
 (27,'a3','a8',0,3,8,'a',NULL,NULL),
 (28,'a5','a7',4,0,8,'a',NULL,NULL);
/*!40000 ALTER TABLE `o12_spl` ENABLE KEYS */;


--
-- Definition of table `o12_tbl`
--

DROP TABLE IF EXISTS `o12_tbl`;
CREATE TABLE `o12_tbl` (
  `TeamID` varchar(3) NOT NULL,
  `Spieler1` int(3) DEFAULT NULL,
  `Spieler2` int(3) DEFAULT NULL,
  `Spieler3` int(3) unsigned DEFAULT '0',
  `Spieler4` int(3) unsigned DEFAULT '0',
  `Spiele` int(3) unsigned DEFAULT '0',
  `S` int(3) unsigned DEFAULT '0',
  `U` int(3) unsigned DEFAULT '0',
  `N` int(3) unsigned DEFAULT '0',
  `ToreP` int(4) unsigned DEFAULT '0',
  `ToreM` int(4) unsigned DEFAULT '0',
  `Punkte` int(3) unsigned DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL DEFAULT 'a',
  `Status` int(3) unsigned DEFAULT '1',
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `o12_tbl`
--

/*!40000 ALTER TABLE `o12_tbl` DISABLE KEYS */;
INSERT INTO `o12_tbl` (`TeamID`,`Spieler1`,`Spieler2`,`Spieler3`,`Spieler4`,`Spiele`,`S`,`U`,`N`,`ToreP`,`ToreM`,`Punkte`,`Gruppe`,`Status`) VALUES 
 ('a1',NULL,NULL,0,0,7,3,1,3,10,7,10,'a',1),
 ('a2',NULL,NULL,0,0,7,2,4,1,6,4,10,'a',1),
 ('a3',NULL,NULL,0,0,7,0,1,6,4,24,1,'a',1),
 ('a4',NULL,NULL,0,0,7,6,0,1,19,3,18,'a',1),
 ('a5',NULL,NULL,0,0,7,4,2,1,14,8,14,'a',1),
 ('a6',NULL,NULL,0,0,7,3,0,4,9,10,7,'a',1),
 ('a7',NULL,NULL,0,0,7,4,1,2,10,8,13,'a',1),
 ('a8',NULL,NULL,0,0,7,1,1,5,4,12,5,'a',1);
/*!40000 ALTER TABLE `o12_tbl` ENABLE KEYS */;


--
-- Definition of table `o12_tms`
--

DROP TABLE IF EXISTS `o12_tms`;
CREATE TABLE `o12_tms` (
  `TeamID` varchar(3) NOT NULL,
  `TeamName` varchar(30) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_tms`
--

/*!40000 ALTER TABLE `o12_tms` DISABLE KEYS */;
INSERT INTO `o12_tms` (`TeamID`,`TeamName`) VALUES 
 ('a1','Aktive'),
 ('a2','Alte Herren'),
 ('a3','Eltern'),
 ('a4','Green Tigers'),
 ('a5','Players'),
 ('a6','Schrotties'),
 ('a7','Tennis'),
 ('a8','Trainer');
/*!40000 ALTER TABLE `o12_tms` ENABLE KEYS */;


--
-- Definition of table `o12_tor`
--

DROP TABLE IF EXISTS `o12_tor`;
CREATE TABLE `o12_tor` (
  `Spieler` varchar(30) NOT NULL,
  `Team` varchar(30) NOT NULL,
  `Tore` int(10) unsigned NOT NULL,
  PRIMARY KEY (`Spieler`,`Team`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `o12_tor`
--

/*!40000 ALTER TABLE `o12_tor` DISABLE KEYS */;
/*!40000 ALTER TABLE `o12_tor` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_01_150909_green_tigers_vs_players`
--

DROP TABLE IF EXISTS `olympiade2011_01_150909_green_tigers_vs_players`;
CREATE TABLE `olympiade2011_01_150909_green_tigers_vs_players` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_01_150909_green_tigers_vs_players`
--

/*!40000 ALTER TABLE `olympiade2011_01_150909_green_tigers_vs_players` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_01_150909_green_tigers_vs_players` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_01_175304_green_tigers_vs_players`
--

DROP TABLE IF EXISTS `olympiade2011_01_175304_green_tigers_vs_players`;
CREATE TABLE `olympiade2011_01_175304_green_tigers_vs_players` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_01_175304_green_tigers_vs_players`
--

/*!40000 ALTER TABLE `olympiade2011_01_175304_green_tigers_vs_players` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_01_175304_green_tigers_vs_players` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_02_180916_eselsberg_vs_aktive`
--

DROP TABLE IF EXISTS `olympiade2011_02_180916_eselsberg_vs_aktive`;
CREATE TABLE `olympiade2011_02_180916_eselsberg_vs_aktive` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_02_180916_eselsberg_vs_aktive`
--

/*!40000 ALTER TABLE `olympiade2011_02_180916_eselsberg_vs_aktive` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_02_180916_eselsberg_vs_aktive` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_03_182041_tennis_vs_volleyball`
--

DROP TABLE IF EXISTS `olympiade2011_03_182041_tennis_vs_volleyball`;
CREATE TABLE `olympiade2011_03_182041_tennis_vs_volleyball` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_03_182041_tennis_vs_volleyball`
--

/*!40000 ALTER TABLE `olympiade2011_03_182041_tennis_vs_volleyball` DISABLE KEYS */;
INSERT INTO `olympiade2011_03_182041_tennis_vs_volleyball` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'06:07',1,0,'Tennis','Harry',NULL);
/*!40000 ALTER TABLE `olympiade2011_03_182041_tennis_vs_volleyball` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_04_183156_rugby_vs_die_pfeifen`
--

DROP TABLE IF EXISTS `olympiade2011_04_183156_rugby_vs_die_pfeifen`;
CREATE TABLE `olympiade2011_04_183156_rugby_vs_die_pfeifen` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_04_183156_rugby_vs_die_pfeifen`
--

/*!40000 ALTER TABLE `olympiade2011_04_183156_rugby_vs_die_pfeifen` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_04_183156_rugby_vs_die_pfeifen` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_05_184318_lauchhausen_vs_green_tigers`
--

DROP TABLE IF EXISTS `olympiade2011_05_184318_lauchhausen_vs_green_tigers`;
CREATE TABLE `olympiade2011_05_184318_lauchhausen_vs_green_tigers` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_05_184318_lauchhausen_vs_green_tigers`
--

/*!40000 ALTER TABLE `olympiade2011_05_184318_lauchhausen_vs_green_tigers` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_05_184318_lauchhausen_vs_green_tigers` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_06_185448_players_vs_eselsberg`
--

DROP TABLE IF EXISTS `olympiade2011_06_185448_players_vs_eselsberg`;
CREATE TABLE `olympiade2011_06_185448_players_vs_eselsberg` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_06_185448_players_vs_eselsberg`
--

/*!40000 ALTER TABLE `olympiade2011_06_185448_players_vs_eselsberg` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_06_185448_players_vs_eselsberg` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_07_190548_alte_herren_vs_tennis`
--

DROP TABLE IF EXISTS `olympiade2011_07_190548_alte_herren_vs_tennis`;
CREATE TABLE `olympiade2011_07_190548_alte_herren_vs_tennis` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_07_190548_alte_herren_vs_tennis`
--

/*!40000 ALTER TABLE `olympiade2011_07_190548_alte_herren_vs_tennis` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_07_190548_alte_herren_vs_tennis` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_08_191821_volleyball_vs_rugby`
--

DROP TABLE IF EXISTS `olympiade2011_08_191821_volleyball_vs_rugby`;
CREATE TABLE `olympiade2011_08_191821_volleyball_vs_rugby` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_08_191821_volleyball_vs_rugby`
--

/*!40000 ALTER TABLE `olympiade2011_08_191821_volleyball_vs_rugby` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_08_191821_volleyball_vs_rugby` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_09_192845_aktive_vs_lauchhausen`
--

DROP TABLE IF EXISTS `olympiade2011_09_192845_aktive_vs_lauchhausen`;
CREATE TABLE `olympiade2011_09_192845_aktive_vs_lauchhausen` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_09_192845_aktive_vs_lauchhausen`
--

/*!40000 ALTER TABLE `olympiade2011_09_192845_aktive_vs_lauchhausen` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_09_192845_aktive_vs_lauchhausen` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_10_194032_green_tigers_vs_eselsberg`
--

DROP TABLE IF EXISTS `olympiade2011_10_194032_green_tigers_vs_eselsberg`;
CREATE TABLE `olympiade2011_10_194032_green_tigers_vs_eselsberg` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_10_194032_green_tigers_vs_eselsberg`
--

/*!40000 ALTER TABLE `olympiade2011_10_194032_green_tigers_vs_eselsberg` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_10_194032_green_tigers_vs_eselsberg` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_11_195114_die_pfeifen_vs_alte_herren`
--

DROP TABLE IF EXISTS `olympiade2011_11_195114_die_pfeifen_vs_alte_herren`;
CREATE TABLE `olympiade2011_11_195114_die_pfeifen_vs_alte_herren` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_11_195114_die_pfeifen_vs_alte_herren`
--

/*!40000 ALTER TABLE `olympiade2011_11_195114_die_pfeifen_vs_alte_herren` DISABLE KEYS */;
INSERT INTO `olympiade2011_11_195114_die_pfeifen_vs_alte_herren` (`ID`,`SpielZeit`,`ToreA`,`ToreB`,`Team`,`Spieler`,`Event`) VALUES 
 (1,'03:20',1,1,'Alte Herren','Hebbe',NULL);
/*!40000 ALTER TABLE `olympiade2011_11_195114_die_pfeifen_vs_alte_herren` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_12_200206_tennis_vs_rugby`
--

DROP TABLE IF EXISTS `olympiade2011_12_200206_tennis_vs_rugby`;
CREATE TABLE `olympiade2011_12_200206_tennis_vs_rugby` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_12_200206_tennis_vs_rugby`
--

/*!40000 ALTER TABLE `olympiade2011_12_200206_tennis_vs_rugby` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_12_200206_tennis_vs_rugby` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_13_201409_lauchhausen_vs_players`
--

DROP TABLE IF EXISTS `olympiade2011_13_201409_lauchhausen_vs_players`;
CREATE TABLE `olympiade2011_13_201409_lauchhausen_vs_players` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_13_201409_lauchhausen_vs_players`
--

/*!40000 ALTER TABLE `olympiade2011_13_201409_lauchhausen_vs_players` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_13_201409_lauchhausen_vs_players` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_14_202513_aktive_vs_green_tigers`
--

DROP TABLE IF EXISTS `olympiade2011_14_202513_aktive_vs_green_tigers`;
CREATE TABLE `olympiade2011_14_202513_aktive_vs_green_tigers` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_14_202513_aktive_vs_green_tigers`
--

/*!40000 ALTER TABLE `olympiade2011_14_202513_aktive_vs_green_tigers` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_14_202513_aktive_vs_green_tigers` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_15_203638_alte_herren_vs_volleyball`
--

DROP TABLE IF EXISTS `olympiade2011_15_203638_alte_herren_vs_volleyball`;
CREATE TABLE `olympiade2011_15_203638_alte_herren_vs_volleyball` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_15_203638_alte_herren_vs_volleyball`
--

/*!40000 ALTER TABLE `olympiade2011_15_203638_alte_herren_vs_volleyball` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_15_203638_alte_herren_vs_volleyball` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_16_204804_die_pfeifen_vs_tennis`
--

DROP TABLE IF EXISTS `olympiade2011_16_204804_die_pfeifen_vs_tennis`;
CREATE TABLE `olympiade2011_16_204804_die_pfeifen_vs_tennis` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_16_204804_die_pfeifen_vs_tennis`
--

/*!40000 ALTER TABLE `olympiade2011_16_204804_die_pfeifen_vs_tennis` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_16_204804_die_pfeifen_vs_tennis` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_17_205900_eselsberg_vs_lauchhausen`
--

DROP TABLE IF EXISTS `olympiade2011_17_205900_eselsberg_vs_lauchhausen`;
CREATE TABLE `olympiade2011_17_205900_eselsberg_vs_lauchhausen` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_17_205900_eselsberg_vs_lauchhausen`
--

/*!40000 ALTER TABLE `olympiade2011_17_205900_eselsberg_vs_lauchhausen` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_17_205900_eselsberg_vs_lauchhausen` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_18_211416_players_vs_aktive`
--

DROP TABLE IF EXISTS `olympiade2011_18_211416_players_vs_aktive`;
CREATE TABLE `olympiade2011_18_211416_players_vs_aktive` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_18_211416_players_vs_aktive`
--

/*!40000 ALTER TABLE `olympiade2011_18_211416_players_vs_aktive` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_18_211416_players_vs_aktive` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_19_212526_rugby_vs_alte_herren`
--

DROP TABLE IF EXISTS `olympiade2011_19_212526_rugby_vs_alte_herren`;
CREATE TABLE `olympiade2011_19_212526_rugby_vs_alte_herren` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_19_212526_rugby_vs_alte_herren`
--

/*!40000 ALTER TABLE `olympiade2011_19_212526_rugby_vs_alte_herren` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_19_212526_rugby_vs_alte_herren` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_20_213655_volleyball_vs_die_pfeifen`
--

DROP TABLE IF EXISTS `olympiade2011_20_213655_volleyball_vs_die_pfeifen`;
CREATE TABLE `olympiade2011_20_213655_volleyball_vs_die_pfeifen` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_20_213655_volleyball_vs_die_pfeifen`
--

/*!40000 ALTER TABLE `olympiade2011_20_213655_volleyball_vs_die_pfeifen` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_20_213655_volleyball_vs_die_pfeifen` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_21_214904_teama_vs_teamb`
--

DROP TABLE IF EXISTS `olympiade2011_21_214904_teama_vs_teamb`;
CREATE TABLE `olympiade2011_21_214904_teama_vs_teamb` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_21_214904_teama_vs_teamb`
--

/*!40000 ALTER TABLE `olympiade2011_21_214904_teama_vs_teamb` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_21_214904_teama_vs_teamb` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_22_220028_teama_vs_teamb`
--

DROP TABLE IF EXISTS `olympiade2011_22_220028_teama_vs_teamb`;
CREATE TABLE `olympiade2011_22_220028_teama_vs_teamb` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_22_220028_teama_vs_teamb`
--

/*!40000 ALTER TABLE `olympiade2011_22_220028_teama_vs_teamb` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_22_220028_teama_vs_teamb` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_23_221134_teama_vs_teamb`
--

DROP TABLE IF EXISTS `olympiade2011_23_221134_teama_vs_teamb`;
CREATE TABLE `olympiade2011_23_221134_teama_vs_teamb` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_23_221134_teama_vs_teamb`
--

/*!40000 ALTER TABLE `olympiade2011_23_221134_teama_vs_teamb` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_23_221134_teama_vs_teamb` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_24_222426_teama_vs_teamb`
--

DROP TABLE IF EXISTS `olympiade2011_24_222426_teama_vs_teamb`;
CREATE TABLE `olympiade2011_24_222426_teama_vs_teamb` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_24_222426_teama_vs_teamb`
--

/*!40000 ALTER TABLE `olympiade2011_24_222426_teama_vs_teamb` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_24_222426_teama_vs_teamb` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_25_223534_teama_vs_teamb`
--

DROP TABLE IF EXISTS `olympiade2011_25_223534_teama_vs_teamb`;
CREATE TABLE `olympiade2011_25_223534_teama_vs_teamb` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_25_223534_teama_vs_teamb`
--

/*!40000 ALTER TABLE `olympiade2011_25_223534_teama_vs_teamb` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_25_223534_teama_vs_teamb` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_26_225237_teama_vs_teamb`
--

DROP TABLE IF EXISTS `olympiade2011_26_225237_teama_vs_teamb`;
CREATE TABLE `olympiade2011_26_225237_teama_vs_teamb` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_26_225237_teama_vs_teamb`
--

/*!40000 ALTER TABLE `olympiade2011_26_225237_teama_vs_teamb` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2011_26_225237_teama_vs_teamb` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_spl`
--

DROP TABLE IF EXISTS `olympiade2011_spl`;
CREATE TABLE `olympiade2011_spl` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Tore1` int(10) unsigned DEFAULT NULL,
  `Tore2` int(10) unsigned DEFAULT NULL,
  `X` int(10) unsigned NOT NULL DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL,
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=28 DEFAULT CHARSET=utf8 PACK_KEYS=1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `olympiade2011_spl`
--

/*!40000 ALTER TABLE `olympiade2011_spl` DISABLE KEYS */;
INSERT INTO `olympiade2011_spl` (`SpielID`,`Team1`,`Team2`,`Tore1`,`Tore2`,`X`,`Gruppe`) VALUES 
 (1,'a1','a2',2,0,8,'a'),
 (2,'a3','a4',0,3,8,'a'),
 (3,'b1','b2',2,2,8,'b'),
 (4,'b3','b4',0,6,8,'b'),
 (5,'a5','a1',1,1,8,'a'),
 (6,'a2','a3',4,2,8,'a'),
 (7,'b5','b1',1,0,8,'b'),
 (8,'b2','b3',2,0,8,'b'),
 (9,'a4','a5',2,0,8,'a'),
 (10,'a1','a3',1,1,8,'a'),
 (11,'b4','b5',3,1,8,'b'),
 (12,'b1','b3',4,2,8,'b'),
 (13,'a5','a2',4,1,8,'a'),
 (14,'a4','a1',1,0,8,'a'),
 (15,'b5','b2',2,1,8,'b'),
 (16,'b4','b1',3,1,8,'b'),
 (17,'a3','a5',0,3,8,'a'),
 (18,'a2','a4',2,4,8,'a'),
 (19,'b3','b5',0,1,8,'b'),
 (20,'b2','b4',0,0,8,'b'),
 (21,'f1','f2',2,3,8,'z'),
 (22,'f1','f2',0,3,8,'z'),
 (23,'f1','f2',2,1,8,'z'),
 (24,'f1','f2',1,3,8,'z'),
 (25,'f1','f2',2,2,8,'z'),
 (26,'f1','f2',NULL,NULL,0,'z'),
 (27,'f1','f2',NULL,NULL,0,'z');
/*!40000 ALTER TABLE `olympiade2011_spl` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_tbl`
--

DROP TABLE IF EXISTS `olympiade2011_tbl`;
CREATE TABLE `olympiade2011_tbl` (
  `TeamID` int(3) unsigned NOT NULL,
  `Spieler1` int(3) DEFAULT NULL,
  `Spieler2` int(3) DEFAULT NULL,
  `Spieler3` int(3) unsigned DEFAULT '0',
  `Spieler4` int(3) unsigned DEFAULT '0',
  `Spiele` int(3) unsigned DEFAULT '0',
  `S` int(3) unsigned DEFAULT '0',
  `U` int(3) unsigned DEFAULT '0',
  `N` int(3) unsigned DEFAULT '0',
  `ToreP` int(4) unsigned DEFAULT '0',
  `ToreM` int(4) unsigned DEFAULT '0',
  `Punkte` int(3) unsigned DEFAULT '0',
  `Gruppe` varchar(3) DEFAULT NULL,
  `Status` int(3) unsigned DEFAULT '1',
  `Team1` varchar(45) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `olympiade2011_tbl`
--

/*!40000 ALTER TABLE `olympiade2011_tbl` DISABLE KEYS */;
INSERT INTO `olympiade2011_tbl` (`TeamID`,`Spieler1`,`Spieler2`,`Spieler3`,`Spieler4`,`Spiele`,`S`,`U`,`N`,`ToreP`,`ToreM`,`Punkte`,`Gruppe`,`Status`,`Team1`) VALUES 
 (1,NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1,'a1'),
 (2,NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1,'a2'),
 (3,NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1,'a3'),
 (4,NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1,'a4'),
 (5,NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1,'a5'),
 (6,NULL,NULL,0,0,0,0,0,0,0,0,0,'b',1,'b1'),
 (7,NULL,NULL,0,0,0,0,0,0,0,0,0,'b',1,'b2'),
 (8,NULL,NULL,0,0,0,0,0,0,0,0,0,'b',1,'b3'),
 (9,NULL,NULL,0,0,0,0,0,0,0,0,0,'b',1,'b4'),
 (10,NULL,NULL,0,0,0,0,0,0,0,0,0,'b',1,'b5');
/*!40000 ALTER TABLE `olympiade2011_tbl` ENABLE KEYS */;


--
-- Definition of table `olympiade2011_tor`
--

DROP TABLE IF EXISTS `olympiade2011_tor`;
CREATE TABLE `olympiade2011_tor` (
  `Spieler` varchar(30) NOT NULL,
  `Team` varchar(30) NOT NULL,
  `Tore` int(10) unsigned NOT NULL,
  PRIMARY KEY (`Spieler`,`Team`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2011_tor`
--

/*!40000 ALTER TABLE `olympiade2011_tor` DISABLE KEYS */;
INSERT INTO `olympiade2011_tor` (`Spieler`,`Team`,`Tore`) VALUES 
 ('Harry','Tennis',1),
 ('Hebbe','Alte Herren',1);
/*!40000 ALTER TABLE `olympiade2011_tor` ENABLE KEYS */;


--
-- Definition of table `olympiade2012_01_095153_alte_herren_vs_eltern`
--

DROP TABLE IF EXISTS `olympiade2012_01_095153_alte_herren_vs_eltern`;
CREATE TABLE `olympiade2012_01_095153_alte_herren_vs_eltern` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2012_01_095153_alte_herren_vs_eltern`
--

/*!40000 ALTER TABLE `olympiade2012_01_095153_alte_herren_vs_eltern` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2012_01_095153_alte_herren_vs_eltern` ENABLE KEYS */;


--
-- Definition of table `olympiade2012_01_095616_alte_herren_vs_eltern`
--

DROP TABLE IF EXISTS `olympiade2012_01_095616_alte_herren_vs_eltern`;
CREATE TABLE `olympiade2012_01_095616_alte_herren_vs_eltern` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2012_01_095616_alte_herren_vs_eltern`
--

/*!40000 ALTER TABLE `olympiade2012_01_095616_alte_herren_vs_eltern` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2012_01_095616_alte_herren_vs_eltern` ENABLE KEYS */;


--
-- Definition of table `olympiade2012_01_163700_alte_herren_vs_eltern`
--

DROP TABLE IF EXISTS `olympiade2012_01_163700_alte_herren_vs_eltern`;
CREATE TABLE `olympiade2012_01_163700_alte_herren_vs_eltern` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2012_01_163700_alte_herren_vs_eltern`
--

/*!40000 ALTER TABLE `olympiade2012_01_163700_alte_herren_vs_eltern` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2012_01_163700_alte_herren_vs_eltern` ENABLE KEYS */;


--
-- Definition of table `olympiade2012_01_173224_alte_herren_vs_eltern`
--

DROP TABLE IF EXISTS `olympiade2012_01_173224_alte_herren_vs_eltern`;
CREATE TABLE `olympiade2012_01_173224_alte_herren_vs_eltern` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2012_01_173224_alte_herren_vs_eltern`
--

/*!40000 ALTER TABLE `olympiade2012_01_173224_alte_herren_vs_eltern` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2012_01_173224_alte_herren_vs_eltern` ENABLE KEYS */;


--
-- Definition of table `olympiade2012_01_173435_alte_herren_vs_eltern`
--

DROP TABLE IF EXISTS `olympiade2012_01_173435_alte_herren_vs_eltern`;
CREATE TABLE `olympiade2012_01_173435_alte_herren_vs_eltern` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2012_01_173435_alte_herren_vs_eltern`
--

/*!40000 ALTER TABLE `olympiade2012_01_173435_alte_herren_vs_eltern` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2012_01_173435_alte_herren_vs_eltern` ENABLE KEYS */;


--
-- Definition of table `olympiade2012_02_173329_green_tigers_vs_players`
--

DROP TABLE IF EXISTS `olympiade2012_02_173329_green_tigers_vs_players`;
CREATE TABLE `olympiade2012_02_173329_green_tigers_vs_players` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2012_02_173329_green_tigers_vs_players`
--

/*!40000 ALTER TABLE `olympiade2012_02_173329_green_tigers_vs_players` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2012_02_173329_green_tigers_vs_players` ENABLE KEYS */;


--
-- Definition of table `olympiade2012_03_173431_schrotties_vs_tennis`
--

DROP TABLE IF EXISTS `olympiade2012_03_173431_schrotties_vs_tennis`;
CREATE TABLE `olympiade2012_03_173431_schrotties_vs_tennis` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2012_03_173431_schrotties_vs_tennis`
--

/*!40000 ALTER TABLE `olympiade2012_03_173431_schrotties_vs_tennis` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2012_03_173431_schrotties_vs_tennis` ENABLE KEYS */;


--
-- Definition of table `olympiade2012_spl`
--

DROP TABLE IF EXISTS `olympiade2012_spl`;
CREATE TABLE `olympiade2012_spl` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Tore1` int(10) unsigned DEFAULT NULL,
  `Tore2` int(10) unsigned DEFAULT NULL,
  `X` int(10) unsigned NOT NULL DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL,
  `Team1Name` varchar(30) DEFAULT NULL,
  `Team2Name` varchar(30) DEFAULT NULL,
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=22 DEFAULT CHARSET=utf8 PACK_KEYS=1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `olympiade2012_spl`
--

/*!40000 ALTER TABLE `olympiade2012_spl` DISABLE KEYS */;
INSERT INTO `olympiade2012_spl` (`SpielID`,`Team1`,`Team2`,`Tore1`,`Tore2`,`X`,`Gruppe`,`Team1Name`,`Team2Name`) VALUES 
 (1,'a1','a2',NULL,NULL,0,'a',NULL,NULL),
 (2,'a3','a4',NULL,NULL,0,'a',NULL,NULL),
 (3,'a5','a6',NULL,NULL,0,'a',NULL,NULL),
 (4,'a7','a1',NULL,NULL,0,'a',NULL,NULL),
 (5,'a2','a3',NULL,NULL,0,'a',NULL,NULL),
 (6,'a4','a5',NULL,NULL,0,'a',NULL,NULL),
 (7,'a6','a7',NULL,NULL,0,'a',NULL,NULL),
 (8,'a1','a3',NULL,NULL,0,'a',NULL,NULL),
 (9,'a5','a2',NULL,NULL,0,'a',NULL,NULL),
 (10,'a7','a4',NULL,NULL,0,'a',NULL,NULL),
 (11,'a6','a1',NULL,NULL,0,'a',NULL,NULL),
 (12,'a3','a5',NULL,NULL,0,'a',NULL,NULL),
 (13,'a2','a7',NULL,NULL,0,'a',NULL,NULL),
 (14,'a4','a6',NULL,NULL,0,'a',NULL,NULL),
 (15,'a1','a5',NULL,NULL,0,'a',NULL,NULL),
 (16,'a7','a3',NULL,NULL,0,'a',NULL,NULL),
 (17,'a6','a2',NULL,NULL,0,'a',NULL,NULL),
 (18,'a4','a1',NULL,NULL,0,'a',NULL,NULL),
 (19,'a5','a7',NULL,NULL,0,'a',NULL,NULL),
 (20,'a3','a6',NULL,NULL,0,'a',NULL,NULL),
 (21,'a2','a4',NULL,NULL,0,'a',NULL,NULL);
/*!40000 ALTER TABLE `olympiade2012_spl` ENABLE KEYS */;


--
-- Definition of table `olympiade2012_tbl`
--

DROP TABLE IF EXISTS `olympiade2012_tbl`;
CREATE TABLE `olympiade2012_tbl` (
  `TeamID` varchar(3) NOT NULL,
  `Spieler1` int(3) DEFAULT NULL,
  `Spieler2` int(3) DEFAULT NULL,
  `Spieler3` int(3) unsigned DEFAULT '0',
  `Spieler4` int(3) unsigned DEFAULT '0',
  `Spiele` int(3) unsigned DEFAULT '0',
  `S` int(3) unsigned DEFAULT '0',
  `U` int(3) unsigned DEFAULT '0',
  `N` int(3) unsigned DEFAULT '0',
  `ToreP` int(4) unsigned DEFAULT '0',
  `ToreM` int(4) unsigned DEFAULT '0',
  `Punkte` int(3) unsigned DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL DEFAULT 'a',
  `Status` int(3) unsigned DEFAULT '1',
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `olympiade2012_tbl`
--

/*!40000 ALTER TABLE `olympiade2012_tbl` DISABLE KEYS */;
INSERT INTO `olympiade2012_tbl` (`TeamID`,`Spieler1`,`Spieler2`,`Spieler3`,`Spieler4`,`Spiele`,`S`,`U`,`N`,`ToreP`,`ToreM`,`Punkte`,`Gruppe`,`Status`) VALUES 
 ('a1',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a2',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a3',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a4',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a5',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a6',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a7',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1);
/*!40000 ALTER TABLE `olympiade2012_tbl` ENABLE KEYS */;


--
-- Definition of table `olympiade2012_tms`
--

DROP TABLE IF EXISTS `olympiade2012_tms`;
CREATE TABLE `olympiade2012_tms` (
  `TeamID` varchar(3) NOT NULL,
  `TeamName` varchar(30) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2012_tms`
--

/*!40000 ALTER TABLE `olympiade2012_tms` DISABLE KEYS */;
INSERT INTO `olympiade2012_tms` (`TeamID`,`TeamName`) VALUES 
 ('a1','Alte Herren'),
 ('a2','Eltern'),
 ('a3','Green Tigers'),
 ('a4','Players'),
 ('a5','Schrotties'),
 ('a6','Tennis'),
 ('a7','Trainer');
/*!40000 ALTER TABLE `olympiade2012_tms` ENABLE KEYS */;


--
-- Definition of table `olympiade2012_tor`
--

DROP TABLE IF EXISTS `olympiade2012_tor`;
CREATE TABLE `olympiade2012_tor` (
  `Spieler` varchar(30) NOT NULL,
  `Team` varchar(30) NOT NULL,
  `Tore` int(10) unsigned NOT NULL,
  PRIMARY KEY (`Spieler`,`Team`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `olympiade2012_tor`
--

/*!40000 ALTER TABLE `olympiade2012_tor` DISABLE KEYS */;
/*!40000 ALTER TABLE `olympiade2012_tor` ENABLE KEYS */;


--
-- Definition of table `rsc11_01_161119_südpack_vs_rubinion`
--

DROP TABLE IF EXISTS `rsc11_01_161119_südpack_vs_rubinion`;
CREATE TABLE `rsc11_01_161119_südpack_vs_rubinion` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc11_01_161119_südpack_vs_rubinion`
--

/*!40000 ALTER TABLE `rsc11_01_161119_südpack_vs_rubinion` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc11_01_161119_südpack_vs_rubinion` ENABLE KEYS */;


--
-- Definition of table `rsc11_02_164604_reinz_vs_beiselen`
--

DROP TABLE IF EXISTS `rsc11_02_164604_reinz_vs_beiselen`;
CREATE TABLE `rsc11_02_164604_reinz_vs_beiselen` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc11_02_164604_reinz_vs_beiselen`
--

/*!40000 ALTER TABLE `rsc11_02_164604_reinz_vs_beiselen` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc11_02_164604_reinz_vs_beiselen` ENABLE KEYS */;


--
-- Definition of table `rsc11_03_165848_dwekd_vs_südpack`
--

DROP TABLE IF EXISTS `rsc11_03_165848_dwekd_vs_südpack`;
CREATE TABLE `rsc11_03_165848_dwekd_vs_südpack` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc11_03_165848_dwekd_vs_südpack`
--

/*!40000 ALTER TABLE `rsc11_03_165848_dwekd_vs_südpack` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc11_03_165848_dwekd_vs_südpack` ENABLE KEYS */;


--
-- Definition of table `rsc11_04_171241_rubinion_vs_reinz`
--

DROP TABLE IF EXISTS `rsc11_04_171241_rubinion_vs_reinz`;
CREATE TABLE `rsc11_04_171241_rubinion_vs_reinz` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc11_04_171241_rubinion_vs_reinz`
--

/*!40000 ALTER TABLE `rsc11_04_171241_rubinion_vs_reinz` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc11_04_171241_rubinion_vs_reinz` ENABLE KEYS */;


--
-- Definition of table `rsc11_05_172607_beiselen_vs_dwekd`
--

DROP TABLE IF EXISTS `rsc11_05_172607_beiselen_vs_dwekd`;
CREATE TABLE `rsc11_05_172607_beiselen_vs_dwekd` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc11_05_172607_beiselen_vs_dwekd`
--

/*!40000 ALTER TABLE `rsc11_05_172607_beiselen_vs_dwekd` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc11_05_172607_beiselen_vs_dwekd` ENABLE KEYS */;


--
-- Definition of table `rsc11_06_173921_südpack_vs_reinz`
--

DROP TABLE IF EXISTS `rsc11_06_173921_südpack_vs_reinz`;
CREATE TABLE `rsc11_06_173921_südpack_vs_reinz` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc11_06_173921_südpack_vs_reinz`
--

/*!40000 ALTER TABLE `rsc11_06_173921_südpack_vs_reinz` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc11_06_173921_südpack_vs_reinz` ENABLE KEYS */;


--
-- Definition of table `rsc11_07_175228_dwekd_vs_rubinion`
--

DROP TABLE IF EXISTS `rsc11_07_175228_dwekd_vs_rubinion`;
CREATE TABLE `rsc11_07_175228_dwekd_vs_rubinion` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc11_07_175228_dwekd_vs_rubinion`
--

/*!40000 ALTER TABLE `rsc11_07_175228_dwekd_vs_rubinion` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc11_07_175228_dwekd_vs_rubinion` ENABLE KEYS */;


--
-- Definition of table `rsc11_08_180524_beiselen_vs_südpack`
--

DROP TABLE IF EXISTS `rsc11_08_180524_beiselen_vs_südpack`;
CREATE TABLE `rsc11_08_180524_beiselen_vs_südpack` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc11_08_180524_beiselen_vs_südpack`
--

/*!40000 ALTER TABLE `rsc11_08_180524_beiselen_vs_südpack` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc11_08_180524_beiselen_vs_südpack` ENABLE KEYS */;


--
-- Definition of table `rsc11_09_181849_reinz_vs_dwekd`
--

DROP TABLE IF EXISTS `rsc11_09_181849_reinz_vs_dwekd`;
CREATE TABLE `rsc11_09_181849_reinz_vs_dwekd` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc11_09_181849_reinz_vs_dwekd`
--

/*!40000 ALTER TABLE `rsc11_09_181849_reinz_vs_dwekd` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc11_09_181849_reinz_vs_dwekd` ENABLE KEYS */;


--
-- Definition of table `rsc11_10_183256_rubinion_vs_beiselen`
--

DROP TABLE IF EXISTS `rsc11_10_183256_rubinion_vs_beiselen`;
CREATE TABLE `rsc11_10_183256_rubinion_vs_beiselen` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `SpielZeit` varchar(5) DEFAULT NULL,
  `ToreA` int(3) unsigned DEFAULT NULL,
  `ToreB` int(3) unsigned DEFAULT NULL,
  `Team` varchar(45) DEFAULT NULL,
  `Spieler` varchar(45) DEFAULT NULL,
  `Event` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc11_10_183256_rubinion_vs_beiselen`
--

/*!40000 ALTER TABLE `rsc11_10_183256_rubinion_vs_beiselen` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc11_10_183256_rubinion_vs_beiselen` ENABLE KEYS */;


--
-- Definition of table `rsc11_spl`
--

DROP TABLE IF EXISTS `rsc11_spl`;
CREATE TABLE `rsc11_spl` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Tore1` int(10) unsigned DEFAULT NULL,
  `Tore2` int(10) unsigned DEFAULT NULL,
  `X` int(10) unsigned NOT NULL DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL,
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8 PACK_KEYS=1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `rsc11_spl`
--

/*!40000 ALTER TABLE `rsc11_spl` DISABLE KEYS */;
INSERT INTO `rsc11_spl` (`SpielID`,`Team1`,`Team2`,`Tore1`,`Tore2`,`X`,`Gruppe`) VALUES 
 (1,'a1','a2',7,1,8,'a'),
 (2,'a3','a4',2,1,8,'a'),
 (3,'a5','a1',2,5,8,'a'),
 (4,'a2','a3',3,4,8,'a'),
 (5,'a4','a5',3,2,8,'a'),
 (6,'a1','a3',4,2,8,'a'),
 (7,'a5','a2',2,5,8,'a'),
 (8,'a4','a1',0,5,8,'a'),
 (9,'a3','a5',6,2,8,'a'),
 (10,'a2','a4',4,2,8,'a');
/*!40000 ALTER TABLE `rsc11_spl` ENABLE KEYS */;


--
-- Definition of table `rsc11_tbl`
--

DROP TABLE IF EXISTS `rsc11_tbl`;
CREATE TABLE `rsc11_tbl` (
  `TeamID` int(3) unsigned NOT NULL,
  `Spieler1` int(3) DEFAULT NULL,
  `Spieler2` int(3) DEFAULT NULL,
  `Spieler3` int(3) unsigned DEFAULT '0',
  `Spieler4` int(3) unsigned DEFAULT '0',
  `Spiele` int(3) unsigned DEFAULT '0',
  `S` int(3) unsigned DEFAULT '0',
  `U` int(3) unsigned DEFAULT '0',
  `N` int(3) unsigned DEFAULT '0',
  `ToreP` int(4) unsigned DEFAULT '0',
  `ToreM` int(4) unsigned DEFAULT '0',
  `Punkte` int(3) unsigned DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL DEFAULT 'a',
  `Status` int(3) unsigned DEFAULT '1',
  `Team1` varchar(45) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `rsc11_tbl`
--

/*!40000 ALTER TABLE `rsc11_tbl` DISABLE KEYS */;
INSERT INTO `rsc11_tbl` (`TeamID`,`Spieler1`,`Spieler2`,`Spieler3`,`Spieler4`,`Spiele`,`S`,`U`,`N`,`ToreP`,`ToreM`,`Punkte`,`Gruppe`,`Status`,`Team1`) VALUES 
 (1,NULL,NULL,0,0,4,4,0,0,21,5,12,'a',1,'a1'),
 (2,NULL,NULL,0,0,4,2,0,2,13,15,6,'a',1,'a2'),
 (3,NULL,NULL,0,0,4,3,0,1,14,10,9,'a',1,'a3'),
 (4,NULL,NULL,0,0,4,1,0,3,6,13,3,'a',1,'a4'),
 (5,NULL,NULL,0,0,4,0,0,4,8,19,0,'a',1,'a5');
/*!40000 ALTER TABLE `rsc11_tbl` ENABLE KEYS */;


--
-- Definition of table `rsc11_tor`
--

DROP TABLE IF EXISTS `rsc11_tor`;
CREATE TABLE `rsc11_tor` (
  `Spieler` varchar(30) NOT NULL,
  `Team` varchar(30) NOT NULL,
  `Tore` int(10) unsigned NOT NULL,
  PRIMARY KEY (`Spieler`,`Team`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc11_tor`
--

/*!40000 ALTER TABLE `rsc11_tor` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc11_tor` ENABLE KEYS */;


--
-- Definition of table `rsc12_spl`
--

DROP TABLE IF EXISTS `rsc12_spl`;
CREATE TABLE `rsc12_spl` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Tore1` int(10) unsigned DEFAULT NULL,
  `Tore2` int(10) unsigned DEFAULT NULL,
  `X` int(10) unsigned NOT NULL DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL,
  `Team1Name` varchar(30) DEFAULT NULL,
  `Team2Name` varchar(30) DEFAULT NULL,
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8 PACK_KEYS=1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `rsc12_spl`
--

/*!40000 ALTER TABLE `rsc12_spl` DISABLE KEYS */;
INSERT INTO `rsc12_spl` (`SpielID`,`Team1`,`Team2`,`Tore1`,`Tore2`,`X`,`Gruppe`,`Team1Name`,`Team2Name`) VALUES 
 (1,'a1','a2',NULL,NULL,0,'a',NULL,NULL),
 (2,'a3','a4',NULL,NULL,0,'a',NULL,NULL),
 (3,'a5','a6',NULL,NULL,0,'a',NULL,NULL),
 (4,'a3','a1',NULL,NULL,0,'a',NULL,NULL),
 (5,'a2','a5',NULL,NULL,0,'a',NULL,NULL),
 (6,'a4','a6',NULL,NULL,0,'a',NULL,NULL),
 (7,'a1','a5',NULL,NULL,0,'a',NULL,NULL),
 (8,'a6','a3',NULL,NULL,0,'a',NULL,NULL),
 (9,'a4','a2',NULL,NULL,0,'a',NULL,NULL),
 (10,'a6','a1',NULL,NULL,0,'a',NULL,NULL),
 (11,'a5','a4',NULL,NULL,0,'a',NULL,NULL),
 (12,'a3','a2',NULL,NULL,0,'a',NULL,NULL),
 (13,'a1','a4',NULL,NULL,0,'a',NULL,NULL),
 (14,'a2','a6',NULL,NULL,0,'a',NULL,NULL),
 (15,'a3','a5',NULL,NULL,0,'a',NULL,NULL);
/*!40000 ALTER TABLE `rsc12_spl` ENABLE KEYS */;


--
-- Definition of table `rsc12_tbl`
--

DROP TABLE IF EXISTS `rsc12_tbl`;
CREATE TABLE `rsc12_tbl` (
  `TeamID` varchar(3) NOT NULL,
  `Spieler1` int(3) DEFAULT NULL,
  `Spieler2` int(3) DEFAULT NULL,
  `Spieler3` int(3) unsigned DEFAULT '0',
  `Spieler4` int(3) unsigned DEFAULT '0',
  `Spiele` int(3) unsigned DEFAULT '0',
  `S` int(3) unsigned DEFAULT '0',
  `U` int(3) unsigned DEFAULT '0',
  `N` int(3) unsigned DEFAULT '0',
  `ToreP` int(4) unsigned DEFAULT '0',
  `ToreM` int(4) unsigned DEFAULT '0',
  `Punkte` int(3) unsigned DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL DEFAULT 'a',
  `Status` int(3) unsigned DEFAULT '1',
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `rsc12_tbl`
--

/*!40000 ALTER TABLE `rsc12_tbl` DISABLE KEYS */;
INSERT INTO `rsc12_tbl` (`TeamID`,`Spieler1`,`Spieler2`,`Spieler3`,`Spieler4`,`Spiele`,`S`,`U`,`N`,`ToreP`,`ToreM`,`Punkte`,`Gruppe`,`Status`) VALUES 
 ('a1',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a2',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a3',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a4',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a5',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a6',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1);
/*!40000 ALTER TABLE `rsc12_tbl` ENABLE KEYS */;


--
-- Definition of table `rsc12_tms`
--

DROP TABLE IF EXISTS `rsc12_tms`;
CREATE TABLE `rsc12_tms` (
  `TeamID` varchar(3) NOT NULL,
  `TeamName` varchar(30) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc12_tms`
--

/*!40000 ALTER TABLE `rsc12_tms` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc12_tms` ENABLE KEYS */;


--
-- Definition of table `rsc12_tor`
--

DROP TABLE IF EXISTS `rsc12_tor`;
CREATE TABLE `rsc12_tor` (
  `Spieler` varchar(30) NOT NULL,
  `Team` varchar(30) NOT NULL,
  `Tore` int(10) unsigned NOT NULL,
  PRIMARY KEY (`Spieler`,`Team`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc12_tor`
--

/*!40000 ALTER TABLE `rsc12_tor` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc12_tor` ENABLE KEYS */;


--
-- Definition of table `rsc_2012_spl`
--

DROP TABLE IF EXISTS `rsc_2012_spl`;
CREATE TABLE `rsc_2012_spl` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Tore1` int(10) unsigned DEFAULT NULL,
  `Tore2` int(10) unsigned DEFAULT NULL,
  `X` int(10) unsigned NOT NULL DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL,
  `Team1Name` varchar(30) DEFAULT NULL,
  `Team2Name` varchar(30) DEFAULT NULL,
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8 PACK_KEYS=1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `rsc_2012_spl`
--

/*!40000 ALTER TABLE `rsc_2012_spl` DISABLE KEYS */;
INSERT INTO `rsc_2012_spl` (`SpielID`,`Team1`,`Team2`,`Tore1`,`Tore2`,`X`,`Gruppe`,`Team1Name`,`Team2Name`) VALUES 
 (1,'a1','a2',NULL,NULL,0,'a',NULL,NULL),
 (2,'a3','a4',NULL,NULL,0,'a',NULL,NULL),
 (3,'a5','a6',NULL,NULL,0,'a',NULL,NULL),
 (4,'a3','a1',NULL,NULL,0,'a',NULL,NULL),
 (5,'a2','a5',NULL,NULL,0,'a',NULL,NULL),
 (6,'a4','a6',NULL,NULL,0,'a',NULL,NULL),
 (7,'a1','a5',NULL,NULL,0,'a',NULL,NULL),
 (8,'a6','a3',NULL,NULL,0,'a',NULL,NULL),
 (9,'a4','a2',NULL,NULL,0,'a',NULL,NULL),
 (10,'a6','a1',NULL,NULL,0,'a',NULL,NULL),
 (11,'a5','a4',NULL,NULL,0,'a',NULL,NULL),
 (12,'a3','a2',NULL,NULL,0,'a',NULL,NULL),
 (13,'a1','a4',NULL,NULL,0,'a',NULL,NULL),
 (14,'a2','a6',NULL,NULL,0,'a',NULL,NULL),
 (15,'a3','a5',NULL,NULL,0,'a',NULL,NULL);
/*!40000 ALTER TABLE `rsc_2012_spl` ENABLE KEYS */;


--
-- Definition of table `rsc_2012_tbl`
--

DROP TABLE IF EXISTS `rsc_2012_tbl`;
CREATE TABLE `rsc_2012_tbl` (
  `TeamID` varchar(3) NOT NULL,
  `Spieler1` int(3) DEFAULT NULL,
  `Spieler2` int(3) DEFAULT NULL,
  `Spieler3` int(3) unsigned DEFAULT '0',
  `Spieler4` int(3) unsigned DEFAULT '0',
  `Spiele` int(3) unsigned DEFAULT '0',
  `S` int(3) unsigned DEFAULT '0',
  `U` int(3) unsigned DEFAULT '0',
  `N` int(3) unsigned DEFAULT '0',
  `ToreP` int(4) unsigned DEFAULT '0',
  `ToreM` int(4) unsigned DEFAULT '0',
  `Punkte` int(3) unsigned DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL DEFAULT 'a',
  `Status` int(3) unsigned DEFAULT '1',
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `rsc_2012_tbl`
--

/*!40000 ALTER TABLE `rsc_2012_tbl` DISABLE KEYS */;
INSERT INTO `rsc_2012_tbl` (`TeamID`,`Spieler1`,`Spieler2`,`Spieler3`,`Spieler4`,`Spiele`,`S`,`U`,`N`,`ToreP`,`ToreM`,`Punkte`,`Gruppe`,`Status`) VALUES 
 ('a1',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a2',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a3',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a4',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a5',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a6',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1);
/*!40000 ALTER TABLE `rsc_2012_tbl` ENABLE KEYS */;


--
-- Definition of table `rsc_2012_tms`
--

DROP TABLE IF EXISTS `rsc_2012_tms`;
CREATE TABLE `rsc_2012_tms` (
  `TeamID` varchar(3) NOT NULL,
  `TeamName` varchar(30) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc_2012_tms`
--

/*!40000 ALTER TABLE `rsc_2012_tms` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc_2012_tms` ENABLE KEYS */;


--
-- Definition of table `rsc_2012_tor`
--

DROP TABLE IF EXISTS `rsc_2012_tor`;
CREATE TABLE `rsc_2012_tor` (
  `Spieler` varchar(30) NOT NULL,
  `Team` varchar(30) NOT NULL,
  `Tore` int(10) unsigned NOT NULL,
  PRIMARY KEY (`Spieler`,`Team`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `rsc_2012_tor`
--

/*!40000 ALTER TABLE `rsc_2012_tor` DISABLE KEYS */;
/*!40000 ALTER TABLE `rsc_2012_tor` ENABLE KEYS */;


--
-- Definition of table `spieler`
--

DROP TABLE IF EXISTS `spieler`;
CREATE TABLE `spieler` (
  `SpielerName` varchar(30) NOT NULL,
  `SpielerID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`SpielerID`)
) ENGINE=InnoDB AUTO_INCREMENT=38 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `spieler`
--

/*!40000 ALTER TABLE `spieler` DISABLE KEYS */;
INSERT INTO `spieler` (`SpielerName`,`SpielerID`) VALUES 
 ('Andreas',1),
 ('Ben vom Berg',2),
 ('Ben vom Wald',3),
 ('Buzz-T',4),
 ('Captain Doerk',5),
 ('Der Jupp',6),
 ('Dirk',7),
 ('Eise',8),
 ('Emre',9),
 ('Flo vom Berg',10),
 ('Fraggy',11),
 ('Inge',12),
 ('Jens',13),
 ('Luca T.',14),
 ('Mad Maz',15),
 ('Maggi',16),
 ('Mahony',17),
 ('Marc',18),
 ('Mario vom Wald',19),
 ('Masmo',20),
 ('Matze vom Berg',21),
 ('Nietzsche',22),
 ('Phantastic Phil',23),
 ('Quicky',24),
 ('Richard',25),
 ('Rio',26),
 ('Schmiddi',27),
 ('Shooter',28),
 ('Shorty',29),
 ('Tha Baral',30),
 ('Tha Man',31),
 ('Thomas',32),
 ('Timo',33),
 ('Wieland',34),
 ('Zwuckel',35),
 ('_Eigentor',36),
 ('_unbekannt',37);
/*!40000 ALTER TABLE `spieler` ENABLE KEYS */;


--
-- Definition of table `t_spieler`
--

DROP TABLE IF EXISTS `t_spieler`;
CREATE TABLE `t_spieler` (
  `SpielerID` int(3) NOT NULL AUTO_INCREMENT,
  `Name` varchar(30) NOT NULL DEFAULT '',
  `Vorname` varchar(30) NOT NULL DEFAULT '',
  `Nickname` varchar(30) NOT NULL DEFAULT '',
  `TeamID` int(3) DEFAULT NULL,
  `Image100` blob,
  PRIMARY KEY (`SpielerID`)
) ENGINE=MyISAM AUTO_INCREMENT=120 DEFAULT CHARSET=utf8;

--
-- Dumping data for table `t_spieler`
--

/*!40000 ALTER TABLE `t_spieler` DISABLE KEYS */;
INSERT INTO `t_spieler` (`SpielerID`,`Name`,`Vorname`,`Nickname`,`TeamID`,`Image100`) VALUES 
 (1,'scheike','jochen','StA',0,NULL),
 (2,'malas','dirk','Captain Doerk',25,),
 (3,'z&auml;hringer','matthias','Mad Maz',2,),
 (4,'wildner','dominic','Domme',0,NULL),
 (5,'wichmann','alexander','Mystique',0,NULL),
 (6,'wichmann','gernot','Gernot W.',0,NULL),
 (7,'rohloff','thorsten','Thorti',0,NULL),
 (8,'engelhardt','rainer','Rainer E.',0,NULL),
 (9,'maierhofer','bernd','Bernd M.',0,NULL),
 (10,'mildenberger','pascal','Pasquale',0,NULL),
 (11,'l&uuml;cke','dirk','Dirk L.',0,NULL),
 (12,'piesch','timo','Timo P.',0,NULL),
 (13,'encke','timo','Timo E.',0,NULL),
 (14,'tessaro','nico','Nico T.',0,NULL),
 (15,'niederberger','jens','Jens N.',0,NULL),
 (16,'hildebrandt','ingo','Inge',9,NULL),
 (17,'lorenz','nadine','Nadine L.',0,NULL),
 (18,'kurrle','markus','Markus K.',0,NULL),
 (19,'de noni','sven','Mahony',0,),
 (20,'zowe','jochen','Shooter',0,);
INSERT INTO `t_spieler` (`SpielerID`,`Name`,`Vorname`,`Nickname`,`TeamID`,`Image100`) VALUES 
 (21,'emhardt','steffen','Emre Deluxe',0,),
 (22,'scheike','thomas','Thomas S.',0,NULL),
 (23,'marder','patrick','Patrick M.',0,NULL),
 (24,'dehm','alexander','Alexander D.',0,NULL),
 (25,'wolf','martin','Tha Man',0,),
 (26,'benzler','claus','Tha Diva',0,NULL),
 (27,'schibilski','marco','Marco S.',0,NULL),
 (28,'hahn','frederic','Fraggy',0,NULL),
 (29,'organzidis','thomas','Orgi',0,NULL),
 (30,'weller','oliver','Oli W.',0,NULL),
 (31,'hahn','oliver','H&uuml;hny',0,NULL),
 (32,'he&szlig;','stefan','Theo',0,),
 (33,'pfuderer','tobias','Tobi P.',0,NULL),
 (34,'l&uuml;cke','jens','Jens L.',0,),
 (35,'enge','sebastian','Buzz-T',0,NULL);
INSERT INTO `t_spieler` (`SpielerID`,`Name`,`Vorname`,`Nickname`,`TeamID`,`Image100`) VALUES 
 (36,'','gregor','Gregor',0,NULL),
 (37,'reiff','christoph','Christoph R.',0,NULL),
 (38,'benz','eric','Eric B.',0,NULL),
 (39,'','','Vesper',0,NULL),
 (40,'hertler','timo','Didi',0,NULL),
 (41,'lindacher','holger','Wolli',0,NULL),
 (42,'schenk','benjamin','Ben vom Berg',0,NULL),
 (43,'forch','sebastian','Seba Forch',0,NULL),
 (44,'','goran','Goran',0,NULL),
 (45,'gr&uuml;ner','axel','Axel G.',0,NULL),
 (46,'gomez','alex','Raul',0,NULL),
 (47,'schmiedel','sebastian','Seba Schmiedel',0,NULL),
 (48,'benzler','steffen','Steven',0,NULL),
 (49,'k&uuml;pper','andreas','Andy Küpper',0,NULL),
 (50,'starkloff','timo','Timo S.',0,NULL),
 (51,'drexler','markus','Markus D.',0,NULL),
 (52,'utz','benjamin','Be_Wave',0,NULL),
 (53,'moll','daniel','Doktor Molli',0,NULL),
 (54,'keiper','benjamin','Ailton',0,NULL),
 (55,'buck','michael','Michael B.',0,NULL),
 (56,'schnell','markus','Schnelle',0,),
 (57,'','yil','Yil',0,NULL),
 (58,'jakob','sabine','Sabine J.',0,NULL),
 (59,'','bj$ouml;rn','Bj&ouml;rn',0,NULL),
 (60,'schenk','david','Dave vom Berg',0,NULL),
 (61,'jung','marcus','Eurus',0,NULL),
 (62,'tessaro','florian','Flo T.',0,NULL),
 (63,'baral','matthias','Tha Baral',0,NULL),
 (64,'schneider','jan','Jan S.',0,NULL),
 (65,'','joseph','Joe',0,NULL),
 (66,'kasper','bj&ouml;rn','Bj&ouml;&ouml;n',0,NULL),
 (67,'k&uuml;rbis','dennis','K&uuml;rbis',0,NULL),
 (68,'jakob','philipp','Fips',0,),
 (69,'forisch','timo','Schlampe',0,NULL),
 (70,'','marco','Mack-O',0,NULL),
 (71,'w&uuml;rz','rainer','Raineh',0,NULL),
 (72,'h&auml;fner','bastian','Heif',0,NULL),
 (73,'eisenmann','frank','Eise F.',0,),
 (74,'michalski','marco','Mekko',0,NULL);
INSERT INTO `t_spieler` (`SpielerID`,`Name`,`Vorname`,`Nickname`,`TeamID`,`Image100`) VALUES 
 (75,'kaiser','otto','Otto K.',0,NULL),
 (76,'krenn','christian','Christian K.',0,NULL),
 (77,'','tobias','Tobi',0,NULL),
 (78,'klotz','sven','El &Ouml;',0,NULL),
 (79,'kirschner','fabian','Buunsch',0,),
 (80,'boese','lars','Lars B.',0,NULL),
 (81,'fai&szlig;t','daniel','Nachbar vom Berg',0,NULL),
 (82,'','','Nachbar vom Nachbar',0,NULL),
 (83,'schmidt','markus','Schmiddi',44,NULL),
 (84,'oberknapp','dirk','Dirk O.',0,NULL),
 (85,'hytrek','florian','Flo vom Berg',0,),
 (86,'wagner','dennis','Ratch',0,NULL),
 (87,'ullmann','sascha','Ullmann',0,NULL),
 (88,'petry','andreas','Andy Petry',0,NULL),
 (89,'erny','thomas','Ernheimer',0,NULL),
 (90,'koch','andreas','Andreas K.',0,),
 (91,'m&uuml;ller','markus','Markus M.',0,NULL);
INSERT INTO `t_spieler` (`SpielerID`,`Name`,`Vorname`,`Nickname`,`TeamID`,`Image100`) VALUES 
 (92,'','marc','Marc',0,NULL),
 (93,'zilles','tina','die Hippe',0,NULL),
 (94,'schmalz','alexander','Alex S.',0,NULL),
 (95,'schanz','oliver','Olli S.',0,NULL),
 (96,'wagenhals','armin','Oarmin',0,NULL),
 (97,'gebhardt','philipp','Phantastic Phil',0,),
 (98,'','matthias','Mehr Maz',0,NULL),
 (99,'wedertz','benjamin','Ben vom Wald',36,),
 (100,'weirich','mario','Mario vom Wald',36,),
 (101,'hees','volker','Volker H.',0,NULL);
INSERT INTO `t_spieler` (`SpielerID`,`Name`,`Vorname`,`Nickname`,`TeamID`,`Image100`) VALUES 
 (102,'ilg','rainer','Rainer I.',0,NULL),
 (103,'weng','andreas','Wengi',44,),
 (104,'zilles','lisa','Lotte',0,NULL),
 (105,'zwicker','michael','Zwuckel',9,NULL),
 (106,'','daniele','Luca T.',2,),
 (107,'geilke','michael','Fjen',0,NULL),
 (108,'','masmo','Masmo',0,NULL),
 (109,'','giovanni','Giovanni',0,NULL),
 (110,'mozer','simon','Simon M.',0,NULL),
 (111,'wengert','nicolai','Maggi',0,),
 (112,'traub','matthias','Nietzsche',0,),
 (113,'stolzenburg','wieland','Wieland S.',0,),
 (114,'biesinger','lars','Lars B.',0,NULL),
 (115,'schmidt','markus','Markus Schmidt',0,NULL),
 (116,'benzler','matthias','Matze vom Berg',0,),
 (117,'ljubicic','marko','Richard',0,),
 (118,'wannhoff','jens','Rio',0,),
 (119,'zander','sebastian','der Jupp',0,);
/*!40000 ALTER TABLE `t_spieler` ENABLE KEYS */;


--
-- Definition of table `t_teams`
--

DROP TABLE IF EXISTS `t_teams`;
CREATE TABLE `t_teams` (
  `TeamID` varchar(3) NOT NULL,
  `TeamName` varchar(30) NOT NULL,
  `TurnierName` varchar(45) NOT NULL,
  `TurnierPos` varchar(45) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=MyISAM AUTO_INCREMENT=50 DEFAULT CHARSET=utf8;

--
-- Dumping data for table `t_teams`
--

/*!40000 ALTER TABLE `t_teams` DISABLE KEYS */;
INSERT INTO `t_teams` (`TeamID`,`TeamName`,`TurnierName`,`TurnierPos`) VALUES 
 ('1','Dom Kölsch','',''),
 ('2','Porsche','kc2011','b4'),
 ('3','AC Milan','',''),
 ('4','Volleyball','',''),
 ('5','FSG Abi 2000','',''),
 ('6','Porsche II','',''),
 ('7','Tesa Power','kc2011','b2'),
 ('8','Beck\'s','',''),
 ('9','Deportivo','kc2011',''),
 ('10','Sauffpark','kc2011','c6'),
 ('11','Gold Ochsen','',''),
 ('12','Wolfbräu','kc2011','c4'),
 ('13','Küppers Kölsch','',''),
 ('14','Rothaus','kc2011','c5'),
 ('15','Gastrupp','',''),
 ('16','FC Marbach','',''),
 ('17','Ensinger','kc2011','b1'),
 ('18','Früh Kölsch','',''),
 ('19','TNT','',''),
 ('20','Malteser','',''),
 ('21','Captain Choc','',''),
 ('22','Augustiner','',''),
 ('23','Condom Kickers','kc2011',''),
 ('24','Mollutz','',''),
 ('25','Sion Kölsch','kc2011','c1'),
 ('26','ABBA','',''),
 ('27','Air Combats','',''),
 ('28','Tipp Kick','',''),
 ('29','Lange Glieder','',''),
 ('30','Sparda Orange','kc2011','c3'),
 ('31','Zechpreller','test_cup',''),
 ('32','Bergmacht','kc2011',''),
 ('33','Streuwagen','',''),
 ('34','KSK','',''),
 ('35','Extinguishers','',''),
 ('36','Waldmeister','kc2011','b5'),
 ('37','K und K','',''),
 ('38','Schmoll','',''),
 ('39','Olympiakos','kc2011',''),
 ('40','Lok Prellbock','',''),
 ('41','Die drei Fragezeichen','',''),
 ('42','Dynamisches Duo','kc2011',''),
 ('43','Hack Men Group','',''),
 ('44','Hulaboys','',''),
 ('45','Dürres Duo','',''),
 ('46','Maggi & Nietzsche','kc2011','b6'),
 ('47','Juventus','',''),
 ('48','Sambuca','',''),
 ('49','Gaffel Kölsch','kc2011',''),
 ('h1','Reinz','soccercup2010',''),
 ('h2','Beiselen','soccercup2010',''),
 ('h3','DWEKD','soccercup2010',''),
 ('h4','rubinion','soccercup2010',''),
 ('h5','Fundracer','soccercup2010',''),
 ('50','die wilden Kerle','kc2011','c2'),
 ('51','Oransina','kc2011','b3'),
 ('o1','Alte Herren','olympiade2011',''),
 ('o2','Green Tigers','olympiade2011',''),
 ('o3','Die Pfeifen','olympiade2011',''),
 ('o4','Rugby','olympiade2011',''),
 ('o5','Deep Team','olympiade2011',''),
 ('o6','Players','olympiade2011',''),
 ('o7','Tennis','olympiade2011',''),
 ('o8','Team8','olympiade2011',''),
 ('o9','Team9','olympiade2011',''),
 ('o10','Team10','olympiade2011',''),
 ('z1','Dogs','default','a1'),
 ('z2','Cats','default','a2'),
 ('z3','Bugs','default','a3'),
 ('z4','Fish','default','a4');
/*!40000 ALTER TABLE `t_teams` ENABLE KEYS */;


--
-- Definition of table `tabelle`
--

DROP TABLE IF EXISTS `tabelle`;
CREATE TABLE `tabelle` (
  `TeamName` varchar(30) NOT NULL,
  `Spiele` int(10) unsigned DEFAULT '0',
  `S` int(10) unsigned DEFAULT '0',
  `U` int(10) unsigned DEFAULT '0',
  `N` int(10) unsigned DEFAULT '0',
  `ToreP` int(10) unsigned DEFAULT '0',
  `ToreM` int(10) unsigned DEFAULT '0',
  `Punkte` int(10) unsigned DEFAULT '0',
  `Gruppe` varchar(1) DEFAULT NULL,
  PRIMARY KEY (`TeamName`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `tabelle`
--

/*!40000 ALTER TABLE `tabelle` DISABLE KEYS */;
INSERT INTO `tabelle` (`TeamName`,`Spiele`,`S`,`U`,`N`,`ToreP`,`ToreM`,`Punkte`,`Gruppe`) VALUES 
 ('Bergmacht',0,0,0,0,0,0,0,'b'),
 ('Condom Kickers',0,0,0,0,0,0,0,'c'),
 ('Deportivo',14,0,12,2,0,3,12,'a'),
 ('Dynamisches Duo',1,0,1,0,0,0,1,'b'),
 ('Ensinger',0,0,0,0,0,0,0,'a'),
 ('Gaffel Kölsch',0,0,0,0,0,0,0,'c'),
 ('Hulaboys',3,0,3,0,0,0,3,'a'),
 ('Maggi & Nietzsche',1,0,1,0,0,0,1,'b'),
 ('Olympiakos',0,0,0,0,0,0,0,'c'),
 ('Porsche',12,2,10,0,3,0,16,'a'),
 ('Rothaus',5,1,4,0,1,0,7,'a'),
 ('Sambuca',0,0,0,0,0,0,0,'b'),
 ('Sauffpark',0,0,0,0,0,0,0,'c'),
 ('Sion Kölsch',0,0,0,0,0,0,0,'c'),
 ('Spaten-Jungs',0,0,0,0,0,0,0,'b'),
 ('Tesa Power',4,0,3,1,0,1,3,'a'),
 ('Waldmeister',0,0,0,0,0,0,0,'c'),
 ('Wolfbräu',0,0,0,0,0,0,0,'b');
/*!40000 ALTER TABLE `tabelle` ENABLE KEYS */;


--
-- Definition of table `teams`
--

DROP TABLE IF EXISTS `teams`;
CREATE TABLE `teams` (
  `TeamID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `TeamName` varchar(45) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `teams`
--

/*!40000 ALTER TABLE `teams` DISABLE KEYS */;
INSERT INTO `teams` (`TeamID`,`TeamName`) VALUES 
 (1,'Deportivo'),
 (2,'Porsche'),
 (3,'Rothaus'),
 (4,'Tesa Power'),
 (5,'Hulaboys'),
 (6,'Ensinger'),
 (7,'Dynamisches Duo'),
 (8,'Maggi & Nietzsche'),
 (9,'Bergmacht'),
 (10,'Spaten-Jungs'),
 (11,'Wolfbräu'),
 (12,'Sambuca'),
 (13,'Condom Kickers'),
 (14,'Sauffpark'),
 (15,'Gaffel Kölsch'),
 (16,'Sion Kölsch'),
 (17,'Waldmeister'),
 (18,'Olympiakos');
/*!40000 ALTER TABLE `teams` ENABLE KEYS */;


--
-- Definition of table `test_spl`
--

DROP TABLE IF EXISTS `test_spl`;
CREATE TABLE `test_spl` (
  `SpielID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Team1` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Team2` varchar(3) CHARACTER SET latin1 NOT NULL,
  `Tore1` int(10) unsigned DEFAULT NULL,
  `Tore2` int(10) unsigned DEFAULT NULL,
  `X` int(10) unsigned NOT NULL DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL,
  `Team1Name` varchar(30) DEFAULT NULL,
  `Team2Name` varchar(30) DEFAULT NULL,
  PRIMARY KEY (`SpielID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=utf8 PACK_KEYS=1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `test_spl`
--

/*!40000 ALTER TABLE `test_spl` DISABLE KEYS */;
INSERT INTO `test_spl` (`SpielID`,`Team1`,`Team2`,`Tore1`,`Tore2`,`X`,`Gruppe`,`Team1Name`,`Team2Name`) VALUES 
 (1,'a1','a2',NULL,NULL,0,'a',NULL,NULL),
 (2,'a3','a4',NULL,NULL,0,'a',NULL,NULL),
 (3,'b1','b2',NULL,NULL,0,'b',NULL,NULL),
 (4,'b3','b4',NULL,NULL,0,'b',NULL,NULL),
 (5,'a1','a3',NULL,NULL,0,'a',NULL,NULL),
 (6,'a4','a2',NULL,NULL,0,'a',NULL,NULL),
 (7,'b1','b3',NULL,NULL,0,'b',NULL,NULL),
 (8,'b4','b2',NULL,NULL,0,'b',NULL,NULL),
 (9,'a1','a4',NULL,NULL,0,'a',NULL,NULL),
 (10,'a2','a3',NULL,NULL,0,'a',NULL,NULL),
 (11,'b1','b4',NULL,NULL,0,'b',NULL,NULL),
 (12,'b2','b3',NULL,NULL,0,'b',NULL,NULL),
 (13,'a1','a3',NULL,NULL,0,'2',NULL,NULL),
 (14,'b1','b3',NULL,NULL,0,'2',NULL,NULL);
/*!40000 ALTER TABLE `test_spl` ENABLE KEYS */;


--
-- Definition of table `test_tbl`
--

DROP TABLE IF EXISTS `test_tbl`;
CREATE TABLE `test_tbl` (
  `TeamID` varchar(3) NOT NULL,
  `Spieler1` int(3) DEFAULT NULL,
  `Spieler2` int(3) DEFAULT NULL,
  `Spieler3` int(3) unsigned DEFAULT '0',
  `Spieler4` int(3) unsigned DEFAULT '0',
  `Spiele` int(3) unsigned DEFAULT '0',
  `S` int(3) unsigned DEFAULT '0',
  `U` int(3) unsigned DEFAULT '0',
  `N` int(3) unsigned DEFAULT '0',
  `ToreP` int(4) unsigned DEFAULT '0',
  `ToreM` int(4) unsigned DEFAULT '0',
  `Punkte` int(3) unsigned DEFAULT '0',
  `Gruppe` varchar(3) NOT NULL DEFAULT 'a',
  `Status` int(3) unsigned DEFAULT '1',
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=DYNAMIC;

--
-- Dumping data for table `test_tbl`
--

/*!40000 ALTER TABLE `test_tbl` DISABLE KEYS */;
INSERT INTO `test_tbl` (`TeamID`,`Spieler1`,`Spieler2`,`Spieler3`,`Spieler4`,`Spiele`,`S`,`U`,`N`,`ToreP`,`ToreM`,`Punkte`,`Gruppe`,`Status`) VALUES 
 ('a1',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a2',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a3',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('a4',NULL,NULL,0,0,0,0,0,0,0,0,0,'a',1),
 ('b1',NULL,NULL,0,0,0,0,0,0,0,0,0,'b',1),
 ('b2',NULL,NULL,0,0,0,0,0,0,0,0,0,'b',1),
 ('b3',NULL,NULL,0,0,0,0,0,0,0,0,0,'b',1),
 ('b4',NULL,NULL,0,0,0,0,0,0,0,0,0,'b',1);
/*!40000 ALTER TABLE `test_tbl` ENABLE KEYS */;


--
-- Definition of table `test_tms`
--

DROP TABLE IF EXISTS `test_tms`;
CREATE TABLE `test_tms` (
  `TeamID` varchar(3) NOT NULL,
  `TeamName` varchar(30) NOT NULL,
  PRIMARY KEY (`TeamID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `test_tms`
--

/*!40000 ALTER TABLE `test_tms` DISABLE KEYS */;
INSERT INTO `test_tms` (`TeamID`,`TeamName`) VALUES 
 ('a1','Bergmacht'),
 ('a2','Deportivo'),
 ('a3','Ensinger'),
 ('a4','Galatasaray'),
 ('b1','Condom Kickers'),
 ('b2','die wilden Kerle'),
 ('b3','Gaffel Kölsch'),
 ('b4','Hulaboys');
/*!40000 ALTER TABLE `test_tms` ENABLE KEYS */;


--
-- Definition of table `test_tor`
--

DROP TABLE IF EXISTS `test_tor`;
CREATE TABLE `test_tor` (
  `Spieler` varchar(30) NOT NULL,
  `Team` varchar(30) NOT NULL,
  `Tore` int(10) unsigned NOT NULL,
  PRIMARY KEY (`Spieler`,`Team`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `test_tor`
--

/*!40000 ALTER TABLE `test_tor` DISABLE KEYS */;
/*!40000 ALTER TABLE `test_tor` ENABLE KEYS */;


--
-- Definition of table `turniere`
--

DROP TABLE IF EXISTS `turniere`;
CREATE TABLE `turniere` (
  `TurnierName` varchar(45) NOT NULL,
  `AnzGrupp` int(3) unsigned NOT NULL,
  `AnzTeams` int(3) unsigned NOT NULL,
  `AnzPerio` int(1) unsigned NOT NULL,
  `DauerSek` int(4) unsigned NOT NULL,
  `aktiv` tinyint(1) NOT NULL,
  `Schuet` tinyint(1) NOT NULL DEFAULT '1',
  `KCMode` tinyint(1) NOT NULL DEFAULT '1',
  PRIMARY KEY (`TurnierName`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `turniere`
--

/*!40000 ALTER TABLE `turniere` DISABLE KEYS */;
INSERT INTO `turniere` (`TurnierName`,`AnzGrupp`,`AnzTeams`,`AnzPerio`,`DauerSek`,`aktiv`,`Schuet`,`KCMode`) VALUES 
 ('default',2,5,1,600,0,1,1),
 ('foxcup',3,8,1,555,0,1,1),
 ('HippenCup',2,4,1,600,0,1,1),
 ('KC2011',3,6,1,600,0,1,1),
 ('Kölsch Cup 2009',3,6,1,600,0,1,1),
 ('O12',1,8,1,480,0,1,1),
 ('Olympiade2012',1,7,1,600,0,1,1),
 ('soccercup2010',1,5,1,62,0,1,1),
 ('test',2,4,1,600,0,1,1);
/*!40000 ALTER TABLE `turniere` ENABLE KEYS */;


--
-- Definition of table `turniere_default`
--

DROP TABLE IF EXISTS `turniere_default`;
CREATE TABLE `turniere_default` (
  `TurnierName` varchar(45) NOT NULL,
  `AnzGrupp` int(3) unsigned NOT NULL,
  `AnzTeams` int(3) unsigned NOT NULL,
  `AnzPerio` int(1) unsigned NOT NULL,
  `DauerSek` int(4) unsigned NOT NULL,
  `aktiv` tinyint(1) NOT NULL,
  PRIMARY KEY (`TurnierName`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `turniere_default`
--

/*!40000 ALTER TABLE `turniere_default` DISABLE KEYS */;
INSERT INTO `turniere_default` (`TurnierName`,`AnzGrupp`,`AnzTeams`,`AnzPerio`,`DauerSek`,`aktiv`) VALUES 
 ('default',1,4,1,600,1);
/*!40000 ALTER TABLE `turniere_default` ENABLE KEYS */;


--
-- Definition of view `kc2011_v_spl`
--

DROP TABLE IF EXISTS `kc2011_v_spl`;
DROP VIEW IF EXISTS `kc2011_v_spl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `kc2011_v_spl` AS select `t1`.`TeamName` AS `TeamA`,`t2`.`TeamName` AS `TeamB`,`s`.`SpielID` AS `SpielID`,`s`.`Team1` AS `Team1`,`s`.`Team2` AS `Team2`,`s`.`Tore1` AS `Tore1`,`s`.`Tore2` AS `Tore2`,`s`.`X` AS `X`,`s`.`Gruppe` AS `Gruppe` from ((`kc2011_tms` `t1` join `kc2011_tms` `t2`) join `kc2011_spl` `s`) where ((`t1`.`TeamID` = `s`.`Team1`) and (`t2`.`TeamID` = `s`.`Team2`)) order by `s`.`SpielID`;

--
-- Definition of view `kc2011_v_tbl`
--

DROP TABLE IF EXISTS `kc2011_v_tbl`;
DROP VIEW IF EXISTS `kc2011_v_tbl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `kc2011_v_tbl` AS select `t1`.`TeamID` AS `TeamID`,`t1`.`TeamName` AS `TeamName`,`t2`.`Gruppe` AS `Gruppe`,`t2`.`Spiele` AS `Spiele`,`t2`.`S` AS `S`,`t2`.`U` AS `U`,`t2`.`N` AS `N`,`t2`.`ToreP` AS `ToreP`,`t2`.`ToreM` AS `ToreM`,`t2`.`Punkte` AS `Punkte`,`t2`.`Status` AS `Status` from (`kc2011_tms` `t1` join `kc2011_tbl` `t2`) where (`t1`.`TeamID` = `t2`.`TeamID`) order by `t1`.`TeamID`;

--
-- Definition of view `o12_v_spl`
--

DROP TABLE IF EXISTS `o12_v_spl`;
DROP VIEW IF EXISTS `o12_v_spl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `o12_v_spl` AS select `t1`.`TeamName` AS `TeamA`,`t2`.`TeamName` AS `TeamB`,`s`.`SpielID` AS `SpielID`,`s`.`Team1` AS `Team1`,`s`.`Team2` AS `Team2`,`s`.`Tore1` AS `Tore1`,`s`.`Tore2` AS `Tore2`,`s`.`X` AS `X`,`s`.`Gruppe` AS `Gruppe` from ((`o12_tms` `t1` join `o12_tms` `t2`) join `o12_spl` `s`) where ((`t1`.`TeamID` = `s`.`Team1`) and (`t2`.`TeamID` = `s`.`Team2`)) order by `s`.`SpielID`;

--
-- Definition of view `o12_v_tbl`
--

DROP TABLE IF EXISTS `o12_v_tbl`;
DROP VIEW IF EXISTS `o12_v_tbl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `o12_v_tbl` AS select `t1`.`TeamID` AS `TeamID`,`t1`.`TeamName` AS `TeamName`,`t2`.`Gruppe` AS `Gruppe`,`t2`.`Spiele` AS `Spiele`,`t2`.`S` AS `S`,`t2`.`U` AS `U`,`t2`.`N` AS `N`,`t2`.`ToreP` AS `ToreP`,`t2`.`ToreM` AS `ToreM`,`t2`.`Punkte` AS `Punkte`,`t2`.`Status` AS `Status` from (`o12_tms` `t1` join `o12_tbl` `t2`) where (`t1`.`TeamID` = `t2`.`TeamID`) order by `t1`.`TeamID`;

--
-- Definition of view `olympiade2011_v_spl`
--

DROP TABLE IF EXISTS `olympiade2011_v_spl`;
DROP VIEW IF EXISTS `olympiade2011_v_spl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `olympiade2011_v_spl` AS select `t1`.`TeamName` AS `TeamA`,`t2`.`TeamName` AS `TeamB`,`s`.`SpielID` AS `SpielID`,`s`.`Team1` AS `Team1`,`s`.`Team2` AS `Team2`,`s`.`Tore1` AS `Tore1`,`s`.`Tore2` AS `Tore2`,`s`.`X` AS `X`,`s`.`Gruppe` AS `Gruppe` from ((`t_teams` `t1` join `t_teams` `t2`) join `olympiade2011_spl` `s`) where ((`t1`.`TurnierPos` = convert(`s`.`Team1` using utf8)) and (`t2`.`TurnierPos` = convert(`s`.`Team2` using utf8))) order by `s`.`SpielID`;

--
-- Definition of view `olympiade2011_v_tbl`
--

DROP TABLE IF EXISTS `olympiade2011_v_tbl`;
DROP VIEW IF EXISTS `olympiade2011_v_tbl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `olympiade2011_v_tbl` AS select `t1`.`TeamID` AS `TeamID`,`t1`.`TeamName` AS `TeamName`,`t2`.`Team1` AS `Team1`,`t2`.`Gruppe` AS `Gruppe`,`t2`.`Spiele` AS `Spiele`,`t2`.`S` AS `S`,`t2`.`U` AS `U`,`t2`.`N` AS `N`,`t2`.`ToreP` AS `ToreP`,`t2`.`ToreM` AS `ToreM`,`t2`.`Punkte` AS `Punkte`,`t2`.`Status` AS `Status` from (`t_teams` `t1` join `olympiade2011_tbl` `t2`) where (`t1`.`TurnierPos` = convert(`t2`.`Team1` using utf8)) order by `t2`.`Gruppe`;

--
-- Definition of view `olympiade2012_v_spl`
--

DROP TABLE IF EXISTS `olympiade2012_v_spl`;
DROP VIEW IF EXISTS `olympiade2012_v_spl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `olympiade2012_v_spl` AS select `t1`.`TeamName` AS `TeamA`,`t2`.`TeamName` AS `TeamB`,`s`.`SpielID` AS `SpielID`,`s`.`Team1` AS `Team1`,`s`.`Team2` AS `Team2`,`s`.`Tore1` AS `Tore1`,`s`.`Tore2` AS `Tore2`,`s`.`X` AS `X`,`s`.`Gruppe` AS `Gruppe` from ((`olympiade2012_tms` `t1` join `olympiade2012_tms` `t2`) join `olympiade2012_spl` `s`) where ((`t1`.`TeamID` = `s`.`Team1`) and (`t2`.`TeamID` = `s`.`Team2`)) order by `s`.`SpielID`;

--
-- Definition of view `olympiade2012_v_tbl`
--

DROP TABLE IF EXISTS `olympiade2012_v_tbl`;
DROP VIEW IF EXISTS `olympiade2012_v_tbl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `olympiade2012_v_tbl` AS select `t1`.`TeamID` AS `TeamID`,`t1`.`TeamName` AS `TeamName`,`t2`.`Gruppe` AS `Gruppe`,`t2`.`Spiele` AS `Spiele`,`t2`.`S` AS `S`,`t2`.`U` AS `U`,`t2`.`N` AS `N`,`t2`.`ToreP` AS `ToreP`,`t2`.`ToreM` AS `ToreM`,`t2`.`Punkte` AS `Punkte`,`t2`.`Status` AS `Status` from (`olympiade2012_tms` `t1` join `olympiade2012_tbl` `t2`) where (`t1`.`TeamID` = `t2`.`TeamID`) order by `t1`.`TeamID`;

--
-- Definition of view `rsc11_v_spl`
--

DROP TABLE IF EXISTS `rsc11_v_spl`;
DROP VIEW IF EXISTS `rsc11_v_spl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `rsc11_v_spl` AS select `t1`.`TeamName` AS `TeamA`,`t2`.`TeamName` AS `TeamB`,`s`.`SpielID` AS `SpielID`,`s`.`Team1` AS `Team1`,`s`.`Team2` AS `Team2`,`s`.`Tore1` AS `Tore1`,`s`.`Tore2` AS `Tore2`,`s`.`X` AS `X`,`s`.`Gruppe` AS `Gruppe` from ((`t_teams` `t1` join `t_teams` `t2`) join `rsc11_spl` `s`) where ((`t1`.`TurnierPos` = convert(`s`.`Team1` using utf8)) and (`t2`.`TurnierPos` = convert(`s`.`Team2` using utf8))) order by `s`.`SpielID`;

--
-- Definition of view `rsc11_v_tbl`
--

DROP TABLE IF EXISTS `rsc11_v_tbl`;
DROP VIEW IF EXISTS `rsc11_v_tbl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `rsc11_v_tbl` AS select `t1`.`TeamID` AS `TeamID`,`t1`.`TeamName` AS `TeamName`,`t2`.`Team1` AS `Team1`,`t2`.`Gruppe` AS `Gruppe`,`t2`.`Spiele` AS `Spiele`,`t2`.`S` AS `S`,`t2`.`U` AS `U`,`t2`.`N` AS `N`,`t2`.`ToreP` AS `ToreP`,`t2`.`ToreM` AS `ToreM`,`t2`.`Punkte` AS `Punkte`,`t2`.`Status` AS `Status` from (`t_teams` `t1` join `rsc11_tbl` `t2`) where (`t1`.`TurnierPos` = convert(`t2`.`Team1` using utf8)) order by `t2`.`Gruppe`;

--
-- Definition of view `rsc12_v_spl`
--

DROP TABLE IF EXISTS `rsc12_v_spl`;
DROP VIEW IF EXISTS `rsc12_v_spl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `rsc12_v_spl` AS select `t1`.`TeamName` AS `TeamA`,`t2`.`TeamName` AS `TeamB`,`s`.`SpielID` AS `SpielID`,`s`.`Team1` AS `Team1`,`s`.`Team2` AS `Team2`,`s`.`Tore1` AS `Tore1`,`s`.`Tore2` AS `Tore2`,`s`.`X` AS `X`,`s`.`Gruppe` AS `Gruppe` from ((`rsc12_tms` `t1` join `rsc12_tms` `t2`) join `rsc12_spl` `s`) where ((`t1`.`TeamID` = `s`.`Team1`) and (`t2`.`TeamID` = `s`.`Team2`)) order by `s`.`SpielID`;

--
-- Definition of view `rsc12_v_tbl`
--

DROP TABLE IF EXISTS `rsc12_v_tbl`;
DROP VIEW IF EXISTS `rsc12_v_tbl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `rsc12_v_tbl` AS select `t1`.`TeamID` AS `TeamID`,`t1`.`TeamName` AS `TeamName`,`t2`.`Gruppe` AS `Gruppe`,`t2`.`Spiele` AS `Spiele`,`t2`.`S` AS `S`,`t2`.`U` AS `U`,`t2`.`N` AS `N`,`t2`.`ToreP` AS `ToreP`,`t2`.`ToreM` AS `ToreM`,`t2`.`Punkte` AS `Punkte`,`t2`.`Status` AS `Status` from (`rsc12_tms` `t1` join `rsc12_tbl` `t2`) where (`t1`.`TeamID` = `t2`.`TeamID`) order by `t1`.`TeamID`;

--
-- Definition of view `rsc_2012_v_spl`
--

DROP TABLE IF EXISTS `rsc_2012_v_spl`;
DROP VIEW IF EXISTS `rsc_2012_v_spl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `rsc_2012_v_spl` AS select `t1`.`TeamName` AS `TeamA`,`t2`.`TeamName` AS `TeamB`,`s`.`SpielID` AS `SpielID`,`s`.`Team1` AS `Team1`,`s`.`Team2` AS `Team2`,`s`.`Tore1` AS `Tore1`,`s`.`Tore2` AS `Tore2`,`s`.`X` AS `X`,`s`.`Gruppe` AS `Gruppe` from ((`rsc_2012_tms` `t1` join `rsc_2012_tms` `t2`) join `rsc_2012_spl` `s`) where ((`t1`.`TeamID` = `s`.`Team1`) and (`t2`.`TeamID` = `s`.`Team2`)) order by `s`.`SpielID`;

--
-- Definition of view `rsc_2012_v_tbl`
--

DROP TABLE IF EXISTS `rsc_2012_v_tbl`;
DROP VIEW IF EXISTS `rsc_2012_v_tbl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `rsc_2012_v_tbl` AS select `t1`.`TeamID` AS `TeamID`,`t1`.`TeamName` AS `TeamName`,`t2`.`Gruppe` AS `Gruppe`,`t2`.`Spiele` AS `Spiele`,`t2`.`S` AS `S`,`t2`.`U` AS `U`,`t2`.`N` AS `N`,`t2`.`ToreP` AS `ToreP`,`t2`.`ToreM` AS `ToreM`,`t2`.`Punkte` AS `Punkte`,`t2`.`Status` AS `Status` from (`rsc_2012_tms` `t1` join `rsc_2012_tbl` `t2`) where (`t1`.`TeamID` = `t2`.`TeamID`) order by `t1`.`TeamID`;

--
-- Definition of view `test_v_spl`
--

DROP TABLE IF EXISTS `test_v_spl`;
DROP VIEW IF EXISTS `test_v_spl`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `test_v_spl` AS select `t1`.`TeamName` AS `TeamA`,`t2`.`TeamName` AS `TeamB`,`s`.`SpielID` AS `SpielID`,`s`.`Team1` AS `Team1`,`s`.`Team2` AS `Team2`,`s`.`Tore1` AS `Tore1`,`s`.`Tore2` AS `Tore2`,`s`.`X` AS `X`,`s`.`Gruppe` AS `Gruppe` from ((`t_teams` `t1` join `t_teams` `t2`) join `test_spl` `s`) where ((`t1`.`TurnierPos` = convert(`s`.`Team1` using utf8)) and (`t2`.`TurnierPos` = convert(`s`.`Team2` using utf8)) and (`t1`.`TurnierName` = 'test') and (`t2`.`TurnierName` = 'test')) order by `s`.`SpielID`;



/*!40101 SET SQL_MODE=@OLD_SQL_MODE */;
/*!40014 SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS */;
/*!40014 SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS */;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
