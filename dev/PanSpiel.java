...
public class PanSpiel extends JPanel
{
	...
	public PanSpiel()
	{
		...
		setSize(new Dimension(860, 60));
		setPreferredSize(new Dimension(860, 50));
	}
	...
	public void setSpiel(ResultSet myResultSet)
	{
		try
		{
			myLabel[0].setIcon(myIcons.getIcon(myResultSet.getString("TeamA"), "_Wappen", 40));
			myLabel[0].setName(String.valueOf(myResultSet.getString("SpielID")));
			myLabel[1].setText(myResultSet.getString("TeamA"));
			myLabel[3].setIcon(myIcons.getIcon(myResultSet.getString("TeamB"), "_Wappen", 40));
			myLabel[4].setText(myResultSet.getString("TeamB"));
	...
	{
/*		int intSpielID = Integer.parseInt(myLabel[0].getName());
		System.out.println("PanSpiel.getSpielID() " + intSpielID);
		if(intSpielID > 0)
		{
			return intSpielID;
		}
