...
//		System.out.println("CountDown.5MinSirene: " + sirene5Min);
	...
	{
		if((tempZeit < 5 * 60 * 1000) && (sirene5Min == true))
		{
//			System.out.println("CountDown: löse Alarm aus bei " + tempZeit); //--> Zeit war negativ
			if(TestOMat.instanz.myMP3Thread != null) 
			{
				TestOMat.instanz.myMP3Thread.stop();
				TestOMat.instanz.myMP3Thread = new MP3Thread(srn5MinPfad, 100);
			}
			TestOMat.instanz.myMP3Thread.start();
			sirene5Min 	= false;
		}
		if((tempZeit < 0 * 60 * 1000) && (sireneEnde == true))
		{
			if(TestOMat.instanz.myMP3Thread != null) 
			{
				TestOMat.instanz.myMP3Thread.stop();
				TestOMat.instanz.myMP3Thread = new MP3Thread(srnEndePfad, 100);
			}
			TestOMat.instanz.myMP3Thread.start();
			sireneEnde 	= false;
		}
