...
//	ImageIcon[]	icnWappen		= new ImageIcon[8];
	...
	public PanGruppe(String strTurnierName, String strGruppe)
	{
		this.strTurnierName = strTurnierName;
		this.strGruppe		= strGruppe;
		
		setLayout(null);
		intTeamAnz = Integer.parseInt(myAccess.select("KC", "turniere", "AnzTeams", "aktiv = 1"));
...
//			labTablle[i][0].setBackground(Color.white);
	...
	public void refresh()
	{
		myResultSet = myAccess.getSet("kc", strTurnierName + "_v_tbl", "*", "Gruppe = '" + strGruppe + "'" + " ORDER BY Punkte DESC, Spiele DESC, ToreP-ToreM DESC ");
		try
		{
			int i = 0;
			while(myResultSet.next() && i < intTeamAnz)	//Abfangen: nur 8 Teams pro Gruppe möglich?
			{
				...
				{
					labTablle[i][0].setIcon(myIcons.getIcon("_default" + sep + myResultSet.getString("TeamName"), "_Wappen", 40));					
				}
				else
				{
